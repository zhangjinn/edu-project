...
import {
  ...
  addScheduleCategoryType,
  updateScheduleCategoryType,
  removeScheduleCategoryType,
  updateScheduleCategorySort
...
export default {
  ...
  methods: {
    getData () {
      this.dragData = []
      queryScheduleCategoryList().then(res => {
        res.data.forEach(v => {
          this.dragData.push({
            text: v.name,
            id: v.id
    ...
    deleteItem (e) {
      ...
      removeScheduleCategoryType({
        scheduleTypeId: this.dragData[e[0].index].id
      }).then(res => {
    ...
    updateNew (index) {
      ...
      const data = {
        scheduleTypeName: target.text,
        sort: index
      }
      if (target.id) {
        // 存在id，修改
        data.scheduleTypeId = target.id
        updateScheduleCategoryType(data).then(res => {
        ...
        // 不存在id，新增
        addScheduleCategoryType(data).then(res => {
          const result = res.data
          target.id = result.id
        }).catch(err => {
          console.error(err)

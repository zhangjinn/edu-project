...
export default {
  ...
  methods: {
    ...
    handleChange () {
      ...
      courseSubjectList.scopeSortIndexList = ids
      updateScopeSort(courseSubjectList).then(res => {
    ...
    deleteItem (e) {
      ...
      removeScopeType({
        scopeId: this.dragData[e[0].index].id
      }).then(res => {
    ...
    updateNew (index) {
      ...
      const data = {
        enable: true,
        scopeName: target.text
      }
      if (target.id) {
        // 存在id，修改
        data.id = target.id
        updateScopeType(data).then(res => {

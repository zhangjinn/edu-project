<template>
  <div class="captchaLogin">
    <div class="container">
      <p class="textLeft">短信验证登录</p>
      <div class="inputBox textLeft">
        <van-cell-group :border="false">
          <van-field
            v-model="phone"
            type="tel"
            input-align="left"
            placeholder="请输入手机号"
            :class="active === 1 ? 'focusBorder' : ''"
            @focus="addClass(1)"
            @blur="removeClass"
          />
          <van-field
            v-model="picCode"
            class="codeCell"
            input-align="left"
            placeholder="请输入图片中的安全码"
            :class="active === 2 ? 'focusBorder' : ''"
            @focus="addClass(2)"
            @blur="removeClass"
          >
            <van-button
              slot="button"
              size="small"
              tag="span"
              class="codeBtn"
              @click.stop="pictureCaptcha()"
            >
              <img :src="imgCaptcha">
            </van-button>
          </van-field>
          <van-field
            v-model="captcha"
            input-align="left"
            placeholder="请输入验证码"
            :class="active === 3 ? 'focusBorder' : ''"
            @focus="addClass(3)"
            @blur="removeClass"
          >
            <van-button
              v-if="sendType === 0"
              slot="button"
              size="small"
              tag="span"
              class="verifyBtn"
              :disabled="verifyDisabled"
              @click="sendMsg()"
            >发送验证码
            </van-button>
            <van-button
              v-else-if="sendType === 1"
              slot="button"
              size="small"
              tag="span"
              class="verifyBtn disBtn"
            >发送中 {{ countdown }}s
            </van-button>
            <van-button
              v-else
              slot="button"
              size="small"
              tag="span"
              class="verifyBtn"
              :disabled="verifyDisabled"
              @click="sendMsg"
            >重新发送
            </van-button>
          </van-field>
        </van-cell-group>
      </div>
      <div class="btn" @click="login">
        <control-Btn text="登录" type="primary" :disabled="disabled" />
      </div>
      <div class="psdLogin textLeft"><span @click="toPsdLogin">账号密码登录</span></div>
    </div>
  </div>
</template>

<script>
import { getImgCaptcha, getSmsCaptcha, login } from '../../../../service/base/login'
import controlBtn from '../../../../components/common/controlBtn'
import { judgePhone, replaceSpace } from '../../../../js/rules'
import {
  setExpiresIn,
  setLoginInfo,
  setOrganization,
  setOrganizationId,
  setPhone,
  setToken,
  setTokenTime
} from '../../../../js/localStore'
import { getMenuList } from '@/js/menu'

export default {
  name: 'CaptchaLogin',
  components: {
    controlBtn
  },
  data() {
    return {
      sendType: 0,
      countdown: 60,
      sessionTime: 5,
      picCode: '',
      phone: '',
      captcha: '',
      imgCaptcha: '',
      imgCaptchaToken: '',
      active: 0
    }
  },
  computed: {
    verifyDisabled() {
      return !this.phone || !this.picCode
    },
    disabled() {
      return !this.phone || !this.picCode || !this.captcha
    }
  },
  watch: {
    sessionTime: function() {
      if (this.sessionTime === 0) {
        this.sessionTime = 5
      }
      sessionStorage.setItem('captchaLogin_countdown', this.countdown)
      sessionStorage.setItem('sendType', this.sendType)
    },
    captcha() {
      if (this.captcha.length > 6) {
        this.captcha = this.captcha.substring(0, 6)
      }
    }
  },
  created() {
    this.pictureCaptcha()
    const sessionCountdown = sessionStorage.getItem('captchaLogin_countdown')
    const sessionSendType = sessionStorage.getItem('sendType')
    if (sessionCountdown) {
      this.countdown = sessionCountdown
    }
    if (sessionSendType) {
      this.sendType = sessionSendType
      if (sessionSendType === 1) {
        this.countDownSendMsgTime()
      }
    }
  },
  methods: {
    toPsdLogin() {
      this.$router.replace({
        path: '/passwordLogin'
      })
    },
    addClass(i) {
      this.active = i
    },
    removeClass() {
      this.active = 0
      this.phone = replaceSpace(this.phone)
      this.picCode = replaceSpace(this.picCode)
      this.captcha = replaceSpace(this.captcha)
    },
    // 图片验证码
    pictureCaptcha() {
      getImgCaptcha().then(res => {
        this.imgCaptchaToken = res.headers.token
        return 'data:image/png;base64,' + btoa(
          new Uint8Array(res.data).reduce((data, byte) => data + String.fromCharCode(byte), '')
        )
      }).then(data => {
        this.imgCaptcha = data
      })
    },
    sendMsg() {
      if (this.verifyDisabled) return
      if (!judgePhone(this.phone)) {
        this.$toast({
          message: '手机号格式错误',
          duration: 1000,
          icon: 'icon iconfont iconwrong'
        })
        return
      }
      this.getCaptchaByPhone()
    },
    // 倒计时
    countDownSendMsgTime() {
      const _this = this
      _this.sendType = 1
      const time = window.setInterval(function() {
        if (_this.countdown > 0) {
          _this.countdown -= 1
          _this.sessionTime -= 1
        } else if (_this.countdown === 0) {
          _this.sendType = 2
          _this.countdown = 60
          window.clearInterval(time)
          sessionStorage.removeItem('captchaLogin_countdown')
          sessionStorage.removeItem('sendType')
        }
      }, 1000)
    },
    getCaptchaByPhone() {
      const param = {
        type: 'LOGIN',
        captcha: this.picCode,
        token: this.imgCaptchaToken
      }
      getSmsCaptcha(this.phone, param).then(data => {
        const expireTime = data.data.data
        setTimeout(() => {
          this.$toast({
            message: '验证码已发送,' + expireTime / 60 + '分钟内有效',
            duration: 1000
          })
          this.countDownSendMsgTime()
        }, 50)
      })
    },
    login() {
      if (this.disabled) return
      if (!judgePhone(this.phone)) {
        this.$toast({
          message: '手机号格式错误',
          duration: 1000,
          icon: 'icon iconfont iconwrong'
        })
        return
      }
      const param = {
        telephone: this.phone,
        smsCaptcha: this.captcha,
        type: 'SMS_CAPTCHA'
      }
      login(param).then(res => {
        const data = res.data.data
        setToken(data.token)
        setPhone(data.telephone)
        setLoginInfo(data.account)
        setOrganization(data.organizationList)
        setOrganizationId(data.currentOrganizationId)
        setExpiresIn(data.expiresIn)
        setTokenTime(new Date())
        localStorage.removeItem('tempAccount')
        setTimeout(() => {
          this.$toast({
            message: '登录成功',
            duration: 1000,
            icon: 'icon iconfont iconsuccess'
          })
        }, 50)
        setTimeout(() => {
          if (!data.hasPassword) {
            this.$router.replace({
              path: '/setPassword'
            })
          } else {
            getMenuList()
          }
        }, 1000)
      })
    }
  }
}
</script>

<style lang="scss">

    .captchaLogin {
        position: relative;
        min-height: 100vh;
        background-color: $white;

        img {
            height: auto;
        }

        .container {
            padding: 50px 35px;

            p {
                font-size: 23px;
                font-weight: 600;
            }

            .inputBox {
                margin: 40px 0;

                .van-cell {
                    padding: 25px 0 15px;
                }

                .codeCell {
                    .van-field__button {
                        height: 24px;
                    }

                    .codeBtn {
                        padding: 0;
                        @include wh(88px, 24px);
                        border: none;

                        .van-button__text {
                            display: inline-block;
                            height: 24px;
                        }
                    }
                }

                .verifyBtn {
                    @include wh(85px, 24px);
                    @include border-radius(12px);
                    line-height: 24px;
                    font-size: 13px;
                    color: $theme;
                    border: none;

                    &.disBtn {
                        color: $white;
                        background-color: $text-e;
                        border-color: $text-e;
                    }
                }

                .van-icon {
                    font-size: 24px;
                    color: $theme;
                }

            }

            .btn {
                margin: 0 auto;
            }

            .psdLogin {
                margin-top: 12px;
                color: $text-5;
                font-size: 13px;
            }
        }

    }
</style>

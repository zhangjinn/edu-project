      ...
      <div class="content">
        ...
        >
          <div class="top">
            <div class="iconBox">
              <span class="icon" />
              <span class="text">{{ item.examinationSettingsName }}</span>
            </div>
            <div class="time">{{ getTime(item.createDate) }}</div>
          </div>
          <div class="main">
            <div class="innerBox">
              <p class="state" :class="item.abnormal ? 'errorColor' : 'greenColor'">
                ...
                <span
                  v-if="item.abnormal"
                  class="degress"
                  :class="abnormalDegress(item.bodyTemperature) ? 'abnormal' : 'normal'"
                >{{ item.bodyTemperature }}°C</span>
              </p>
              <p v-if="item.checkSettingsNames" class="innerText errorColor">
                <span>症状：{{ item.checkSettingsNames }}</span>
              </p>
              <p v-if="!item.abnormal" class="innerText">您的宝宝非常健康哦~</p>
              <p v-if="item.remarks" class="innerText">{{ item.remarks }}</p>
...
import { queryStu, queryWeekCheck, queryCheckDetail } from '@/service/daycare/bodyCheck'
...
export default {
  ...
  methods: {
    ...
    getCustomer() {
      ...
      // })
      queryStu({ childId: this.childId }).then(res => {
        this.child = res.data.data
      })
    },
    subName(name) {
    ...
    getDetail() {
      ...
      //   }
      // })
      queryCheckDetail({ checkDate: this.currentDate, childId: this.childId }).then(res => {
        this.attendanceData = res.data.data
    ...
    getState() {
      ...
      //   })
      // })
      queryWeekCheck({ child: this.childId, localDate: this.currentDate }).then(res => {
        // console.log(res)
        const data = res.data.data
        data.forEach(item => {
          const date = DateFormat(new Date(item.date), 'yyyy-MM-dd')
          ...
          if (!item.exist) {
            if (this.markDate[1].date.indexOf(date) === -1) {
              this.markDate[1].date.push(date)
              this.disableClick.push(item.date)
            }
          } else if (item.abnormal) {
            if (this.markDate[0].date.indexOf(date) === -1) {
              this.markDate[0].date.push(date)
    ...
    disabledDate(date) {
      ...
      return this.disableClick.find(cur =>
        timestamp === cur
      )
    },
    showImg(img) {

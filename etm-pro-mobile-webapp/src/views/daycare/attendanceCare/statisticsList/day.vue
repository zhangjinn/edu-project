...
import { queryDayCountDetail } from '@/service/daycare/attendanceCare'
...
export default {
  ...
  data() {
    return {
      tabActive: 0,
      tabList: ['已考勤', '未考勤', '已请假'],
      tabTypeList: [['IN_SCHOOL', 'OUT_SCHOOL'], 'NOT_ATTENDANCE', 'ASK_FOR_LEAVE'],
      statusList: {
        NOT_ATTENDANCE: '未考勤',
        IN_SCHOOL: '已入园',
        OUT_SCHOOL: '已离园',
  ...
  created() {
    const type = this.$route.query.tabActiveType
    if (type === 'leave') {
      this.tabActive = 2
    ...
    if (this.$route.query.courseManagementId) {
      this.courseManagementId = parseInt(this.$route.query.courseManagementId) || null
    }
  ...
  methods: {
    ...
    queryData() {
      ...
      const type = this.tabTypeList[this.tabActive]
    ...
    confirmCourse(val, type) {
      this.courseManagementId = val.value || null
      if (type === 'load') return
      this.queryData()
    },
    // 日期切换

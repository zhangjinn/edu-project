...
import { queryPerformed, createPerformed, removePerformed } from '@/service/daycare/commentTeach'
...
export default {
  ...
  watch: {
    ...
    schoolBehaviorList: {
      handler(v) {
        this.settingItems[0].value = `已有${v.length}项`
      },
      immediate: true
    }
  },
  ...
  methods: {
    getHomeBehaviorList() { // 获取在园表现项
      queryPerformed({ whichPerformedTypeEnum: 'IN_THE_PARK' }).then(res => {
        this.schoolBehaviorList = res.data.data
      })
    },
    getSchoolBehaviorList() { // 获取在家表现项
      queryPerformed({ whichPerformedTypeEnum: 'AT_HOME' }).then(res => {
    ...
    deleteItem(index, item) {
      removePerformed({ whichPerformedId: item.whichPerformedId }).then(res => {
        this.$toast('删除成功')

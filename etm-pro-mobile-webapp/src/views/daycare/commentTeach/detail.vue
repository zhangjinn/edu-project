...
import { queryPerformed, createReviews, queryReviews, queryRemind } from '@/service/daycare/commentTeach'
// import {createReviews} from "@/service/daycare/commentCare";
export default {
  components: {
    settingTitle
  },
  ...
  methods: {
    queryBehaviorList() { // 获取评价项
      queryPerformed({ whichPerformedTypeEnum: { school: 'IN_THE_PARK', home: 'AT_HOME' }[this.tabsValue] }).then(res => {
    ...
    getBehaviorDetail() { // 获取表现详情
      queryReviews({ reviewsId: this.id }).then(res => {
        const data = res.data.data
        const imgArr = []
    ...
    handleComment() {
      ...
      if (this.tabsValue === 'home') {
        ...
        console.log(this.sentMsg.childId)
        console.log(this.sentMsg.courseManagementId)
        queryRemind({
          childId: this.sentMsg.childId,
          type: 'TEACH',
          courseManagementId: this.sentMsg.courseManagementId
        }).then(() => {
          this.$toast('提醒成功！')
        })
      }

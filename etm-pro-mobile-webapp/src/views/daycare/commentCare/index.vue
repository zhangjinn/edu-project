<template>
  <div class="index-wrap">
    ...
    <van-dropdown-menu active-color="#10C2C4">
      ...
      <van-dropdown-item v-model="value" :options="option" @change="changeValue" />
    </van-dropdown-menu>
...
<script>
import { queryFrequency, queryFindByDate, queryClass, queryReviews } from '@/service/daycare/commentCare'
  ...
  methods: {
    getFrequency() {
      queryFrequency().then(res => {
        // console.log(res.data.data.frequencyType)
        this.frequency = res.data.data.frequencyType
        this.frequencyId = res.data.data.frequencyId
        this.show_time = this.frequency !== 'DAILY'
        this.getFindByDate()
      })
    },
    getFindByDate() {
      queryFindByDate({ type: this.frequency }).then(res => {
        // console.log(res.data.data)
        // this.columns = res.data.data
        this.choseTime = JSON.parse(JSON.stringify(res.data.data))
        this.pickTime = res.data.data
        this.pickTime.forEach(v => {
          v.endLocalDate = DateFormat(v.endLocalDate, 'yyyy-MM-dd')
          v.startLocalDate = DateFormat(v.startLocalDate, 'yyyy-MM-dd')
          this.columns.push(v.startLocalDate + '至' + v.endLocalDate)
          this.classTime.push({ endLocalDate: v.endLocalDate, startLocalDate: v.startLocalDate })
        })
        // console.log(this.choseTime)
        if (this.show_time) {
          for (let i = 0; i < this.choseTime.length; i++) {
            if (this.choseTime[i].startLocalDate <= new Date().getTime() && this.choseTime[i].endLocalDate >= new Date().getTime()) {
              this.data = this.columns[i]
              this.startLocalDate = this.classTime[i].startLocalDate
              this.endLocalDate = this.classTime[i].endLocalDate
              this.showIndex = i
              // return
            }
          }
          // console.log(this.data)
          // console.log(this.startLocalDate)
          // console.log(this.endLocalDate)
        } else {
          this.startLocalDate = this.data
          this.endLocalDate = this.data
        }
        // console.log(this.data)
        this.getClass()
      })
    ...
    getClass() {
      queryClass({ startLocalDate: this.startLocalDate, endLocalDate: this.endLocalDate }).then(res => {
        // console.log(res.data.data)
    ...
    getReviews() {
      ...
      queryReviews({
        ...
        caScourseManagementId: this.value,
        reviewTypeId: this.frequencyId,
    ...
    toDetail(item) {
      this.weekIdList.reviewTypeId = this.frequencyId
      this.weekIdList.startLocalDate = this.startLocalDate
      this.weekIdList.endLocalDate = this.endLocalDate
      this.weekIdList.caScourseManagementId = this.value
      ...
      if (this.tabsValue === 'school') {
        if (item.reviewStatus !== 'UNREAD' && item.reviewsId && item.whetherToFillIn) { // 已经点评，显示点评详情
          this.$router.push(`/daycare_commentCareDetail?type=${this.tabsValue}&id=${item.reviewsId}`)
        } else { // 未点评，去写点评
          this.$router.push(`/daycare_commentCareDetail?type=${this.tabsValue}`)
        }
      } else {
      ...
      // this.$router.push(`/daycare_commentCareDetail`)
    },
    footBtnClick() {

<template>
  <div class="drag-wrap">
    <div ref="addMealsRef" class="add-meals" :size="size" @click="showType">
      <van-icon name="plus" />
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    this.$refs.addMealsRef.addEventListener('touchstart', event => {
      ...
      // console.log(screenWidth, '窗口宽度')
      // console.log(screenHeight, '窗口高度')
      // 获取当前触摸坐标
      const currentX = event.touches[0].pageX
      const currentY = event.touches[0].pageY
      // console.log(currentX, 'x坐标')
      // console.log(currentY, 'y坐标')
      // 获取元素的大小
      ...
      this.$refs.addMealsRef.addEventListener('touchmove', event => {
        ...
        // console.log(dynamicX, '当前鼠标的坐标x')
        // console.log(dynamicY, '当前鼠标的坐标Y')
        // 计算元素要移动的坐标
        ...
        if (moveX < 0) {
          ...
          // x坐标往右移会飞出去，需要判断,这个是最右侧的x坐标
          moveX = screenWidth - elementWidth
          // console.log(moveX - screenWidth - elementWidth, '当前鼠标的坐标x')
        }
        if (moveY < 0) {
          // 到达坐标（0,0）会飞出去，需要判断
          moveY = 0
        } else if (moveY > screenHeight - elementHeight) {
          // y坐标往下移会飞出去，需要判断,这个是最下侧的y坐标
          moveY = screenHeight - elementHeight
          // console.log(moveY - screenHeight - elementHeight, '当前鼠标的坐标Y')
        }
        // 将坐标赋值给元素

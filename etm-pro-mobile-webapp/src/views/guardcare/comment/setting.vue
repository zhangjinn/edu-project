      ...
      <div class="content">
        <ul>
          <li v-for="(item, index) in behaviorList" :key="index">
            <span>{{ item.performanceItemName || '-' }}</span>
...
export default {
  ...
  methods: {
    getHomeBehaviorList() { // 获取在园表现项
      queryBehaviorList({ type: 'IN_HOME' }).then(res => {
    ...
    getSchoolBehaviorList() { // 获取在家表现项
      queryBehaviorList({ type: 'IN_SCHOOL' }).then(res => {
    ...
    addItem() {
      ...
      createBehaviorItem({
        performanceItemName: this.addItemText,
        type: { school: 'IN_SCHOOL', home: 'IN_HOME' }[this.state]
      }).then(res => {
        this.$toast('新增成功')
    ...
    deleteItem(index, item) {
      removeBehaviorItem({
        performanceItemId: item.performanceItemId
      }).then(res => {
        this.$toast('删除成功')

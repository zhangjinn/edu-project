...
import { getCheckState, getCheckCustomerList, postCheck } from '../../../../service/guardcare/check'
    ...
    checkAbnormal() {
      ...
      if (this.abnormalData.imgUrl.length) {
        ...
        this.abnormalData.imgUrl.forEach(item => {
          if (item.file) { // 新上传图片
            uploadFile(item.file, 'guardcare/check/abnormal', 'image').then(res => {
              const img = {
                name: item.file.name,
                url: res.data.key
              }

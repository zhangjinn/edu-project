  ...
  <div class="postDynamics">
    ...
    <div class="content">
      <div class="header">
        ...
        <div class="upload">
          <!--          如果是上传图片使用uploader，如果是上传视频使用video-->
          <van-uploader
        ...
        <div class="comment">
          <div class="comment-left">
            <span class="bigFont">发布班级</span>
          </div>
          <div class="comment-right basis" @click="showSelectPopup">
            <span v-if="classAuthLen <= 1 && classList[0]" class="smallFont">{{
              classList[0].clazzes[0].name
            }}</span>
            <span
              v-else-if="subSelectClassIdList.length <= 3"
              class="smallFont"
            >{{ selectText !== '' ? selectText : '请选择班级' }}</span>
            <span v-else class="smallFont">{{ `${ selectText }...` }}</span></div>
        </div>
    ...
    <van-popup v-model="selectPopup" class="selectClass" position="bottom" :style="{height:'80%'}">
      <div class="header">
        <span>选择班级</span>
      ...
      <div class="content">
        <van-collapse v-model="selectClassName">
          <van-checkbox-group v-model="selectClassList">
            <van-collapse-item v-for="grade in classList" :key="grade.id" :title="grade.name" :name="grade.id ">
              <template #title>
                <div>
                  {{ grade.name }}
                </div>
              ...
              <van-checkbox-group v-model="subSelectClassIdList">
                <div v-for="(subClazz) in grade.clazzes" :key="subClazz.id">
                  <van-checkbox
...
import { createDynamic, queryClassAuth } from '@/service/guardcare/dynamic'
...
export default {
  ...
  async mounted() {
    ...
    if (this.$route.params.type) {
      ...
      // console.log(this.uploadType, 'uploadType')
    }
    // console.log(this.fileInfoList, 'THI')
  },
  methods: {
    // 截取video的封面
    createCover() {
      ...
      // 生成video的封面
      uploadFile(file, 'guardcare/matterList/dynamics', 'image').then(res => {
      ...
      for (const grade of this.classList) {
        for (const clazz of grade.clazzes) {
          ...
          if (this.selectAllClass) {
            clazz.subCheck = true
            this.subSelectClassIdList.push(clazz.id)
            this.subSelectItemList.push(clazz)
            this.selectClassList.push(grade.id)

            this.subSelectClassIdList = [...new Set(this.subSelectClassIdList)]
            this.selectClassList = [...new Set(this.selectClassList)]
    ...
    selelctSimpleClass(subClazz, grade, subCheck) {
      ...
      if (subClazz.subCheck) {
        this.subSelectClassIdList.push(subClazz.id)
        ...
        // console.log(this.subSelectClassIdList, '删除')
      }
    },
    // 确认选中班级
    sureClass() {
    ...
    createDyn() {
      ...
      params.allowComments = this.allowComments
      params.fileInfoList = this.uploadList
      params.clazzIdList = this.subSelectClassIdList
      params.type = this.uploadType.toUpperCase()
      ...
      createDynamic(params).then(() => {
        ...
        setTimeout(() => {
          this.$router.push('/guardcare_clazzPost')
        }, 200)
      })
    },
    ...
    async upload(fileInfoList) {
      ...
      if (fileInfoList instanceof Array) {
        fileInfoList.map(async(item, index) => {
          await uploadFile(item.file, 'guardcare/matterList/dynamics', this.uploadType).then((res) => {
            // 获取到的key值作为url,type上传类型,有封面上传封面

            const obj = {}
            obj.url = res.data.key
            obj.type = this.uploadType.toUpperCase()
            obj.coverUrl = this.videoCover

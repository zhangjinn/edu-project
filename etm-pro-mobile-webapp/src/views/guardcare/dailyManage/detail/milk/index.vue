<template>
  <div class="milk">
    ...
    <div class="scrollWrap">
      <head-info ref="head" type="喂奶" :child-list="childList" />
      <div class="milkBox-wrap">
        <div class="content">
          <div class="imgBox">
            ...
            <div class="scaleBox">
              <img src="../../../../../assets/images/milk_scale.png">
      ...
      if (this.$refs.text.imgUrl.length) { // 需要上传图片
        if (this.$route.query.id) {
          ...
          this.$refs.text.imgUrl.forEach(v => {
            pArr.push(uploadFile(v.file, 'guardcare/dailyManage/milk', 'image'))
          })
          await Promise.all(pArr).then(res => {
            res.forEach(v => {
              imageList.push({
                name: 'guardcare/dailyManage/milk' + Date.now(),
                url: v.data.key
              })
            })
          }).catch(err => {

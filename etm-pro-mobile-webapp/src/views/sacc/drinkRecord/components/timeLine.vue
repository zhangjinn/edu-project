  ...
  <div class="timeLineWrap">
    ...
    <div class="wrapper">
      <div
        ...
        class="record"
      >
        <h6>{{ handleTime(time.date).date }}</h6>
        <ul>
          <li v-for="(record, rindex) in time.list" :key="rindex">
            <div class="time top">
              <i class="dot" />
              <span>{{ handleTime(record.drinkingTime).time }}</span>
              <span class="date">{{ handleTime(record.drinkingTime).date }}</span>
            </div>
            <div class="wrap">
              <div class="content">
                <p v-if="record.drinkingRecordName">饮料种类：{{ record.drinkingRecordName || '未知' }}</p>
                <p v-if="record.drinkingQuantity">饮用量：{{ record.drinkingQuantity + 'ml(毫升)' || '未知'
                }}</p>

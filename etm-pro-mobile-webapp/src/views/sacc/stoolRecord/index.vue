...
import { getStoolRecordList } from '../../../service/sacc/stoolRecord'
...
export default {
  ...
  methods: {
    refreshData() {
      ...
      const one = transferDate(Date.now() - 1000 * 60 * 60 * 24)
      ...
      getStoolRecordList({
        ...
        data.forEach(v => {
          ...
          if (v.stoolTime) {
            const time = transferDate(v.stoolTime)
            const gaptime = transferDate().gap(Date.now() - v.stoolTime)

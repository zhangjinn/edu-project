  ...
  <div class="dayStatictics">
    <div class="circleBox">
      <div class="locationBox">
        ...
        <div class="countBox">
          <p class="count">
            <span class="mainColor">{{ attendanceData.arrivedNumber || 0 }}</span>/{{
              attendanceData.shouldArriveNumber || 0 }}
          </p>
    ...
    <div class="counts van-hairline--top">
      <van-row>
        <van-col :span="8" @click="toList('leave')">
          ...
          <div class="num mainColor">{{ attendanceData.requestForLeaveNumber || 0 }}</div>
          <div class="txt">已请假</div>
        </van-col>
        <van-col :span="8" @click="toList('notArrive')">
          ...
          <div class="num errorColor">{{ attendanceData.notArrivedNumber || 0 }}</div>
          <div class="txt">未到园</div>
        </van-col>
        <van-col :span="8" @click="toList('arrive')">
          ...
          <div class="num enterColor">{{ attendanceData.arrivedNumber || 0 }}</div>
          <div class="txt">已到园</div>
        </van-col>
      </van-row>
    </div>
    <drag-button
...
export default {
  ...
  methods: {
    // 到统计明细页
    toList(type) {
      this.$store.commit('neuroo/SET_ACTIVE', type)
      this.$router.push({
        path: '/statisticsDay_neuroo',
        query: {
          school: this.attendanceData.schoolId,
          clazz: this.attendanceData.clazzId,
          date: this.date
        }
      })
    },

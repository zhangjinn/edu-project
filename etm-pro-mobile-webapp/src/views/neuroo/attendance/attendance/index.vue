...
import { getAttendanceDay, getAttendanceMonth } from '../../../../service/neuroo/attendance'
  ...
  methods: {
    ...
    getMonthData() {
      ...
      getAttendanceMonth(param).then(res => {
        this.monthData = res.data.data ? res.data.data.schoolList : []
        if (this.monthData.length === 1 && this.monthData[0].clazzStatistics.length === 1) {
          this.onlyOne = true
        } else {
          this.onlyOne = false
        }

  ...
  <div class="class-wrap">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button
            v-has="{authId : 'daycare.room.create'}"
            type="primary"
            @click="classOpen"
          >
            <i class="el-icon-plus" />&nbsp;&nbsp;新增教室
          </el-button>
        </div>
      ...
      >
        <template slot-scope="scoped">
          <el-button
            type="text"
            @click="editClassOpen(scoped.row.roomId)"
          >
            {{ scoped.row.roomName }}
          </el-button>
        </template>
      ...
      >
        <template slot-scope="scoped">
          <etm-text
            v-has="{authId : 'daycare.room.update'}"
            type="primary"
            @click="editClassOpen(scoped.row.roomId, true)"
          >
            编辑
          </etm-text>
          <etm-text
            v-has="{authId : 'daycare.room.remove'}"
            type="danger"
            @click="delectClass(scoped.row.roomId)"
          >
            删除
          </etm-text>
        </template>
    ...
    >
      <etm-layout-split>
        ...
        <template v-else>
          <div class="banner">
            <el-carousel
              v-if="classdetail.resultImg && classdetail.resultImg.length"
              height="200px"
            >
              <el-carousel-item

...
import { createCacContract, updateRecord } from '@/api/newDaycare/collectionRecord'
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      createCacContract({ orderNumber, phone, childName }).then(res => {
    ...
    link (row) {
      updateRecord({ collectionRecordId: this.collectionRecordId, orderId: row.cacContractId, orderNumber: row.contractNo, payOrderType: 'CARE_ORDER' }).then(() => {

<template>
  <div class="orderForm-wrap">
    <etm-title>
      账单信息 （{{ formData.orderList && formData.orderList.length }}人）
      <template slot="tool">
        <el-input
          v-model="searchValue"
      ...
      <template v-for="(item,index) in formData.orderList">
        ...
        >
          <el-row class="total">
            ...
            <el-col :span="6">
              订金：¥{{ formatMoney(item.depositAmount) || 0 }}
            </el-col>
            <el-col :span="6">
              实收总价：
    ...
    <div
      v-if="formData.orderList && formData.orderList.length === searchResult.length"
      class="noData"
    >

<template>
  <div class="charge-wrap">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button
            ...
            @click="openChargePop"
          >
            <i class="el-icon-plus" /> 发起收费
          </el-button>
          <el-button
            plain
            @click="exportOrderBunch"
          >
            导出Excel
          </el-button>
        </div>
      ...
      >
        <template slot-scope="scoped">
          <etm-status :type="getEtmStatus(transferStatus(scoped.row.status))">
      ...
      >
        <template slot-scope="scoped">
          ...
          <etm-text
            v-if="scoped.row.status === 'UN_FULL'"
            type="primary"
            @click="callPay(scoped.row)"
          >
            催缴
          </etm-text>
...
export default {
  ...
  methods: {
    ...
    cellClassNameMethod ({ row, columnIndex }) {
      if (columnIndex === 16) { // 状态栏
        if (row.status === 'UN_FULL') {
          return 'enabled'
        }
        if (row.status === 'FULL') {
          return 'disabled'
        }
        if (row.status === 'UN_AUDIT') {
          return 'danger'
        }
      }
    },

...
import { queryChildAttendanceList, queryChildAttendanceStatistics } from '@/api/newDaycare/childManageGuard'
  ...
  methods: {
    getStatistics () {
      if (this.childId) {
        ...
        queryChildAttendanceStatistics(param).then(res => {
          if (res.data) {
    ...
    getData () {
      if (this.childId) {
        ...
        param.date = parseTime(this.monthValue, '{y}-{m}-{d}')
        queryChildAttendanceList(param).then(res => {
          const data = res.data
          this.attendanceData = []
          data.forEach(item => {

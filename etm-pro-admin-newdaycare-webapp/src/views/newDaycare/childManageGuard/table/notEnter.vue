...
import {
  ...
  queryChildById,
  queryChildPendingList, createGuardDeposit
...
export default {
  ...
  methods: {
    ...
    addDepositConfirm () {
      this.$refs.childDeposit.$refs.form.$children[0].validate(valid => {
        if (valid) {
          const param = {
            money: this.depositData.money,
            paymentType: this.depositData.paymentType,
            childId: this.depositData.childId,
            remark: this.depositData.remark,
            dateCharge: parseTime(this.depositData.dateCharge, '{y}-{m}-{d}')
          }
          createGuardDeposit(param).then(() => {
            this.$message.success('订金添加成功')

      ...
      <etm-layout-split>
                ...
                >
                  <template slot-scope="{ row }">
                    <div class="book-name">
                      <div
                        class="img-left"
                        @click="bookDetail(row.bookManageId)"
                      >
                        <img
              ...
              <template slot="right">
                ...
                >
                  <template slot-scope="{ row }">
                    <etm-text
                      type="primary"
                      @click="editOperate(row.bookManageId)"
                    >
                      编辑
                    </etm-text>
                    <etm-text
                      type="primary"
                      @click="storeOperate(row.bookManageId)"
                    >
                      入库
                    </etm-text>
                    <etm-text
                      type="danger"
                      @click="deleteOperate(row.bookManageId)"
                    >
                      删除
                    </etm-text>
                  </template>
          ...
          <template v-else>
            ...
            >
              <etm-table-tool-bar slot="tool">
                ...
                <div slot="right">
                  <el-button
                    :disabled="!selectTableValue.length"
                    type="primary"
                    @click="storeDetailQRcodePop = true"
                  >
                    打印二维码
                  </el-button>
                </div>
            ...
            <div slot="default">
              ...
              <template v-if="Object.keys(bookDetailPopData).length !== 0 && !bookDetailPopEditStatus">
                ...
                >
                  <template slot="tool">
                    <el-button
                      v-if="activateName === 'libraryManage'"
                      size="mini"
                      plain
                      @click="bookDetailPopEdit"
                    >
                      编辑
                    </el-button>
                  </template>
              ...
              <template v-else-if="bookDetailPopEditStatus">
                ...
                <etm-form
                  ref="editBookDetailFormRef"
                  type="dialog"
                  :model="bookDetailPopDataDeepCopyData"
                >
                  <el-form-item label="封面">
                    <div class="cover-img">
                      <img
                        v-if="bookDetailPopDataDeepCopyData.cover[0].url"
                        :src="bookDetailPopDataDeepCopyData.cover[0].url"
                        alt=""
                      >
                      <img
                        v-else
                  ...
                  <el-form-item
                    label="编码"
                    prop="coding"
                    :rules="bookDetailPopEditFormRules.coding"
                  >
                  ...
                  <el-form-item
                    label="名称"
                    prop="bookManageName"
                    :rules="bookDetailPopEditFormRules.bookManageName"
                  >
                  ...
                  >
                    <el-select
                      v-model="bookDetailPopDataDeepCopyData.bookTypeId"
                      placeholder="请选择图书分类"
                    >
                      <el-option
                  ...
                  <el-form-item label="ISBN">
                    <el-input
                      v-model="bookDetailPopDataDeepCopyData.bookNumber"
        ...
        >
          ...
          <etm-field-label
            ...
            label="出版社"
          >
            {{ storeData.publishingHouse }}
          </etm-field-label>
          <etm-form
            ref="etmForm"
            :model="storeData"
            :rules="storeDataRuleForm"
            type="dialog"
          >
            <el-form-item
              label="入库位置"
            ...
            >
              <el-input
                v-model="storeData.storageLocation"
                placeholder="请输入本次入库位置"
              />
            </el-form-item>
            <el-form-item
              label="入库数量"
            ...
            >
              <el-input
                v-model="storeData.storageCount"
        ...
        >
          <etm-form
            ...
            ref="addBookFormRef"
            :model="addBookData"
            :rules="addBookDataFormRules"
            type="dialog"
          >
            <el-form-item label="封面">
              <div
                class="upload-cover"
                @click="$refs.uploadCoverRef.click()"
              >
                <img
                  v-if="addBookData.cover[0].url"
                  :src="addBookData.cover[0].url"
                  alt=""
                >
                <i
                  v-else
            ...
            >
              <el-input
                v-model="addBookData.coding"
                :disabled="addBookAutoGenerate"
                placeholder="请输入编码"
              />
              <div class="auto-generate">
                <el-checkbox
                  v-model="addBookAutoGenerate"
                  @change="autoGenerate"
                >
                  自动生成
                </el-checkbox>
            ...
            >
              <el-input
                v-model="addBookData.bookManageName"
                v-in-focus
                placeholder="请输入名称"
              />
            </el-form-item>
            ...
            >
              <el-select
                v-model="addBookData.bookTypeId"
                placeholder="请选择图书分类"
              >
                <el-option
                  v-for="item in storeDetailFormData[1].options"
            ...
            <el-form-item label="作者">
              <el-input
                v-model="addBookData.author"
                placeholder="请输入作者"
              />
            </el-form-item>
            <el-form-item label="出版社">
              <el-input
                v-model="addBookData.publishingHouse"
                placeholder="请输入出版社"
              />
            </el-form-item>
            <el-form-item label="ISBN">
              <el-input
                v-model="addBookData.bookNumber"
                placeholder="请输入ISBN码"
              />
            </el-form-item>
            <el-form-item
            ...
            >
              <el-input
                v-model="addBookData.storageLocation"
                placeholder="请输入入库位置"
              />
            </el-form-item>
            <el-form-item
            ...
            >
              <el-input
                v-model="addBookData.storageCount"
                placeholder="请输入入库数量"
              />
            </el-form-item>
            <el-form-item label="简介">
        ...
        >
          <div
            v-if="storeDetailQRCodeImageList.length !== 0"
            class="qrcode"
          >
            <div
...
export default {
  ...
  methods: {
    cellClassNameMethod ({ row, columnIndex }) {
      if (columnIndex === 6) { // 状态栏
        // if (row.bookStatus === '待确认') {
        //   return 'enabled'
        // }
        if (row.bookStatus === '未借出') {
          return 'disabled'
        }
        if (row.bookStatus === '已借出') {
          return 'enabled'
        }
        // if (row.bookStatus === '已报损') {
        //   return 'disabled'
        // }
      }
    },

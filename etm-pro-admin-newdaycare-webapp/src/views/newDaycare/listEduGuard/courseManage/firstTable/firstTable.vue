  ...
  <div>
    ...
    >
      <etm-table-tool-bar slot="tool">
        <el-button
          slot="right"
          v-has="{authId: 'guardcare.courseExtra.create'}"
          type="primary"
          @click="handleAdd"
        >
      ...
      >
        <template v-slot="scoped">
          <etm-text
            v-if="!hasAuthOfBool('guardcare.courseExtra.query')"
            type="normal"
          >
            {{ scoped.row.name }}
          </etm-text>
      ...
      >
        <template v-slot="scoped">
          <etm-text
            v-has="{authId: 'guardcare.courseExtra.update'}"
            type="primary"
          ...
          <etm-text
            v-has="{authId: 'guardcare.courseExtra.remove'}"
            type="danger"
    ...
    <etm-drawer :visible.sync="drawerVisible">
      <etm-layout-split line>
        ...
        >
          ...
          >
            <el-button
              v-has="{authId: 'guardcare.courseExtra.query'}"
              plain
              size="mini"
              @click="cancelDrawer"
            >
...
import { getCourseSpecial, postCourseSpecial, putCourseSpecial, delCourseSpecial, putCourseSpecialStatus } from '@/api/newDaycare/listEduGuardCourseManage'
export default {
  name: 'MaterialTable',
  components: {
    edit,
  ...
  methods: {
    ...
    handleDelete (row) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.courseId = row.id
          delCourseSpecial({ id: this.courseId }).then((res) => {
            this.$etmTip.close()
    ...
    switchChange (row) {
      ...
      putCourseSpecialStatus(param).then((res) => {
        this.reset()
      })
    },
      ...
      this.$refs.childPopForm.$refs.form.$children[0].validate(valid => {
        if (valid) {
          ...
          if (this.drawerType === 'add') {
            ...
            putCourseSpecial(param).then((res) => {
              // this.drawerVisible = false
              this.drawerType = 'detail'
              this.$message({
                message: '已修改',

  ...
  <div class="table">
    ...
    >
      <etm-table-tool-bar slot="tool">
        ...
        <el-button
          slot="right"
          v-has="{authId: 'guardcare.child.create'}"
          type="primary"
          @click="add"
        >
          <i
        ...
        <el-button
          slot="right"
          v-has="{authId: 'guardcare.childLeave.createBatch'}"
          plain
          @click.native="leaveBatch"
        >
          批量退园
        </el-button>
      </etm-table-tool-bar>
      ...
      >
        <template slot-scope="scoped">
          <etm-text
            v-has="{authId: 'guardcare.childClazzChange.update'}"
            type="primary"
          ...
          <etm-text
            v-has="{authId: 'guardcare.child.remove'}"
            type="danger"
...
import { getEnterChildList, childLeave, clazzAdjust, addChild, deleteChild } from '@/api/newDaycare/listEduGuardChildManage'
// import {
//   getClazzItem
  ...
  methods: {
    ...
    remove (row) { // 删除幼儿
      this.$etmTip.init({
        ...
        confirm: () => {
          deleteChild({ childId: row.childId }).then(() => {

<template>
  <div class="addCourse-wrap">
    ...
    >
      ...
      >
        <el-input
          v-model="courseForm.headTeacherName"
          disabled
          placeholder="班主任"
        />
      </el-form-item>
      ...
      >
        <el-select
          v-model="courseForm.helperTeacherIds"
          multiple
        >
          <el-option
            v-for="(item,index) of teacherList"
      ...
      >
        <el-select v-model="courseForm.classsroomId">
          <el-option
            v-for="(item,index) of classroomList"
            :key="index"
            :label="item.label"
            :value="item.value"
          />
      ...
      <!--        <el-date-picker v-model="courseForm.courseDate" type="date" placeholder="选择日期" />-->
      <!--      </el-form-item>-->
      ...
      <template v-else>
        ...
        >
          <el-select
            v-model="courseForm.weekdayList"
            multiple
            placeholder="请选择"
          >
            <el-option
    ...
    <template v-if="!!courseForm.courseId">
      ...
      <el-collapse-transition>
        ...
        >
            ...
            >
              <template v-slot="{row}">
                <etm-text
                  style="margin: 0;"
                  type="primary"
                  @click="cancelCourse(row,courseForm.expChildIdList)"
                >
                  取消排课
                </etm-text>
              </template>
          ...
          <el-button @click="handleShowAddExpChild">
            添加试听学员
          </el-button>
        </div>
      </el-collapse-transition>
    ...
    <template v-if="!!courseForm.courseId">
      ...
      <el-collapse-transition>
        ...
        >
            ...
            >
              <template v-slot="{row}">
                <etm-text
                  style="margin: 0;"
                  type="primary"
                  @click="cancelCourse(row,courseForm.formalChildIdList)"
                >
                  取消排课
                </etm-text>
              </template>
          ...
          <el-button @click="handleShowAddFormalChild">
            添加正式学员
          </el-button>
        </div>
      </el-collapse-transition>
    ...
    >
      <template v-if="showChildTable1">
        ...
        <div style="margin-bottom: 16px;">
          已选择 {{ selectedTable1.length }}
        </div>
        <etm-table
          :data="tableData3"
          selection
          @selection-change="(e) => selectedTable1 = e"
        >
          <el-table-column
            slot="left"
            label="学员姓名"
    ...
    >
      ...
      <etm-table
        :data="childTable2"
        selection
        @selection-change="(e) => selectedTable2 = e"
      >
        <el-table-column
          slot="left"
          label="学员姓名"
 ...
 */
// import { queryEarlyCourseList, queryEarlyCourseTypeList } from '@/api/daycare/courseManage'

import { queryNurseCourseList, queryNurseCourseTypeList } from '@/api/newDaycare/courseManage'

import baseMixin from '@/layout/mixin/baseMixin'
import { queryClassroomList, queryClasses } from '@/api/newDaycare/classes'
import { queryEmployeeSelectByTabCode } from '@/api/base/employee'
// import { queryCourseDetailById, queryScheduleCourseTime } from '@/api/daycare/scheduleManage'
import { queryCaCourseDetailById, queryCaScheduleCourseTime } from '@/api/newDaycare/nurseSchedule'
import EtmColor from '@/views/newDaycare/courseTimeManageTeach/components/etmColor'
import { unique } from '@/utils/arrUtils'
import { queryCaeChildHaveCourse, queryChildNotInPool } from '@/api/newDaycare/childManage'
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        // courseDate: { required: true, message: '请选择日期', trigger: 'change' },
  ...
  methods: {
    async init () {
      // TODO 别动这个流程
      // 课程类型
      await queryNurseCourseTypeList().then(res => {
        this.courseTypeList = this.convertDataToOptions(res.data, 'caeCourseTypeName', 'caeCourseTypeId')
      })
      // 班级列表
      await queryClasses({ clazzName: this.getValueByProp('clazzName', this.formData2) }).then(res => {
        this.classList = this.convertDataToOptions(res.data, 'caeClazzName', 'caeClazzId')
      ...
      // 时段
      await queryCaScheduleCourseTime().then(res => {
      ...
      if (this.courseManagementId) {
        queryCaCourseDetailById({ caScourseManagementId: this.courseManagementId }).then(async res => {
          if (!res.data.helperTeacherIds) {
            res.data.helperTeacherIds = []
          } else {
            res.data.helperTeacherIds = res.data.helperTeacherIds.split(',')
          }
          this.courseForm = Object.assign(this.courseForm, res.data)

          const { helperTeacherIds, courseId, colorCode } = this.courseForm
          this.courseForm.helperTeacherIds = helperTeacherIds.map(item => parseInt(item))
      ...
      queryCaScheduleCourseTime().then(res => {
        this.courseTimePeriodIdList = res.data.map(item => {
          return {
            ...
            value: item.caScourseTimePeriodId,
            ...item
          }
        })
    ...
    async handleCourseTypeChange () {
      ...
      await queryNurseCourseList({ caeCourseType: this.courseForm.courseTypeId }).then(res => {
    ...
    handleClazzChange (value) {
      const find = this.classList.find(item => item.caeClazzId === value)
      // console.log(find)
    ...
    searchTable2 () {
      ...
      queryCaeChildHaveCourse(require).then(res => {
    ...
    submit () {
      ...
      this.$refs.etmForm.$children[0].validate(valid => {
        if (valid) {
          ...
          this.courseForm.courseDate = this.date || this.courseForm.courseDate

          ...
          >
            ...
            >
              <template v-slot="scoped">
                <etm-status
                  v-if="scoped.row.status==='PENDING_REVIEW'"
                  type="danger"
                >
                  {{
                ...
                <etm-status
                  v-else-if="scoped.row.status==='PASSED'"
                  type="primary"
                >
                  {{
                    scoped.row.enumState
...
export default {
  ...
  methods: {
    toSetting () { // 跳转到设置页
      this.$router.push({ name: 'newdaycare_attendanceGuardcare_setting' })
    },
    getDiseaseList () {
      this.diseaseList = []

...
import { queryAreaList, queryTree, createOrganization, removeOrganization, updateOrganization } from '@/api/newDaycare/areaSetHead'
...
export default {
  ...
  methods: {
    ...
    init () {
      queryAreaList(this.pageInfo).then(res => {
      ...
      this.$refs.childPopForm.$refs.form.$children[0].validate(valid => {
        if (valid) {
          ...
          updateOrganization({
            ...
            areaName: formData.organizationName
          }).then(() => {
            this.init()
            this.$message.success('操作成功')

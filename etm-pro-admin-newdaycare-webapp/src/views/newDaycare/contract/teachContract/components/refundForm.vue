          ...
          <el-col :span="16">
            ...
            >
              <el-form-item
                label="实退总计"
                prop="refund"
                :rules="formValidate.nullValueRule(true, '请输入实退总计')"
              >
              ...
              <el-form-item
                label="退款方式"
                prop="refundId"
                :rules="formValidate.nullValueRule(true, '请选择退款方式')"
              >
...
import { queryPayType, queryTeachExitDetail, exitTeachContract } from '@/api/newDaycare/contract'
import { deepClone } from '@/views/newDaycare/contract/common'
import { parseTime } from '@/utils'
// import { exitTeachContract } from '@/api/employee'
...
export default {
  ...
  methods: {
    ...
    init (prop) {
      console.log(prop.id)
      queryTeachExitDetail(prop.id).then(res => {
        const { data } = res
    ...
    popConfirm () {
      this.$refs.form1.$children[0].validate(valid => {
        if (valid) {
          ...
          exitTeachContract(this.form).then(res => {
            this.isShow = false
            this.$message({
              type: 'success',
              message: '退费成功'

        ...
        >
          ...
          >
            <template slot-scope="scoped">
              <el-button
                type="text"
                @click="trigger({name : 'chargeDetail', arguments : {edit:false, id : scoped.row.tcContractId,deposit : scoped.row.contractStatusEnum === '待审核' ? true : false, pid : scoped.row.childId, contractType : scoped.row.contractType, contractStatusEnum : scoped.row.contractStatusEnum}}, false)"
              >
          ...
          >
            <template slot-scope="scoped">
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['退费', '作废'])"
                v-has="{authId: 'daycare.contractTeach.update'}"
                type="primary"
                @click="trigger({name : 'chargeDetail', arguments : {edit : true, id : scoped.row.tcContractId, pid : scoped.row.childId, deposit : scoped.row.contractStatusEnum === '待审核' ? true : false}}, true)"
              >
                编辑
              </etm-text>
              <!--              TODO: 隐藏收款功能-->
              <!--              <etm-text-->
              <!--                v-if="!scoped.row.pay && !exclude(scoped.row.contractStatusEnum, ['待审核', '审核未通过'])"-->
              <!--                type="primary"-->
              <!--                @click="showCollection(scoped.row)"-->
              <!--              >-->
              <!--                收款-->
              <!--              </etm-text>-->
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['退费', '作废', '正常', '过期'])"
                v-has="{authId: 'daycare.contractTeach.exam'}"
                type="primary"
                @click="trigger({name : 'examineForm', arguments : {id : scoped.row.tcContractId}})"
              >
                审核
              </etm-text>
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['退费', '作废', '正常', '过期'])"
                v-has="{authId: 'daycare.contractTeach.remove'}"
                type="danger"
                @click="remover(scoped.row.tcContractId)"
              >
                删除
              </etm-text>
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['待审核', '审核未通过', '退费', '作废', '过期'])"
                v-has="{authId: 'daycare.contractTeach.courseTransfer'}"
                type="primary"
                @click="trigger({name : 'transferClass', arguments : {id : scoped.row.tcContractId, name : scoped.row.childName, no : scoped.row.contractNo, childId : scoped.row.childId}})"
              >
                转课
              </etm-text>
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['待审核', '审核未通过', '退费', '作废', '过期'])"
                v-has="{authId: 'daycare.contractTeach.refund'}"
                type="primary"
                @click="trigger({name : 'refundForm', arguments : {id : scoped.row.tcContractId}})"
              >
                退费
              </etm-text>
              ...
              <etm-text
                v-if="exclude(scoped.row.contractStatusEnum, ['待审核', '审核未通过', '退费', '作废', '过期'])"
                v-has="{authId: 'daycare.contractTeach.cancel'}"
                type="primary"
                @click="openScrap(scoped.row.tcContractId)"
              >
                作废
              </etm-text>
            </template>
...
import { queryTeachContractList, removeTeachContract, updateScrap } from '@/api/newDaycare/contract'
  ...
  methods: {
    ...
    confirmScrap () {
      this.$refs.scrap.$children[0].validate(err => {
        if (err) {
          // this.scrapForm.cacContractId = this.scrapForm.teContractId
          updateScrap({ ...this.scrapForm, type: 'TEACH' }).then(res => {
    ...
    init (search = {}) {
      ...
      queryTeachContractList(result).then(res => {
        ...
        result.content.forEach((cur, index) => {
          ...
          result.content[index].remainingDays = (cur.remainingDays || 0) + '天'
    ...
    remover (id) {
      this.$etmTip.init({
        ...
        cancelBtn: '取消',
        confirm: () => {
          removeTeachContract(id).then(res => {
            this.init(this.search)
            this.$etmTip.close()
          })
        },

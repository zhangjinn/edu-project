        ...
        >
          ...
          >
            <template slot-scope="scoped">
              <el-button
                type="text"
                @click="changeEarnes(scoped.row.earnestMoneyId)"
              >
                {{
                  scoped.row.orderNo
                }}
          ...
          >
            <template slot-scope="scoped">
              ...
              <!-- TODO: 隐藏收款功能 -->
              <!--              <etm-text-->
              <!--                v-if="!scoped.row.pay && scoped.row.status === '未使用'"-->
              <!--                type="primary"-->
              ...
              <etm-text
                v-if="scoped.row.status === '未使用'"
                v-has="{authId: 'daycare.earnestMoney.remove'}"
                type="danger"
                @click="deleteEarnes(scoped.row.earnestMoneyId)"
              >
                删除
              </etm-text>
            </template>
    ...
    init (search = {}) {
      ...
      queryDepositList(params).then(res => {
        ...
        result.forEach((cur, index) => {
          result[index].status = (cur.status === 'NOT_USED' || !cur.status) ? '未使用' : '已使用'
          result[index].paymentType = this.payType[cur.paymentType]
          result[index].createTime = cur.createTime ? parseTime(cur.createTime, '{y}-{m}-{d}') : ''
          result[index].dateCharge = cur.dateCharge ? parseTime(cur.dateCharge, '{y}-{m}-{d}') : ''
        })

    ...
    <etm-layout-split>
      <div slot="header">
        <etm-tabs
          v-model="activeName"
          setting
          @setting="$router.push({name: 'card_setting'})"
        >
          <el-tab-pane
            name="order"
            label="会员卡管理"
        ...
        >
          ...
          >
            <template slot-scope="scoped">
              <el-button
                type="text"
                @click="changeEarnes(scoped.row)"
              >
                {{ scoped.row.cardNumber }}
              </el-button>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scoped">
              ...
              <etm-text
                type="primary"
                @click="triggerMember(scoped.row.membershipCardId, scoped.row.status === '使用中')"
              >
                {{ scoped.row.status === '使用中' ? '禁用' : '启用' }}
              </etm-text>
              <etm-text
                type="danger"
                @click="deleteEarnes(scoped.row.membershipCardId)"
              >
                删除
              </etm-text>
            </template>
          ...
          <template v-else>
            <div>
              <etm-field-label
                ...
                type="left"
              >
                {{ earnestDetail.cardNumber }}
              </etm-field-label>
              <etm-field-label
                label="卡类型"
                type="left"
              >
                {{ earnestDetail.cardTypeName }}
              </etm-field-label>
              <etm-field-label
                label="学员姓名"
                type="left"
              ...
              <etm-field-label
                ...
                type="left"
              >
                {{ earnestDetail.personRelationName }}
              </etm-field-label>
              <etm-field-label
                label="联系人电话"
                type="left"
...
export default {
  ...
  methods: {
    ...
    triggerMember (id, status) {
      updateTriggerMember({ membershipCardId: id, status: !status }).then(() => {
    ...
    deleteEarnes (id) {
      this.$etmTip.init({
        ...
        cancelBtn: '取消',
        confirm: () => {
          removeMemberCard({ membershipCardId: id }).then(() => {
            this.init(this.searchKey)
          })
          this.$etmTip.close()
        },

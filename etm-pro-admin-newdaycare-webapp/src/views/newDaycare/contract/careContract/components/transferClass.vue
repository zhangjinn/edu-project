    ...
    >
      ...
      <div class="itemTable">
        ...
        <div class="buttonBox">
          <el-button
            type="dashed"
            @click="selectEvent(form.type === 'CARE' ? 'nurseType' : 'earlyType')"
          >
            <i class="iconfont iconplus" />选择课程类型
          </el-button>
          <el-button
            type="dashed"
            @click="selectEvent(form.type === 'CARE' ? 'nurseCourse' : 'earlyCourse')"
          >
        ...
        <nesting-table v-model="templateData2" />
        <div
          v-if="templateData2.length"
          class="total"
        >
          转入总计：
          <el-button type="text">
            ￥{{ form.totalTransferIn }}
          </el-button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    classOutChange (event) {
      queryInClassItem(event).then((res) => {
    ...
    init (prop) {
      ...
      queryTransClass(prop.id).then((res) => {
    ...
    popTransferFirm () {
      Promise.all([
        ...
        createTransformOut(this.form).then((res) => {
          this.isShow = false
          this.reset()
          this.$emit('update', {})
          this.$message({

  ...
  <div class="chargeDetailEdit-wrap">
    ...
    <div class="itemTable">
      ...
      >
        <el-button
          type="primary"
          @click="selectEvent('nurseSetMeal')"
        >
          选择套餐
        </el-button>
...
import {
  ...
  updateContract,
  // queryIsRenewal,
  queryMoneyLists, checkCareRepeatTime
  ...
  methods: {
    ...
    checkRepeat () {
      return new Promise((resolve, reject) => {
        // if (!['UNDER_REVIEW', 'NORMAL', 'AUDIT_FAILED'].includes(this.form.contractStatusEnum)) {
        //   resolve()
        //   return
        // }
        const api = checkCareRepeatTime
        ...
        if (new Date(endDate).getTime() < (new Date().getTime() - 1000)) {
          resolve()
          return
        }
        ...
        api({
          ...
          if (res.data) {
            this.$etmTip.init({
              type: 'warn',
              title: '提示',
              content: '该学员已存在当前有效期期间的托育合同，保存后会对托育剩余天数统计造成影响，是否确定保存合同？',
              confirmBtn: '确定',
              cancelBtn: '取消',
              confirm: () => {
                resolve()
    ...
    save () {
      ...
      return new Promise((resolve, reject) => {
        ...
        Promise.all(form).then(resolve2 => {

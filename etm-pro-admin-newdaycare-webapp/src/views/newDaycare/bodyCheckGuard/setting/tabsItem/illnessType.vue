...
import {
  queryOddTypeList,
  createOddTypeListItem,
  updateOddTypeListItem,
  removeOddTypeListItem,
  updateOddTypeListSort
...
export default {
  ...
  methods: {
    ...
    handleChange () {
      ...
      updateOddTypeListSort({ ids }).then(res => {
    ...
    deleteItem (e) {
      ...
      removeOddTypeListItem({ checkProjectId: this.dragData[e[0].index].id }).then(res => {
    ...
    updateNew (index) {
      ...
      if (target.id) {
        // 存在id，修改
        p = updateOddTypeListItem({
          checkProjectId: target.id,
          checkProjectName: target.text
        })
      } else {
        // 不存在id，新增
        p = createOddTypeListItem({ checkProjectName: target.text })
      }
      p.then(res => {

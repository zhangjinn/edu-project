      ...
      <etm-layout-split>
          ...
          >
            ...
            >
              <template v-slot="scope">
                <etm-status :type="scope.row.postStatus==='已发布' ? 'active' : 'inactive'">
            ...
            >
              <template slot-scope="scope">
                ...
                <template v-else>
                  ...
                  <etm-text
                    type="primary"
                    @click="addNotification(scope.row.id)"
                  >
                    编辑
                  </etm-text>
                  <etm-text
                    type="primary"
                    @click="publicNotification(scope.row.id)"
                  >
                    发布
                  </etm-text>
                  <etm-text
        ...
        >
          <etm-layout-split>
            ...
            >
              <el-tab-pane
                :label="`未读(${unReadNumber}人)`"
                name="false"
              >
                <etm-table
                  v-if="unReadCountTableData.length !== 0"
                  :columns="readCountColumns"
                  :data="unReadCountTableData"
                />
              </el-tab-pane>
              <el-tab-pane
                :label="`已读(${readNumber}人)`"
                name="true"
              >
                <etm-table
                  v-if="readCountTableData.length !== 0"
                  :columns="readCountColumns"
                  :data="readCountTableData"
                />
              </el-tab-pane>
            </etm-tabs>
          </etm-layout-split>
            ...
            <div class="mockup">
              <div class="box-wrap">
                ...
                <div
                  ...
                  v-html="previewData.notificationContent"
                />
              </div>
            </div>
          </etm-layout-split>
...
export default {
  ...
  methods: {
    cellClassNameMethod ({ row, columnIndex }) {
      if (columnIndex === 4) { // 状态栏
        if (row.postStatus === '已发布') {
          return 'enabled'
        }
        if (row.postStatus === '未发布') {
          return 'disabled'
        }
      }
    },

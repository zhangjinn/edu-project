...
<script>
import {
  queryFollowTemplateList,
  createFollowTemplateItem,
  updateFollowTemplateItem,
  queryFollowTemplateItem,
  removeFollowTemplateItem
...
export default {
  ...
  methods: {
    refreshTableData () {
      ...
      queryFollowTemplateList(options).then(res => {
        const { data } = res
        this.tableData = data
      }).catch(err => {
        console.log(err)
    ...
    editTemplate (row) {
      this.popTitle = '编辑模板'
      this.currentRow = row
      queryFollowTemplateItem({ id: row.id }).then(res => {
    ...
    handleTipConfirm () { // 执行删除模板操作
      removeFollowTemplateItem({ id: this.currentRow.id }).then(res => {
        this.$etmTip.close()

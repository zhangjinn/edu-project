...
<script>
import {
  queryCustomerFieldList,
  updateCustomerFieldItem,
  createCustomerFieldItem,
  queryCustomerFieldItem
...
export default {
  ...
  methods: {
    refreshTableData () {
      queryCustomerFieldList().then(res => {
        const { data } = res
        this.cacheNames = []
        data.forEach(v => {
    ...
    handleEdit (row) {
      ...
      this.editId = row.id
      queryCustomerFieldItem({ id: row.id }).then(res => {
        const { data } = res
        this.formData = {
          name: data.name || '',
    ...
    handleChange (row) {
      updateCustomerFieldItem(row).then(res => {

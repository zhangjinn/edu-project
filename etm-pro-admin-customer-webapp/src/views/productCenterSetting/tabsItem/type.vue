...
<script>
import itemTemplate from '@/views/templates/itemTemplate'
import {
...
export default {
  ...
  methods: {
    ...
    deleteItem (e) {
      ...
      removeProductType({
        productTypeId: this.dragData[e[0].index].id
      }).then(res => {
    ...
    updateNew (index) {
      ...
      if (target.id) {
        ...
        updateProductType(data).then(res => {
          const result = res.data
          target.id = result.productTypeId
        }).catch(err => {
          console.error(err)
        })
      } else {

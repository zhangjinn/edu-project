  ...
  <div class="nestingTable-wrap">
    ...
    >
      ...
      >
            ...
            <template v-if="k === 3">
              ...
              <div class="input">
                ...
                >
                  ...
                  <!--                  />-->
                  <!--                  v-else-->
                  <p :style="{width: status === 'transfer' ? '100px' : 'auto'}">
        ...
        <div
          v-for="(i, k) in item[map['childList']]"
          :key="k"
          class="row child-row"
        >
          <div
            v-for="(ii, kk) in columns"
            :key="kk"
            class="col"
            :style="{width : ii.width || '30%'}"
          >
            <template v-if="kk===0">
              <!--              <div v-if="showImg" class="itemImg">-->
              <!--                <img src="../../../../assets/images/etm-logo.png">-->
            ...
            <template v-if="kk===3">
              ...
              <div class="input">
                ...
                >
                  ...
                  <p
                    v-else
                    :style="{width: status === 'transfer' ? '100px' : 'auto'}"
                  >
...
export default {
  ...
  data () {
    return {
      placeImg: require('@/assets/images/etm-logo.png'),
      cacheData: [] // 最开始的数据
    }
  },
  computed: {},
  ...
  methods: {
    change (parentId, childId) {
      if (childId === false) {
        this.data[parentId][this.map.receivable] = (this.data[parentId][this.map.receivableNotes] / this.data[parentId][this.map.purchase]).toFixed(2)
        this.data[parentId][this.map.netReceipts] = (this.data[parentId][this.map.netReceiptsNotes] / this.data[parentId][this.map.purchase]).toFixed(2)
      } else {
        this.data[parentId][this.map.childList][childId][this.map.receivable] = (this.data[parentId][this.map.childList][childId][this.map.receivableNotes] / this.data[parentId][this.map.childList][childId][this.map.purchase]).toFixed(2)
        this.data[parentId][this.map.childList][childId][this.map.netReceipts] = (this.data[parentId][this.map.childList][childId][this.map.netReceiptsNotes] / this.data[parentId][this.map.childList][childId][this.map.purchase]).toFixed(2)

...
function downloadPDF1(ele, pdfName) {
  // 导出之前先将滚动条置顶,不然会出现数据不全的现象
  window.pageYOffset = 0
  document.documentElement.scrollTop = 0
  document.body.scrollTop = 0

  const clonedNode = ele.cloneNode(true) // 克隆节点
  clonedNode.setAttribute('class', 'renderPdf')
  document.body.appendChild(clonedNode) // 在body插入克隆的节点
  // ele.parentNode.appendChild(clonedNode) // 在父节点插入克隆的节点

  clonedNode.style.width = ele.offsetWidth
  clonedNode.style.height = ele.offsetHeight
  ...
  html2canvas(clonedNode, {
    ...
    const child = document.getElementsByClassName('renderPdf')[0]
    console.log(child)

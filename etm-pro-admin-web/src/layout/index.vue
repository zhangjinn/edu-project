<template>
  <div :class="classObj" class="app-wrapper">
    ...
    <div class="main-container">
      ...
      <div class="header">
        <navbar :un-read-count="unReadCount" @isRead="getMsgNum" />
        <tags-view />
      </div>
      <app-main v-show="!isMicro" />
      <div v-show="isMicro" id="appContainer" />
      <!--      <app-main />-->
      <!--      <div id="appContainer" />-->
    </div>
    <!--      新消息提示弹窗-->
    <div v-if="showNotification" class="notificationBox">
      <div class="title">
        消息通知
        <span class="more">{{ number > 0 ? '(还有' + number + '条消息未读)' : '' }}</span>
      </div>
      <div class="mainMsg">{{ messageTxt }}</div>
      <div class="btn">
        <el-button class="btnDefault" @click="closeNot">忽略</el-button>
        <el-button type="primary" @click="readMsg">知道了</el-button>
      </div>
    </div>
    <etm-pop
      :visible="pop"
    ...
    >
      <etm-form ref="form" :model="form" :rules="rules" type="dialog">
        <el-form-item label="用户账号">
          <el-input v-model="accountNumber" disabled />
        </el-form-item>
        <el-form-item label="用户姓名">
          <el-input v-model="accountName" disabled />
        </el-form-item>
        <el-form-item label="设置密码" prop="password">
          <el-input v-model.trim="form.password" placeholder="请输入密码" show-password />
        </el-form-item>
        <el-form-item label="再次输入" prop="passwordAgain">
          <el-input v-model.trim="form.passwordAgain" placeholder="请再次输入密码" show-password />
        </el-form-item>
      </etm-form>
    </etm-pop>
  </div>
...
export default {
  ...
  methods: {
    changeRouter() {
      ...
      const micros = ['/opc', '/daycare', '/customer', '/standardcare', '/newdaycare', '/evaluation', '/guardcare']
...
.hideSidebar.mobile .header, .openSidebar.mobile .header {
  width: 100%;
}

  ...
  <div class="orderTable-wrap">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId: 'guardcare.order.remindBatch'}" plain @click="urgePayBatch">批量催缴
          </el-button>
          <el-button v-has="{authId: 'guardcare.orderBunch.createBatch'}" plain @click="chargeConfirmBatch">
            批量收费
          </el-button>
          <el-button v-has="{authId: 'guardcare.order.excelExport'}" plain @click="exportOrder">导出Excel
          </el-button>
        </div>
      </etm-table-tool-bar>
      ...
      <el-table-column slot="right" fixed="right" label="操作" prop="operate" width="200">
        <template slot-scope="scoped">
          <etm-text
            ...
            v-has="{authId: 'guardcare.order.remind'}"
            type="primary"
          ...
          <etm-text
            v-if="scoped.row.status === 'UN_PAY'"
            v-has="{authId: 'guardcare.order.confirm'}"
            type="primary"
...
export default {
  ...
  methods: {
    ...
    judgeColumnType(type) { // 判断行是否合并
      if (type === 'code' || type === 'childName' || type === 'clazzName' || type === 'totalAmount' || type === 'actualAmount' || type === 'name' ||
        type === 'payType' || type === 'paymentType' || type === 'operateBy' || type === 'paymentDate' || type === 'createTime' || type === 'createBy' ||
        type === 'confirmEmployee' || type === 'status' || type === 'operate' || type === 'depositAmount') {
        return true
      } else {
        return false
      }
    ...
    exportOrder() {
      ...
      exportOrderExcel(param).then(res => {

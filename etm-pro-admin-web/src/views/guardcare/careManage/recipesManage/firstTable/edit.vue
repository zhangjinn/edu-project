    ...
    <etm-main-body>
      <etm-layout-split>
        <template slot="left">
          <div class="recipe-collapse">
            <el-input v-model="dishesName" placeholder="请输入搜索内容" @keyup.enter.native="handleSearch">
              <span slot="suffix" class="el-input__icon el-icon-search" />
            </el-input>
            <el-collapse v-if="recipeLists.length>0" v-model="activeNames">
              <div v-for="(recipeList, index) in recipeLists" :key="index">
                <el-collapse-item
          ...
          >
            ...
            <template slot="toolRight">
              <div class="showIngredients">
                <el-checkbox v-model="checked" @change="changeCheckbox">显示食材用量</el-checkbox>
              </div>
              <el-button slot="right" type="primary" @click="popVisibleRecommend = true">选用推荐食谱</el-button>
              <el-button slot="right" type="primary" @click="handleCopy">复制周食谱</el-button>
              <el-button slot="right" plain class="exportBtn" @click="exportTable">导出Excel</el-button>
            </template>
      ...
      >
        <div class="popRecommendContent">
          <recommendedRecipe v-if="popVisibleRecommend" :week-recipe-id="weekRecipeId" @handleRecipeChange="handleRecipeChange" />
        </div>
      </etm-pop>
    </etm-main-body>
  </div>
...
export default {
  ...
  watch: {
    $route: {
      handler: function(val, oldVal) {
        if (val.name === 'guardcare_recipeEdit') {
          this.entrance()
        }
      },
      // 深度观察监听
  ...
  methods: {
    ...
    getFoodClassificationData(type) {
      ...
      getFoodClassification(param).then((res) => {
        const { data } = res
    ...
    exportTable() {
      this.exportTableToExcel({
        ref: this.$refs['xlsxTable'].$refs['weeklyCalendarTable'].$children[0],
        name: `${this.gradeName} ${this.weekName}食谱`,

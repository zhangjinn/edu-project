    ...
    <etm-layout-split>
      <div slot="header">
        <etm-tabs
          ref="tabs"
          v-model="tabs"
          :setting="hasAuthOfBool('guardcare.bodyCheck.setting')"
          @setting="toSetting"
        >
          <template v-for="tab in tabsList">
            <el-tab-pane
              ...
              :name="tab.name"
            >
              <etm-tool-bar v-model="tab.filters" @reset="resetQuery" @search="confirmQuery" />
            </el-tab-pane>
          </template>
        </etm-tabs>
      </div>
      <div slot="body">
        <!-- TODO: 按钮v-has权限渲染不出，可以使用v-if="hasAuthOfBool()方法" -->
        <etm-table-tool-bar slot="tool">
          <template v-if="tabs === 'record'">
            <div slot="right">
              <el-button
                v-if="hasAuthOfBool('guardcare.bodyCheck.excelExport')"
                plain
          ...
          <template v-if="tabs === 'detail'">
            <div slot="right">
              <el-button
                v-if="hasAuthOfBool('guardcare.unhealthy.excelExport')"
                plain
                @click.stop="exportToExcel"
              >导出Excel
              </el-button>
          ...
          >
            <template slot-scope="scoped">
              <etm-text
                ...
                v-has="{authId: 'guardcare.unhealthy.confirm'}"
                type="primary"
        ...
        >
          <etm-form ref="form" :model="formData" :rules="rules" type="dialog">
            <el-form-item label="体温" prop="temperature">
              <el-input v-model="formData.temperature" onkeyup="this.value = this.value.replace(/[^\d.]/g,'')" />
            </el-form-item>
            <el-form-item label="常见病状" prop="illness">
              <etm-screen-tab v-model="formData.illness" :options="illnessList" multiple />
            </el-form-item>
          </etm-form>
        </etm-pop>
      </div>
...
import {
  ...
  queryCheckTypeList,
  queryClazzList
} from '@/api/guardcare/careManage/healthCheck'
...
export default {
      ...
      tabsList: [
        {
          label: '检查记录',
          name: 'record',
          authId: 'guardcare.bodyCheck.queryBatch',
          filters: [
            {
              type: 'date',
              label: '检查日期',
        ...
        },
        {
          label: '异常明细',
          name: 'detail',
          authId: 'guardcare.unhealthy.queryBatch',
          filters: [
            {
              type: 'date',
              label: '检查日期',
  ...
  methods: {
    toSetting() {
      this.$router.push('/guardcareSettings/healthCheckSetting')
    },
    confirmQuery() {
      const filters = {
    ...
    abnConfirm(s) {
      this.activeRow = s.row
      queryCheckRecord(s.row.checkRecordId).then(res => {
        const { data } = res
        this.cacheData = data
        this.formData = {

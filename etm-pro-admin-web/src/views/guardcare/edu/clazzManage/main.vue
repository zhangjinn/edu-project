      ...
      <template slot="body">
        ...
        >
          <etm-table-tool-bar slot="tool">
            <div slot="right">
              <el-button v-has="{authId: 'guardcare.clazz.create'}" type="primary" @click="addGrade"><i class="el-icon-plus" />&nbsp;&nbsp;添加班级</el-button>
            </div>
          </etm-table-tool-bar>
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.clazzId = row.id
          delClazz(this.clazzId).then((res) => {
            this.$etmTip.close()
    ...
    handleSave() {
      this.$refs['childDrawerForm'].$refs['form'].$children[0].validate(valid => {
        if (valid) {
          ...
          putClazz(param, this.clazzId).then((res) => {
            // this.drawerVisible = false
            this.drawerType = 'detail'
            this.$message({
              message: '已修改',

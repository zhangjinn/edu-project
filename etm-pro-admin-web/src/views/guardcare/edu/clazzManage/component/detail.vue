  ...
  <div class="detail-wrap">
    <etm-layout-split :left-styles="{minWidth: '500px',maxWidth: '500px'}">
      ...
      <div slot="right" class="rightBox">
        <etm-title :show-line="false" border>幼儿详情
          <template slot="tool">
            <el-button v-if="drawerType === 'detail'" plain size="mini" @click="editChild">编辑</el-button>
            <el-button v-if="drawerType === 'edit'" plain size="mini" @click="cancel">取消</el-button>
            <el-button v-if="drawerType === 'edit'" type="primary" size="mini" @click="saveChild">保存</el-button>
          </template>
        </etm-title>
        <div v-if="drawerType === 'detail' || drawerType === 'leaveDetail'" class="detailMessage">
          <div class="avatar">
            <div class="img">
    ...
    saveChild() {
      this.$refs.addForm.$children[1].$children[0].validate(valid => { // 表单验证
        if (valid) {
          ...
          renewChild(this.childInfo.childId, param).then(() => {

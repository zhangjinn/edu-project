  ...
  <div>
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId: 'guardcare.courseClazz.create'}" type="primary" @click="handleAdd"><i class="el-icon-plus" />&nbsp;&nbsp;添加课程</el-button>
        </div>
      </etm-table-tool-bar>
      <el-table-column slot="left" label="课程名称">
        <template v-slot="scoped">
          <etm-text v-if="!hasAuthOfBool('guardcare.courseClazz.query')" type="normal">{{ scoped.row.name }}</etm-text>
      ...
      <el-table-column slot="right" label="操作">
        <template v-slot="scoped">
          <etm-text v-has="{authId: 'guardcare.courseClazz.update'}" type="primary" @click="handleEdit(scoped.row, true)">编辑</etm-text>
          <etm-text v-has="{authId: 'guardcare.courseClazz.remove'}" type="danger" @click="handleDelete(scoped.row)">删除</etm-text>
        </template>
      </el-table-column>
    </etm-table-page>
    ...
    >
      <div class="popContent">
        <edit v-if="popVisibleEdit" ref="childPopForm" />
      </div>
    </etm-pop>
      ...
      <etm-layout-split line>
        <etm-title :title="drawerTitle" border size="big">
          <template v-if="drawerVisible && drawerType==='edit'" slot="tool">
            <el-button v-has="{authId: 'guardcare.courseClazz.query'}" plain size="mini" @click="cancelDrawer">取消</el-button>
          ...
          <template v-if="drawerVisible && drawerType==='detail'" slot="tool">
            <el-button v-has="{authId: 'guardcare.courseClazz.update'}" plain size="mini" @click="handleEdit">编辑</el-button>
          </template>
        </etm-title>
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.courseId = row.id
          delCourseClazz(this.courseId).then((res) => {
            this.$etmTip.close()
      ...
      this.$refs['childPopForm'].$refs['form'].$children[0].validate(valid => {
        if (valid) {
          ...
          if (this.drawerType === 'add') {
            ...
            putCourseClazz(param, this.courseId).then((res) => {
              // this.drawerVisible = false
              this.drawerType = 'detail'
              this.$message({
                message: '已修改',

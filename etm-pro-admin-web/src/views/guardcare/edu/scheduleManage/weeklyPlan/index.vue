  ...
  <div>
      ...
      <template v-slot:cell="data">
        <div v-if="data.cellList">
          <div v-for="(item,index) in data.cellList" :key="index">
            <EtmCell :data="item" @handleDelete="handleDelete(item)" @handleEdit="handleEdit(item)" />
          </div>
    ...
    >
      <div class="popContent">
        <edit v-if="popVisibleEdit && drawerType==='add'" ref="childPopForm" :type="drawerType" :cell-data="cellData" />
      </div>
    </etm-pop>
      ...
      <etm-layout-split line>
        <etm-title :title="drawerTitle" border size="big">
          <template v-if="drawerVisible && drawerType==='edit'" slot="tool">
            <el-button size="mini" type="primary" @click="popConfirmEdit">保存</el-button>
          </template>
        ...
        <div class="drawerBody">
          <edit v-if="drawerVisible && drawerType==='edit' && scheduleId" ref="childPopForm" :type="drawerType" :cell-data="cellData" :item-id="scheduleId" />
        </div>
      </etm-layout-split>
    </etm-drawer>
  </div>
...
import EtmCellAddBtn from '@/views/guardcare/edu/scheduleManage/components/EtmWeeklyCalendar/components/EtmCellAddBtn'
  ...
  methods: {
    ...
    handleDelete(item) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.scheduleId = item.courseTermWeekScheduleId
          delScheduleWeek(this.scheduleId).then((res) => {
            this.$etmTip.close()
      ...
      this.$refs['childPopForm'].$refs['form'].$children[0].validate(valid => {
        if (valid) {
          ...
          if (this.drawerType === 'add') {
            ...
            putScheduleWeek(param, this.scheduleId).then((res) => {

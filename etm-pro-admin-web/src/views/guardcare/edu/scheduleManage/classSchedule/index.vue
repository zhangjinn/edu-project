      ...
      <template v-slot:cell="data">
        <div v-if="data.cellList">
          <div v-for="(item,index) in data.cellList" :key="index">
            <EtmCell :data="item" @handleDelete="handleDelete(item)" @handleEdit="handleEdit(item, data.colProp)" />
          </div>
        </div>
        <EtmCellAddBtn v-if="!data.cellList || data.cellList.length<1" v-has="{authId: 'guardcare.scheduleCourseClazz.update'}" @click.native.stop="handleAdd(data.labelId, data.colProp)" />
      </template>
    </EtmWeeklyCalendar>
  ...
  methods: {
    ...
    handleDelete(item) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.scheduleId = item.id
          delScheduleClazz(this.scheduleId).then((res) => {
            this.$etmTip.close()
      ...
      this.$refs['childPopForm'].$refs['form'].$children[0].validate(valid => {
        if (valid) {
          ...
          if (this.drawerType === 'add') {
            ...
            putScheduleClazz(param, this.scheduleId).then((res) => {

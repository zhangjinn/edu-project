<template>
  <div class="firstTable">
    <EtmWeeklyCalendar :table-data="tableData" :add="hasAuthOfBool('guardcare.scheduleCourseExtra.create')" :week-tag="false" @handleAdd="handleAdd" @handleUpDate="upDate">
      ...
      <template v-slot:cell="data">
        <div v-if="data.cellList">
          <div v-for="(item,index) in data.cellList" :key="index">
            <EtmCell :data="item" @handleDelete="handleDelete(item)" @handleDetail="handleDetail(item)" />
          </div>
        </div>
      </template>
    </EtmWeeklyCalendar>
      ...
      <etm-layout-split line>
        <etm-title :title="drawerTitle" border size="big">
          <template v-if="drawerVisible && drawerType==='edit'" slot="tool">
            <el-button size="mini" plain @click="handleDetail">取消</el-button>
        ...
        <div class="drawerBody">
          <detail v-if="drawerVisible && drawerType==='detail' && scheduleId" :item-id="scheduleId" />
          <edit
  ...
  methods: {
    ...
    handleDelete(item) {
      this.$etmTip.init({
        ...
        confirm: () => {
          this.scheduleId = item.specialCourseScheduleBatchId
          delSchedule(this.scheduleId).then((res) => {
            this.$etmTip.close()
      ...
      this.$refs['childPopForm'].$refs['form'].$children[0].validate(valid => {
        if (valid) {
          ...
          if (this.drawerType === 'add') {
            ...
            putSchedule(param, this.scheduleId).then((res) => {

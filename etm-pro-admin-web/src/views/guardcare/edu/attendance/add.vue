<template>
  <div class="add">
    <etm-form ref="form" type="dialog" :model="formData" :rules="rules">
      <el-form-item label="请假幼儿" prop="childTargets">
        <div class="childSelect">
          <template v-if="type === 'add'">
            ...
            <el-button v-show="type === 'add'" type="dashed" @click.stop="selectChild"><i class="el-icon-plus" /> 选择幼儿</el-button>
          </template>
      ...
      <el-form-item label="请假类型" prop="leaveType">
        <el-radio v-model="formData.leaveType" label="SICK_LEAVE" @change="formData.disease = []">病假</el-radio>
        <el-radio v-model="formData.leaveType" label="LEAVE" @change="formData.disease = [1]">事假</el-radio>
      </el-form-item>
      <el-form-item v-show="formData.leaveType === 'SICK_LEAVE'" label="病状" prop="disease">
        <etm-screen-tab v-model="formData.disease" :options="list" multiple />
      </el-form-item>
      ...
      <el-form-item label="上传图片" prop="previewSrcLists">
        <etm-upload-img
          :resource-name="'guardcare/edu/attendance'"
      ...
      <el-form-item label="通知家长" prop="notify">
        <el-radio v-model="formData.notify" label="yes">是</el-radio>
        <el-radio v-model="formData.notify" label="no">否</el-radio>
      </el-form-item>
    </etm-form>
  </div>
</template>
...
import { queryLeaveLast, queryLeaveItemDetail } from '@/api/guardcare/edu/attendance'
import { getClazzList, getGradeList } from '@/api/guardcare/childManage/childManage'
import transferDate from '@/utils/date'
import EtmClazzCascader from '@/components/EtmClazzCascader/index.vue'
export default {
  components: {
    etmUploadImg,
    EtmClazzCascader
  ...
  methods: {
    ...
    getClazz(id) { // 根据点击的年级请求班级列表，并塞入到二级菜单里
      const param = {
        gradeId: id[0],
        paged: false
      }
      ...
      getClazzList(param).then(res => {
        const data = res.data
        this.clazzData = res.data
        this.clazzList[i].children = []
        data.forEach(item => {
          const option = {
            label: item.name,
            value: item.id
          }
          this.clazzList[i].children.push(option)
        })
      })
    },
    search() { // 弹窗里搜索

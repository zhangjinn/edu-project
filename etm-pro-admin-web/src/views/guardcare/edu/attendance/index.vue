    ...
    <etm-layout-split>
      <div slot="header">
        <etm-tabs
          ...
          :setting="activeName === 'tab2' && hasAuthOfBool('guardcare.attendance.setting')"
          @setting="toSetting"
        >
          <el-tab-pane label="快捷考勤" name="tab1" />
          <el-tab-pane
            v-if="hasAuth('guardcare.askForLeave.queryBatch', 'tabs', 'activeName')"
            label="幼儿请假"
            name="tab2"
          >
            <etm-tool-bar
        ...
        <div v-else-if="activeName === 'tab2'">
          ...
          >
            <etm-table-tool-bar slot="tool">
              <div slot="right">
                <el-button
                  v-has="{authId: 'guardcare.askForLeave.create'}"
                  type="primary"
            ...
            >
              <template slot-scope="scoped">
                <etm-text
                  ...
                  v-has="{authId: 'guardcare.askForLeave.update'}"
                  type="primary"
                ...
                <etm-text
                  v-if="!scoped.row.isFromParents"
                  v-has="{authId: 'guardcare.askForLeave.remove'}"
                  type="danger"
                ...
                <etm-text
                  ...
                  v-has="{authId: 'guardcare.askForLeave.confirm'}"
                  type="primary"
...
import {
  ...
  modifyLeaveItem,
  approveLeaveItem
} from '@/api/guardcare/edu/attendance'
...
export default {
  ...
  activated() {
    if (this.$store.state.guardcareDashboard.pageEntry === 'childLeave') {
      this.$nextTick(() => {
  ...
  methods: {
    toSetting() { // 跳转到设置页
      this.$router.push('/guardcareSettings/attendanceSetting')
    },
    getDiseaseList() {
      this.diseaseList = []
    ...
    search() { // 确认筛选查询
      if (this.filters.grade && !this.filters.clazz) return this.$message.warning('请选择班级')

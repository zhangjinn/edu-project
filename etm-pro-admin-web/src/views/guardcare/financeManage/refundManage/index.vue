<template>
  <div class="index-wrap">
        ...
        <div slot="header">
          <etm-tabs v-model="activeName">
            <el-tab-pane label="退费管理" name="refund">
              <etm-tool-bar v-model="searchData" @search="handleSearch" @reset="reset">
        ...
        <div slot="body">
          ...
          >
            <etm-table-tool-bar slot="tool">
              <el-alert
                title="温馨提示：当月的退费账单在下个月1号生成。"
                type="warning"
                :closable="false"
              />
            </etm-table-tool-bar>
            <el-table-column slot="left" fixed="left" type="index" label="序号" width="60">
              <template slot-scope="scoped">
                <span>{{ getRowIndex(scoped.$index) }}</span>
              </template>
            </el-table-column>
            <el-table-column slot="left" fixed="left" label="所属年级" width="120" prop="gradeName" />
            <el-table-column slot="left" fixed="left" label="班级" width="120" prop="clazzName" />
            <el-table-column slot="left" fixed="left" label="幼儿姓名" width="120" prop="childName" />
            <el-table-column slot="right" label="退费项目" min-width="120" prop="originName" />
            <el-table-column slot="right" label="已收/剩余金额（元）" min-width="180" prop="remainingAmount" align="right" />
            ...
            <el-table-column slot="right" label="退费日期" width="150">
              <template slot-scope="scoped">
                <span>{{ transferDate(scoped.row.refundTime) }}</span>
              </template>
            </el-table-column>
            <el-table-column slot="right" label="审核日期" width="150">
              <template slot-scope="scoped">
            ...
            <el-table-column slot="right" fixed="right" label="操作" width="120">
              <template slot-scope="scoped">
                <etm-text
                  ...
                  v-has="{authId: 'guardcare.refund.create'}"
                  type="primary"
                ...
                <etm-text
                  ...
                  v-has="{authId: 'guardcare.refund.exam'}"
                  type="primary"
    ...
    >
      <etm-layout-split>
        <etm-title :title="drawerTitle" :show-line="false" border size="big">
          <template slot="tool">
            <el-button v-if="rowData.status === 'IN_PROGRESS'" plain size="mini" @click="submitAudit(false)">不通过
            </el-button>
            <el-button v-if="rowData.status === 'IN_PROGRESS'" type="primary" size="mini" @click="submitAudit(true)">
              通过
            </el-button>
            <el-button
        ...
        <div v-if="rowData.status === 'FAILED'" class="warnText">
          <el-alert title="请重新填写退费金额" type="warning" :closable="false" />
        </div>
        <etm-form type="dialog">
          <etm-field-label type="left" label="幼儿姓名">{{ rowData.childName }}</etm-field-label>
          <etm-field-label type="left" label="年级">{{ rowData.gradeName }}</etm-field-label>
          <etm-field-label type="left" label="班级">{{ rowData.clazzName }}</etm-field-label>
          <etm-field-label type="left" label="退费项目">{{ refundData.originName }}</etm-field-label>
          <etm-field-label type="left" label="已收金额（元）">{{ refundData.remainingAmount }}</etm-field-label>
          <template v-if="rowData.status === 'IN_PROGRESS'">
            <etm-field-label type="left" label="退费金额（元）">{{ refundData.refundAmount }}</etm-field-label>
            <etm-field-label type="left" label="支付方式">{{ transferType(refundData.paymentType) }}</etm-field-label>
          </template>
          <template v-else>
            <etm-field-label type="left" label="退费金额（元）" class="inputLabel">
              <el-input-number v-model="refundAmount" :min="0" placeholder="请输入退费金额" />
            </etm-field-label>
            <etm-field-label type="left" label="支付方式" class="inputLabel">
...
import {
  ...
  getRefund,
  getRefundList
} from '@/api/guardcare/financeManage/refundManage'
...
export default {
  ...
  methods: {
      ...
      getRefundList(param).then(res => {
        ...
        if (res.data.content) {
          res.data.content.forEach(item => { // 拆分数据
            if (item.refundList.length) {
              this.rowNum.push(item.refundList.length) // 记录每项数据有几个子项，在进行表格合并时用到
              item.refundList.forEach(list => { // 将每项数据里的子项抽出并形成新的一项数据
    ...
    judgeColumnType(type) { // 判断行是否合并
      if (type === 'gradeName' || type === 'clazzName' || type === 'childName' || type === 'shouldArriveDays' || type === 'arrivedDays' ||
        type === 'absenceDays' || type === 'requestForLeaveDays') {
        return true
      } else {
        return false
      }
    ...
    refund(row) {
      ...
      checkRefundStatus(param).then(res => {
    ...
    getDetail() {
      getRefund(this.rowData.refundId).then(res => {

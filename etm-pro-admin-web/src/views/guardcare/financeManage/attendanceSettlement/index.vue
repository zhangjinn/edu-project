    ...
    <etm-main-body>
      <etm-layout-split>
        <div slot="header">
          <etm-tabs
            v-model="activeName"
            :setting="hasAuthOfBool('guardcare.attendanceSettlement.setting')"
            @setting="setting"
          >
            <el-tab-pane label="考勤结算" name="settle">
        ...
        <div slot="body">
          ...
          >
            <etm-table-tool-bar slot="tool">
              ...
              <div slot="right">
                <el-button
                  v-has="{authId: 'guardcare.attendanceSettlement.updateBatch'}"
                  plain
                  @click="batchSettle"
                >批量结算
                </el-button>
            ...
            <el-table-column slot="right" label="结算规则" min-width="240">
              <template slot-scope="scoped">
                <div v-html="scoped.row.settlementRule" />
              </template>
            </el-table-column>
            <el-table-column slot="right" label="应结算金额（元）" width="150" align="right" prop="toBeSettledAmount" />
            <el-table-column slot="right" label="实结算金额（元）" width="150" align="right">
              <template slot-scope="scoped">
                <span>{{ scoped.row.actualSettlementAmount }}</span>
                <span
            ...
            <el-table-column slot="right" label="已收剩余（元）" width="150" align="right">
              <template slot-scope="scoped">
                {{ scoped.row.balance }}<span class="labelText">（{{ scoped.row.settlementItem }}）</span>
              </template>
            </el-table-column>
            <el-table-column slot="right" label="状态" width="90">
              <template slot-scope="scoped">
                <span>{{
            ...
            <el-table-column slot="right" label="结算日期" width="150">
              <template slot-scope="scoped">
            ...
            <el-table-column slot="right" fixed="right" label="操作" width="120">
              <template slot-scope="scoped">
                <etm-text
                  v-if="scoped.row.status === 'UNSETTLED'"
                  v-has="{authId: 'guardcare.attendanceSettlement.update'}"
                  type="primary"
...
import {
  ...
  getSettlementList,
  resetSettleAmount
} from '@/api/guardcare/financeManage/attendanceSettlement'
...
export default {
  ...
  methods: {
    setting() {
      this.$router.push({
        path: '/guardcareSettings/guardcare_attendanceSettleSetting'
      })
    },
    getClazz(value) {
    ...
    getList() {
      ...
      getSettlementList(param).then(res => {
    ...
    batchSettle() {
      if (!this.selectionValue.length) {
        ...
        for (let i = 0; i < this.selectionValue.length; i++) {
          ...
          checkSettleStatus(param).then(res => {

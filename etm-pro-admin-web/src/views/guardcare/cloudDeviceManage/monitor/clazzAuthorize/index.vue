<template>
  <div class="clazzAuthorize">
        ...
        <div class="collapse">
          ...
          >
            <el-collapse-item
              ...
              :name="g.gradeId"
            >
              <div v-for="(c, cindex) in g.clazzList" :id="'clazz' + c.clazzId" :key="cindex" class="clazzItem" @click.stop="e => selectClazz(e, c)">{{ c.clazzName }}</div>
            </el-collapse-item>
          </el-collapse>
    ...
    >
      <camera-pop ref="camera" type="PARENT" :cams="authCamsList" />
    </etm-pop>
      ...
      for (let i = 0; i < this.clazzTree.length; i++) {
        if (this.clazzTree[i].gradeId === gradeId) {
          if (this.clazzTree[i].clazzList._request) {
            ...
            // 还未请求
            queryClazzList(gradeId).then(res => {

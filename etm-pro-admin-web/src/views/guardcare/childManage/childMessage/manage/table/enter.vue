  ...
  <div class="table">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId: 'guardcare.child.create'}" type="primary" @click="add">
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="hasAuthOfBool('guardcare.child.import')" @click.native="importChild">导入幼儿信息
              </el-dropdown-item>
              <el-dropdown-item v-if="hasAuthOfBool('guardcare.childBatch.import')" @click.native="toImportBatch">导入批次管理
              </el-dropdown-item>
              <el-dropdown-item
                v-if="hasAuthOfBool('guardcare.childClazzChange.updateBatch')"
                @click.native="adjustBatch"
              >批量调班
              </el-dropdown-item>
              <el-dropdown-item v-if="hasAuthOfBool('guardcare.childLeave.createBatch')" @click.native="leaveBatch">批量退园
              </el-dropdown-item>
              <el-dropdown-item v-if="hasAuthOfBool('guardcare.childSettled.excelExport')" @click.native="exportChild">
                导出Excel
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
      ...
      <el-table-column slot="right" label="操作" fixed="right" width="300">
        <template slot-scope="scoped">
          <etm-text type="primary" @click="showParent(scoped.row)">家长管理</etm-text>
          <etm-text type="primary" @click="edit(scoped.row)">编辑</etm-text>
          <etm-text v-has="{authId: 'guardcare.childClazzChange.update'}" type="primary" @click="adjust(scoped.row)">
            调班
          </etm-text>
          <etm-text v-has="{authId: 'guardcare.childLeave.create'}" type="primary" @click="leave(scoped.row)">退园
          </etm-text>
    ...
    >
      <div class="pop">
        <etm-form ref="smallForm" :model="smallForm" :rules="smallRule" type="dialog">
          ...
          <template v-if="smallTitle === '调班'">
            ...
            <el-form-item label="调班后班级" prop="clazzValue">
              <etm-clazz-cascader @change="getClazz" />
            </el-form-item>
          </template>
          <template v-else-if="smallTitle === '退园'">
            <el-form-item label="退园原因" prop="outReason">
              <el-select v-model="smallForm.outReason" placeholder="请选择退园原因">
    ...
    <etm-pop
      ...
      @cancel="addPopCancel"
    >
      <batch-pop v-if="addTitle === '批量调班' || addTitle === '批量退园'" :batch-data="batchData" :type="batchType" />
      <child-form
        ...
        @repeat="getRepeat"
      />
      <import-pop v-else-if="addTitle === '导入幼儿信息'" :form-data="importData" />
    </etm-pop>
    <etm-drawer
    ...
    >
      <detail
        ...
        @close="closeDrawer"
      />
      <parent-drawer v-else :child-id="childId" :child-name="childName" />
    </etm-drawer>
  </div>
</template>
...
import parentDrawer from '@/views/guardcare/childManage/childMessage/component/parentDrawer'
...
import {
  ...
  clazzAdjust,
  addChild,
  deleteChild, exportChildExcel, importChildExcel
...
export default {
  ...
  methods: {
    ...
    toImportBatch() { // 导入批次管理页面
      this.$router.push({
        path: '/guardcareSettings/guardcare_childImportManage'
      })
    },
    adjustBatch() { // 批量调班弹窗
      this.addTitle = '批量调班'
    ...
    addPopConfirm() { // 添加幼儿大弹窗确认
      if (this.addTitle === '添加幼儿') {
        ...
        if (!this.importData.fileList.length) {
          ...
          importChildExcel(this.importData.fileList[0]).then(res => {
            this.addPop = false
            this.getList()
            const data = res.data
            this.$etmTip.init({

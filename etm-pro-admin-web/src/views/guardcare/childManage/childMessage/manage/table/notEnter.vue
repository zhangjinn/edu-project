  ...
  <div class="table">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId: 'guardcare.childUnsettled.skip'}" type="primary" @click="scan">扫码登记
          </el-button>
        </div>
      </etm-table-tool-bar>
      ...
      <el-table-column slot="right" label="操作" width="200">
        <template slot-scope="scoped">
          <etm-text v-has="{authId: 'guardcare.childUnsettled.exam'}" type="primary" @click="aduit(scoped.row)">入园审核
          </etm-text>
        </template>
      </el-table-column>
    </etm-table-page>
...
export default {
  ...
  methods: {
    getList() {
      ...
      getPendingChildList(param).then(res => {
    ...
    scan() {
      this.$router.push({
        path: '/guardcareSettings/guardcare_register'
      })
    },
    aduit(row) {
      this.showSide = true
      getChildDetail(row.childId).then(res => {
    ...
    saveChild() { // 入园审核
      this.$refs.childForm.$children[1].$children[0].validate(valid => {
        if (valid) {
          const param = {

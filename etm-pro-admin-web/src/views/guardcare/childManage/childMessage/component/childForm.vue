<template>
  <div class="form">
    ...
    <etm-form ref="form" :model="formData" :rules="rule" type="dialog">
      <el-form-item v-show="formType === 'edit'" label="头像">
        <etm-upload-avatar
          v-model="formData.avatar"
          :resource-name="'guardcare/childManage'"
          media-type="image"
          @success="success"
        />
      </el-form-item>
      <el-form-item label="幼儿姓名" prop="childName">
        <el-input v-model="formData.childName" placeholder="请输入幼儿姓名" @blur="verifyChild" />
      </el-form-item>
      ...
      <el-form-item label="入读班级" prop="clazz">
        <div class="multipleForm clazz">
          ...
          <el-select
            ...
            placeholder="请选择班级"
          >
            <el-option v-for="(clazz, i) in clazzList" :key="i" :label="clazz.name" :value="clazz.id" />
          </el-select>
        </div>
      </el-form-item>
      <el-form-item required>
        ...
        <div class="contactBox">
          <div
            ...
            :class="formType"
          >
            <el-form-item :prop="'contactList.'+ index +'.relationship'" :rules="rule.relationship">
              <el-select
                v-model="item.relationship"
                placeholder="联系人关系"
              >
                <el-option v-for="(list, i) in relationList" :key="i" :label="list.label" :value="list.value" />
              </el-select>
            </el-form-item>
            <el-form-item :prop="'contactList.'+ index +'.name'" :rules="rule.name">
              <el-input v-model="item.name" placeholder="联系人姓名" @blur="verifyChild" />
            </el-form-item>
            <el-form-item :prop="'contactList.'+ index +'.telephone'" :rules="rule.telephone">
              <el-input v-model="item.telephone" placeholder="手机号码" @change="checkPhone($event, index)" />
            </el-form-item>
      ...
      <el-form-item label="国籍/地区">
        <el-select v-model="formData.nationality" filterable>
          <el-option
      ...
      <el-form-item label="现住地">
        <div class="multipleForm">
          ...
          >
            <el-option v-for="(city, i) in liveCityList" :key="i" :label="city.name" :value="city.code" />
          </el-select>
          <el-select
            v-model="formData.liveArea.county"
            placeholder="请选择县区"
          >
            <el-option v-for="(county, i) in liveCountyList" :key="i" :label="county.name" :value="county.code" />
          </el-select>
        </div>
      </el-form-item>
      <el-form-item label="详细地址">
        <el-input v-model="formData.currentAddress" placeholder="请输入详细地址" />
      </el-form-item>
      <el-form-item label="户口所在地">
        <div class="multipleForm">
          ...
          >
            <el-option v-for="(city, i) in birthCityList" :key="i" :label="city.name" :value="city.code" />
          </el-select>
          <el-select
            v-model="formData.birthArea.county"
            placeholder="请选择县区"
          >
            <el-option v-for="(county, i) in birthCountyList" :key="i" :label="county.name" :value="county.code" />
          </el-select>
        </div>
      </el-form-item>
    </etm-form>
    ...
    <el-collapse v-model="collapseActive">
      <el-collapse-item name="1">
        <etm-form :model="formData" type="dialog">
          <el-form-item label="幼儿证件">
            <div class="multipleForm twoForm">
              ...
              <el-input v-model="formData.certificateNumber" placeholder="请输入证件号码" @blur="checkCardNumber" />
            </div>
          </el-form-item>
          ...
          <el-form-item label="血型">
            <el-select v-model="formData.bloodType" placeholder="请选择血型">
              <el-option v-for="(blood, i) in bloodList" :key="i" :label="blood.label" :value="blood.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="健康状况">
            <el-select v-model="formData.healthCondition" placeholder="请选择健康状况">
              <el-option v-for="(health, i) in healthList" :key="i" :label="health.label" :value="health.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="曾患疾病">
            <el-select v-model="formData.onceSick" multiple placeholder="请选择疾病状况" @change="$forceUpdate()">
              <el-option v-for="(sick, i) in sickList" :key="i" :label="sick.label" :value="sick.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="食物过敏">
            <el-input v-model="formData.foodAllergyInfo" placeholder="请输入食物过敏情况" />
          </el-form-item>
          <el-form-item label="药物过敏">
            <el-input v-model="formData.drugAllergyInfo" placeholder="请输入药物过敏情况" />
          </el-form-item>
  ...
  methods: {
    ...
    checkPhone(value, index) {
      if (!this.oldPhoneNumber.includes(value) && validPhoneNumber(value)) {
        ...
        param.telephoneList.push(value)
        checkParentPhone(param).then(res => {
          ...
          if (data.length) {
            ...
            data.forEach((item, index) => {
              if (index > 0) {
                str += '、' + item.childName + '（' + this.transferRelation(item.relationship) + '）'
              } else {
                str += item.childName + '（' + this.transferRelation(item.relationship) + '）'
              }
            })
            const html = value + '已绑定' + str + '，是否继续' + (this.formType === 'add' ? '添加？' : '修改？')
            this.$etmTip.init({
              type: 'warn',
              title: '确认',
              html,
    ...
    checkCardNumber() {
      if (!this.formData.certificateNumber) {
        return
      } else if (this.formData.certificateNumber.length !== 15 && this.formData.certificateNumber.length !== 18) {
        this.$message.warning('身份证号格式应为15位或18位')
      }
    }
  }
}

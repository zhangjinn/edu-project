...
export default {
  ...
  computed: {
    basicDone() {
      return this.$store.state.memberManageV2.basic
    },
    statusDone() {
      return this.$store.state.memberManageV2.status
    },
    itemDone() {
      return this.$store.state.memberManageV2.item
    },
    foodDone() {
      return this.$store.state.memberManageV2.food
    },
    medicineDone() {
      return this.$store.state.memberManageV2.medicine
    }
  },
  watch: {
    active(v) {
  ...
  created() {
    ...
    // 初始化表单的必填状态
    this.$store.commit('memberManageV2/SET_BASIC', false)
    this.$store.commit('memberManageV2/SET_STATUS', false)

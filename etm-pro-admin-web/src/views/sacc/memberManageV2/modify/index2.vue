...
export default {
  ...
  computed: {
    cansave() {
      return this.$store.state.memberManageV2.basic && this.$store.state.memberManageV2.status
    },
    currentId() {
    ...
    getContactType('CONTACTS').then(res => {
      ...
      getGardenXq(this.currentId)
        .then(res => {
          ...
          // 初始化进入表单时候的tab状态
          this.$store.commit('memberManageV2/SET_TAB', 'basic')
        })
  ...
  methods: {
    handleTabClick(a, o) {
      if (!this.$store.state.memberManageV2[o] && ['basic', 'status'].indexOf(o) !== -1) {
        this.$message({
          type: 'warning',

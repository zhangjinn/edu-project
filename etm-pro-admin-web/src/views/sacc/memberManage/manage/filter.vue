...
export default {
  ...
  data() {
    const _this = this
    return {
      activeName: this.$store.state.memberManage.gardenState,
      show: true,
      informData: [
        {
  ...
  watch: {
    activeName(val) {
      this.$store.commit('memberManage/SET_GARDENSTATE', val)
    }
  },
  methods: {
    toggle(val) {
    ...
    onSubmit() {
      ...
      this.$emit('query')
      this.$store.commit('memberManage/SET_FILTER', filter)
    },
    reset() {
      let formData
      if (this.activeName === 'IN') {

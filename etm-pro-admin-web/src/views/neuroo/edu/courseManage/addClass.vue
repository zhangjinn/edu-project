  ...
  <div class="addClass-wrap">
    <etm-form ref="form" class="form" :model="form" :rules="rules" style="flex: 1;overflow:auto;">
      ...
      <el-form-item label="园区:" prop="parkName">
        <el-select v-model="form.parkName" placeholder="请选择园区">
          <el-option
            v-for="(park, index) of parkNames"
            :key="index"
            :label="park.label"
            :value="park.value"
          />
      ...
      <el-form-item label="会员:" prop="customerIds">
        <el-select
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        parkName: { required: true, message: '请选择园区', trigger: 'change' },
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].$children[0].validate(valid => {
        if (valid) {
          const { clazzName, monthAgeStart, monthAgeEnd, fullNumber, remark, headTeacherIds, teacherIds, nurseryIds, customerIds, doctorIds } = this.form
          if (this.clazzId) {
            updateClazz({
              clazzId: this.clazzId, clazzName, monthAgeStart, monthAgeEnd, fullNumber, remark, headTeacherIds, teacherIds, nurseryIds, doctorIds,
              customerIds
            }).then(res => {
            ...
            addClazz({ clazzName, monthAgeStart, monthAgeEnd, fullNumber, remark, headTeacherIds, teacherIds, nurseryIds, customerIds, doctorIds }).then(res => {

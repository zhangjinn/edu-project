<template>
  <el-form class="recharge" label-width="80px">
    <el-form-item label="充值组织:">
      {{ organization }}
    </el-form-item>
    <el-form-item v-if="item.type === 'CALL'" label="电话套餐:">
      <choice @change="change" />
    </el-form-item>
    <el-form-item v-if="item.type === 'MESSAGE'" label="短信套餐:">
      <msg-package @change="handleMsgChange" />
    </el-form-item>
    <el-form-item v-if="item.type === 'CLOUD_DISK'" label="云盘套餐:">
      <cloud-package @change="handleCloud" />
    </el-form-item>
    <el-form-item label="合计金额:">
      <span v-if="total" class="amount">{{ total }} 元 </span>

...
import { getPositionEmployees, removePositionEmployees } from '@/api/post'
...
export default {
  ...
  methods: {
    ...
    getStaffs() {
      this.entry.postId = this.currentPostId
      getPositionEmployees(this.entry).then(response => {
        this.postStaffList = response.data.content
    ...
    removePost() {
      this.removeEntry.fkPostIds.push(this.currentPostId)
      this.removeEntry.employeeIds = this.getSelectEmployeeList()
      if (this.removeEntry.employeeIds.length === 0) {
        this.$message.error('请选择员工')
      } else {
        removePositionEmployees(this.removeEntry).then(response => {
          if (response.code === 0) {
            // 移除成功
            this.getStaffs()
          } else {

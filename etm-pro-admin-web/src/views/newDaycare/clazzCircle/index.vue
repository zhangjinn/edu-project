      ...
      <etm-layout-split>
        <div slot="header">
          <etm-tabs v-model="activeName" setting setting-text="家长端设置" @setting="$router.push({name: 'cm_setting'})">
            <el-tab-pane label="家园动态" name="dynamic">
        ...
        <div slot="body">
          <etm-table-page :columns="columns" :data="tableData" @currentChange="currentChange">
            <etm-table-tool-bar slot="tool">
              <div slot="right">
                <el-button plain @click="addDynamic">发布动态</el-button>
              </div>
            </etm-table-tool-bar>
            <el-table-column prop="orderNo" label="操作" fixed="right" min-width="120">
              <template slot-scope="scope">
                <etm-text type="primary" @click="preview(scope.row.dynamicId)">预览</etm-text>
                <etm-text type="primary" @click="deleteDynamic(scope.row.dynamicId)">删除</etm-text>
              </template>
            </el-table-column>
          </etm-table-page>
        </div>
        ...
        >
          <etm-layout-split>
            ...
            <div v-if="Object.keys(dynamicPreviewData).length !== 0" class="preview-box">
              <div class="content">
                <div class="top-txt">1/{{ dynamicPreviewData.urlList.length }}</div>
                <div class="middle-content">
                  ...
                  <img
                    ...
                    alt=""
                  >
                  <video v-else-if="dynamicPreviewData.dynamicType === 'VIDEO'" :src="dynamicPreviewData.urlList[0].url" />
                </div>
                <div class="bottom-txt">
                  <div class="avatar-name">
                    <el-avatar size="small" :src="dynamicPreviewData.createAvatar" />
                    <div class="name">{{ dynamicPreviewData.createName }}</div>
                  </div>
                  <div class="content-txt">{{ dynamicPreviewData.content }}</div>
                </div>
              </div>
            </div>
          </etm-layout-split>
        ...
        <etm-pop
          ...
          @cancel="popCancel"
        >
          <etm-form v-if="showPop" type="dialog">
              ...
              <div class="upload-img-video">
                <div class="flex-box">
                  <template v-if="radio === 'image'">
                    <div v-for="(imgUrl, index) in imgUrls" :key="index" class="preview">
                      <img :src="imgUrl" alt="">
                      <i class="el-icon-close" @click="removeImage(index)" />
                    </div>
                    <div v-if="imgUrls.length < 30" class="box-upload" @click="chooseImageOrVideo">
                  ...
                  <template v-if="radio === 'video'">
                    <div v-if="videoUrl" class="preview">
                      ...
                      <i class="el-icon-close" @click="removeVideo" />
                    </div>
                    <div v-else class="box-upload" @click="chooseImageOrVideo">
                      <i class="el-icon-plus" />
                    </div>
                  </template>
                </div>
                <span v-if="radio==='image'" class="tip-txt">图片最多上传30张</span>
                <span v-if="radio==='video'" class="tip-txt">视频最多上传1个</span>
              </div>
              <input ref="imageRef" type="file" multiple accept="image/*" style="display: none" @change="chooseImage">
              <input ref="videoRef" type="file" accept="video/*" style="display: none" @change="chooseVideo">
            </el-form-item>

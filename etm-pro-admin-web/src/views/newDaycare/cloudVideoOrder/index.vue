  ...
  <div class="index-wrap">
    <etm-main-body>
      <etm-layout-split>
        <div slot="header">
          <etm-tabs ref="tabs" v-model="activeName">
            ...
            <el-tab-pane
              ...
              name="cloudOrder"
            >
              <etm-tool-bar v-model="formData2" @search="search" @reset="reset" />
            </el-tab-pane>
          </etm-tabs>
        </div>
        <div slot="body">
          <etm-table-page v-if="activeName === 'cloudVideoOrders'" :columns="columns" :data="tableData" index selection label="序号" @currentChange="currentChange">
            ...
            <el-table-column prop="orderNo" label="订单编号" :fixed="true">
              <template slot-scope="scope">
                <etm-text type="primary" @click="clickOrder(scope.row.cameraOrderId)">{{ scope.row.orderNo }}</etm-text>
              </template>
            </el-table-column>
          </etm-table-page>
          <cloudOrder v-else :search-info="orderInfo" />
        </div>
        ...
        >
          <etm-layout-split>
            <etm-title size="big" :show-line="true" border>详情</etm-title>
            <template>

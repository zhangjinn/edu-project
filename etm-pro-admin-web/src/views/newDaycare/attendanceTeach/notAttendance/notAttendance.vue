  ...
  <div class="notAttendance-wrap">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId: 'daycare.attendanceTeach.createBatch'}" plain @click="addArticle">批量考勤
          </el-button>
          <el-button v-has="{authId: 'daycare.attendanceTeach.cancel'}" plain @click="cancelAppointment">
            取消预约
          </el-button>
        </div>
      </etm-table-tool-bar>
      ...
      <el-table-column slot="right" fixed="right" label="操作" width="120">
        <template v-slot="row">
          <etm-text v-has="{authId: 'daycare.attendanceTeach.createBatch'}" type="primary" @click="handleEdit(row.row)">
            考勤
          </etm-text>
        </template>
      </el-table-column>

<template>
  <div class="index-wrap">
    <etm-main-body>
      <etm-layout-split>
        <div slot="header">
          <etm-tabs ref="tabs" v-model="activeName" :setting="hasAuthOfBool('daycare.attendanceTeach.setting')" @setting="setting">
            <el-tab-pane v-if="hasAuth('daycare.attendanceUndoTeach.queryBatch', 'tabs', 'activeName')" label="未考勤" name="notAttendance">
              <etm-tool-bar v-model="formData" @search="handleSearch" />
            </el-tab-pane>
            <el-tab-pane v-if="hasAuth('daycare.attendanceDoneTeach.queryBatch', 'tabs', 'activeName')" label="已考勤" name="attendance">
              <etm-tool-bar v-model="formData2" @search="handleSearch2" />
            </el-tab-pane>
          </etm-tabs>
        </div>
        <div slot="body">
          <not-attendance v-if="activeName === 'notAttendance'" :not-search="notSearch" />
          <attendance v-if="activeName === 'attendance'" :attend-search="attendSearch" />
        </div>
      </etm-layout-split>
    </etm-main-body>
  </div>

<template>
  <div class="feedback-wrap">
      ...
      <etm-layout-split>
        <div slot="header">
          <etm-tabs ref="tabs" v-model="activeName">
            <el-tab-pane label="员工反馈" name="employee">
              <etm-tool-bar v-model="employeeFilter" @search="handleSearch" @reset="handleReset">
                <div class="garden">
                  <span>园所:</span>
                  <el-cascader
                    v-model="queryGarden"
        ...
        <div slot="body">
          ...
          >
            <el-table-column slot="left" label="头像" prop="avatar" align="center" width="80" fixed="left">
              <template slot-scope="{row}">
                <div class="avatar">
                  <img v-if="row.avatar !== '--' && row.avatar" :src="row.avatar" alt="">
            ...
            >
              <template slot-scope="{row}">
                ...
                >查看图片</etm-text>
                <etm-text v-if="row.status === 'PENDING' || row.status === 'PROCESSED'" type="primary" @click="handleSingleReply(row)">回复</etm-text>
                <etm-text v-if="row.status === 'PROCESSED'" type="primary" @click="handleShowResult(row)">回复详情</etm-text>
              </template>
            </el-table-column>
          </etm-table-page>
        </div>
    ...
    >
      <etm-form ref="form" :model="formData" type="dialog" :rules="rules">
        <el-form-item v-if="popTitle !== '批量回复'" label="问题/意见">
          {{ opinion }}
        </el-form-item>
        <!-- 回复详情 -->
        <template v-if="popTitle === '回复详情'">
          <div v-for="(item, index) in resultList" :key="index" class="wrapper">
        ...
        <el-form-item v-else label="回复内容" prop="content">
          <el-input v-model="formData.content" type="textarea" :rows="5" placeholder="请输入内容" maxlength="200" show-word-limit />
        </el-form-item>
...
import { queryFeedbackList, handleFeedback, queryFeedbackResultDetail } from '@/api/newDaycare/feedb'

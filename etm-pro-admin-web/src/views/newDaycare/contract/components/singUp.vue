<template>
  <div class="chargeDetailEdit-wrap">
    ...
    >
      <div v-if="isShow">
        <etm-form ref="form1" type="dialog" :model="form">
          <el-form-item label="合同单号" prop="contractNo" :rules="formValidate.nullValueRule(true,'请输入合同单号')">
            <el-input v-model="form.contractNo" />
          </el-form-item>
          <el-form-item label="学员姓名" prop="type">
            {{ form.childName }}
          </el-form-item>
          <el-form-item v-if="prop.signUpType === 'EARNEST'" label="业务类型" prop="type">
            托育
          </el-form-item>

          <el-form-item label="合同类型" prop="contractType" :rules="formValidate.nullValueRule(true,'请选择合同类型')">
        ...
        <div class="itemTable">
          ...
          <div class="buttonBox">
            ...
            <el-button v-if="prop.signUpType !== 'EARNEST'" type="dashed" @click="selectEvent(form.type === 'EARNEST' ? 'nurseType' : 'earlyType', '课程类型')">
              选择课程类型
            </el-button>
            <el-button v-if="prop.signUpType !== 'EARNEST'" type="dashed" @click="selectEvent(form.type === 'EARNEST' ? 'nurseCourse' : 'earlyCourse', '课程')">
              选择课程
            </el-button>

            <el-button type="dashed" @click="selectEvent(prop.signUpType !== 'EARNEST' ? 'fee' : 'feeCare', '费用')">选择费用</el-button>
            <el-button type="dashed" @click="selectEvent('item', '物品')">选择物品</el-button>
          </div>

          <!--    嵌套模板组件-->

          <nesting-table v-model="templateData" :show-img="true" @del="deleteTel" />
        </div>
            ...
            <el-col :span="24">
              <etm-form ref="form2" type="dialog" :model="form">
                <el-form-item label="使用定金">
                  <el-select v-model="form.earnestMoneyId" @change="userDJ">
                    <el-option
                ...
                <el-form-item prop="paymentId" label="支付方式" :rules="formValidate.nullValueRule(true,'请选择支付方式')">
        ...
        <div>
          ...
          <el-row>
            <el-col :span="24">
              <etm-form ref="form3" type="dialog" :model="form">
                ...
                <el-form-item v-if="prop.signUpType === 'EARNEST'" required label="合同时长">
                  <el-row>
                    <el-col :span="8">
                      <el-form-item
                        prop="duration_month"
                ...
                <el-form-item v-if="prop.signUpType === 'CARE'" label="有效期" prop="timeRange" :rules="formValidate.nullValueRule(true, '请选择有效期')">
                  <el-date-picker
                ...
                <el-form-item label="备注">
                  <el-input v-model="form.remake" type="textarea" :rows="5" placeholder="请输入备注内容" />
                </el-form-item>
              </etm-form>
            </el-col>
          </el-row>
    ...
    >
      <div class="printPop">
        <p>签约成功</p>
        <el-button type="primary" @click="printMyContract">打印合同</el-button>
      </div>
    </etm-pop>
  </div>
...
export default {
  ...
  methods: {
    ...
    init(prop) {
      ...
      queryMoneyLists(this.prop.id).then(res => {
        const _result = {}

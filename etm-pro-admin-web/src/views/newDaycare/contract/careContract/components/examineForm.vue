    ...
    <etm-drawer :visible.sync="isShow" type="big">
      <etm-layout-split>
        <etm-title :show-line="false" border size="big">
          ...
          <template slot="tool">
            <el-button size="mini" plain @click="noReason = true">不通过</el-button>
            <el-button size="mini" type="primary" @click="verifContract">通过</el-button>
          </template>
        </etm-title>
        ...
        <div>
          ...
          <etm-field-label label="使用定金" type="left">{{ detailData.earnestMoney || '无' }}</etm-field-label>
          <etm-field-label label="应收总额" type="left">{{ detailData.receivable }}</etm-field-label>
          <etm-field-label label="实收总计" type="left">{{ detailData.paid }}</etm-field-label>
          <etm-field-label label="支付方式" type="left">{{ detailData.paymentName }}</etm-field-label>
          <!--          <el-row>-->
          <!--            <el-col :span="16">-->
        ...
        <div>
          ...
          <etm-field-label label="签约日期" type="left">{{ detailData.date }}</etm-field-label>
          <etm-field-label label="有效期" type="left">
            {{ detailData.validBeginTime }}至{{ detailData.validEndTime }}
          </etm-field-label>
          <etm-field-label label="业绩人及占比" type="left">
            {{ detailData.mainPerformanceName }}-{{ detailData.mainPercentage }}%
          </etm-field-label>
          <etm-field-label label="次业绩人及占比" type="left">
            <span v-if="detailData.secondaryPerformanceName">{{
              detailData.secondaryPerformanceName
            }}-{{ detailData.secondaryPercentage }}%</span>
            <span v-else>无</span>
          </etm-field-label>
          <etm-field-label label="经办人" type="left">{{ detailData.createName }}</etm-field-label>
          <etm-field-label label="经办时间" type="left">{{ detailData.createTime }}</etm-field-label>
          <etm-field-label label="备注" type="left">{{ detailData.remake }}</etm-field-label>

          <etm-field-label label="资料文件">
            <template v-if="detailData.annex && detailData.annex.length">
              <div v-for="(item, index) in (detailData.annex || [])" :key="index" @click="download(item)"><etm-text>{{ item.name }}</etm-text></div>
            </template>
            <span v-else>无</span>
          </etm-field-label>
      ...
      >
        <el-input v-model="reasonText" type="textarea" :rows="6" placeholder="请输入不通过原因" />
      </etm-pop>
    </etm-drawer>
  </div>
...
import { queryCacContractDetail, updateVerifContract } from '@/api/newDaycare/contract'
...
export default {
  ...
  methods: {
    ...
    verifContract() {
      updateVerifContract({ id: this.prop.id, toExamineState: true }).then(res => {
        this.$message('审核通过成功')
        this.$emit('update', {})
        this.isShow = false
      })
    ...
    noReasonEvent() {
      ...
      updateVerifContract({ id: this.prop.id, toExamineState: false, reason: this.reasonText }).then(res => {
        // this.$message('审核通过成功')
        this.$message('提交成功')
        this.$emit('update', {})
        this.isShow = false
    ...
    init(prop) {
      ...
      queryCacContractDetail(prop.id).then(res => {
        ...
        // this.tableData = res.data.listPurchaseItemsVo

<template>
  <div class="deposit-wrap">
    <etm-form ref="form" type="dialog" :model="detail">
      <el-form-item label="会员卡号" prop="cardNumber" :rules="formValidate.nullValueRule(true, '请输入会员卡号')">
        <el-input v-model.number="detail.cardNumber" />
      </el-form-item>
      <el-form-item label="卡类型" prop="cardTypeId">
        <el-select v-model="detail.cardTypeId">
          <el-option v-for="(item, index) in cardType" :key="index" :label="item.membershipCardTypeName" :value="item.membershipCardTypeId" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="关联联系人" prop="kinshipId">
        <el-select v-model="detail.kinshipId">
          <el-option v-for="(item, index) in relateLists" :key="index" :label="`${item.parentName} (${item.relationshipName})-${item.telephone}`" :value="item.kinshipId" />
        </el-select>
      </el-form-item>

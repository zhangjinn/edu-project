<template>
  <div class="aboutCourse-wrap">
    <etm-pop
      ...
      @confirm="confirm"
    >
      <etm-tool-bar v-if="isShow" v-model="formData_education" @reset="reset" @search="searchHandle" />
      <div class="aboutBody">
        <etm-table :data="data" :columns="columns" @selection-change="choiceChild">
          <etm-table-tool-bar slot="tool">
            <div class="aboutTools">
              <div class="toolItem">已选择
                <span class="theme">{{ choiceNumber }}</span>
                节
              </div>
              <div v-if="prop.type === 'app'" class="toolItem subTool">
                <div>剩余约课课时：</div>
                <div class="toolNumber">
                  <div v-for="(item, index) in remainingCourseLessonMap.slice(0, 3)" :key="index">
                    <h3>{{ item.value }}</h3>
                    <span>{{ item.name }}</span>
                  </div>
          ...
          <el-table-column slot="left" type="selection" :selectable="selectable" />
        </etm-table>
      </div>
    </etm-pop>
  </div>

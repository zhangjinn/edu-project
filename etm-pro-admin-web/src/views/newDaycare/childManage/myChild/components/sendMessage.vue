<template>
  <div class="sendMessage-wrap">
    ...
    >
      <etm-form ref="etmForm" type="dialog" :model="message" :rules="rules">
        <el-form-item label="收信人号码">
          <div>
            <el-dropdown :hide-on-click="false">
              ...
              <el-dropdown-menu v-if="sendMessage.phone.length > 1" slot="dropdown">
                <el-dropdown-item
                  v-for="(item, index) in sendMessage.phone"
                  :key="index"
                >
                  <div :class="[arrayInclude(choicePhone, item) ? 'choice' : '']" @click="phonePush(item)">
                    {{ sendMessage.parentName[index] }}
                    {{ item }}
                    &emsp;
                    <i v-if="arrayInclude(choicePhone, item)" class="el-icon-check" />
        ...
        <el-form-item label="选择模板">
          <el-select v-model="message.templateId" @change="handleTemplateChange">
            <el-option v-for="(temp, i) of templates" :key="i" :label="temp.label" :value="temp.value" />
          </el-select>
        </el-form-item>

        <el-form-item label="短信内容" prop="content">
          <el-input v-model="message.content" type="textarea" :rows="4" />
        </el-form-item>
      </etm-form>
    </etm-pop>
  </div>
  ...
  methods: {
    ...
    init() {
      querySmsTemplate({ keyword: null }).then(res => {
        this.templates = this.convertDataToOptions(res.data, 'name', 'id')
      })
    },
      ...
      this.$refs.etmForm.$children[0].validate(valid => {
        if (valid) {
          createSms({
            ...
            childId: this.sendMessage.childId
          }).then(res => {

    ...
    >
      <etm-form ref="form" :model="form" type="dialog">
        <el-form-item label="预约试听">
          <div v-for="(item, index) in form.list" :key="index" class="auditionItem">
            <el-row>
              <el-col :span="11">
                <el-form-item :prop="`list.${index}.datetime`" :rules="formValidate.nullValueRule(true,'请选择日期')">
                  <el-date-picker
              ...
              <el-col :span="9" :offset="1">
                <el-form-item :prop="`list.${index}.clazzId`" :rules="formValidate.nullValueRule(true,'请选择班级')">
                  <el-select v-model="form.list[index].clazzId" @change="selectChange($event, index)">
                    <el-option v-for="(i,k) in item.clazz" :key="k" :label="`${i.caeClazzName || ''} ${i.headTeacherName || ''}`" :value="i.caeClazzId" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="2" :offset="1">
                <i v-if="index === 0" class="el-icon-plus" @click="plus" />
                <i v-else class="el-icon-minus" @click="reduce(index)" />
              </el-col>
            </el-row>
          </div>

<template>
  <div class="childFollowPanel-wrap">
    <div class="btn-box">
      <el-button type="primary" @click="commentFollow('FOLLOW_UP')">写跟进</el-button>
      <el-button plain @click="commentFollow('PHONE', $event)">打电话</el-button>
      <!--<el-button @click="commentFollow('TASK')">任务</el-button>-->
      <el-button plain @click="commentFollow('SEND_MESSAGE', $event)">发短信</el-button>
      ...
      >
        <ul>
          <li v-for="(item, index) of childInfoDetail.relationshipList" :key="index" class="callLi">
    ...
    <div v-if="isCommentFollow" class="follow-box">
      <el-collapse-transition>
        <el-form label-width="100px" label-position="left" :model="followData">
          <el-row :span="24" :gutter="24">
            <el-col :span="12">
              <el-form-item label="跟进时间">
                <el-date-picker v-model="followData.followTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="下次跟进时间">
                <el-date-picker v-model="followData.nextFollowTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :span="24" :gutter="24">
            <el-col :span="12">
              <el-form-item label="跟进方式">
                <el-select v-model="followData.meansId" clearable>
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="学员状态">
                <el-select v-model="followData.childStatus" clearable>
                  <el-option
          ...
          <el-row :span="24" :gutter="24">
            <el-col :span="12">
              <el-form-item label="重要程度">
                <el-select v-model="followData.childImportance" clearable>
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="预约到访时间">
                <el-date-picker v-model="followData.visitTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="选择模板" placeholder="请选择模板">
                <el-select v-model="followData.templateId" clearable @change="handleTemplateChange">
                  <el-option
          ...
          <el-row style="padding-bottom: 24px;">
            <el-col>
              <el-input v-model="followData.content" type="textarea" resize="none" :rows="4" :maxlength="200" placeholder="请填写跟进内容" />
            </el-col>
          </el-row>
          <el-row style="padding-bottom: 24px;">
            <el-col>
          ...
          <el-row style="padding-bottom: 8px;">
            <el-col>
              <div class="btn-box">
                ...
                <el-button v-throttle type="primary" @click="next">保存并下一个</el-button>
              </div>
            </el-col>
          </el-row>
        </el-form>
    ...
    <etm-tabs v-model="currentTab" :show-toggle="false">
      <el-tab-pane label="全部" name="all" />
      <el-tab-pane label="跟进记录" name="follow" />
      <el-tab-pane label="任务" name="task" />
      <el-tab-pane label="到访记录" name="visit" />
      <div class="timeline">
        <el-timeline>
          <template v-if="currentTab === 'all'">
            ...
            >
              <template v-if="item.type === '到访'">
                <div class="title-box" style="align-items:flex-start">
                  <div class="title">预约{{ item.type }}
                ...
                <etm-field-label type="left" label="操作人">{{ item.createName }}</etm-field-label>
              </template>
              ...
              <template v-if="item.type === '跟进'">
                ...
                <etm-field-label type="left" label="跟进内容">{{ item.content }}</etm-field-label>
                <etm-field-label type="left" label="跟进方式">{{ item.meansName }}</etm-field-label>
                ...
                <etm-field-label type="left" label="跟进时间">{{
                  item.followTime | parseTime('{y}-{m}-{d} {h}:{i}')
                }}
                </etm-field-label>
                <etm-field-label type="left" label="下次跟进时间">{{
              ...
              <template v-if="item.type === '打电话'">
                ...
                <etm-field-label type="left" label="播放">
                  <etm-text v-if="!item.phoneRecordingUrl" type="normal">未接通</etm-text>
                  <etm-text v-else type="primary" @click="playAudio(item.phoneRecordingUrl, $event)">播放</etm-text>
                </etm-field-label>
                <etm-field-label type="left" label="操作人">{{ item.followerName }}</etm-field-label>
              </template>
              <template v-if="item.type === '发短信'">
                ...
                <etm-field-label type="left" label="短信内容">{{ item.content }}</etm-field-label>
                <etm-field-label type="left" label="操作人">{{ item.followerName }}</etm-field-label>
              </template>
          ...
          <template v-else-if="currentTab === 'task'">
            <el-timeline-item v-for="(item, index) of childDetail.taskList" :key="index" icon="iconfont iconrenwu">
          ...
          <template v-else-if="currentTab === 'visit'">
            <el-timeline-item v-for="(item, index) of childDetail.visit" :key="index" icon="iconfont iconyuyuedaofang">

              <div class="title-box" style="align-items:flex-start">
                <div class="title">预约{{ visitType[item.type] }}
                  <span :class="[item.visitType]">{{ visitStuts[item.visitType] }}</span>
                </div>
                <div class="time">
                  <span>{{ item.createTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
    ...
    <send-message ref="sendMessage" :send-message="sendMessage" />

    <resolve-visit ref="resolveVisit" :follow-id="followId" @update="updateVisit" />
    <about-course ref="aboutCourse" :prop="aboutProp" @getIds="getIds" />
    <audition-course ref="auditionCourse" :prop="auditionProp" @getIds="getIds" />
  </div>

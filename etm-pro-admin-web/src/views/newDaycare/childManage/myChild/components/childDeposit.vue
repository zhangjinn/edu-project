<template>
  <div class="child-deposit-wrap">
    <etm-form ref="etmForm" type="dialog" :model="deposit" :rules="rules">
      <el-form-item label="学员姓名">
        <span>{{ childInfo.childName }}</span>
      </el-form-item>
      <el-form-item label="收费日期">
        <el-date-picker v-model="deposit.dateCharge" value-format="yyyy-MM-dd" placeholder="请选择收费日期" />
      </el-form-item>
      ...
      <el-form-item label="支付方式" prop="paymentType">
        <el-select v-model="deposit.paymentType">
          <el-option v-for="(item, index) of paymentTypes" :key="index" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="备注">
        <el-input v-model="deposit.remark" :maxlength="200" type="textarea" :rows="4" />
      </el-form-item>
    </etm-form>
  </div>
...
const validatorMoneny = (rule, value, cb) => {
  if (!value) {
    return cb(new Error('定金金额不能为空'))
  }
  console.log(/^[1-9]\d*(\.\d+)?$/.test(value), value, rule)
  if (!/^[1-9]\d*(\.\d+)?$/.test(value)) {
    return cb(new Error('定金金额不正确'))
  }
  return cb()
}

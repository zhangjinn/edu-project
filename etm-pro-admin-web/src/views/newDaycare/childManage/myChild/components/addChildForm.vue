<template>
  <div class="addChildForm-wrap" :class="{layout : !childId}">
    <etm-form ref="etmForm" type="dialog" :model="childForm" :rules="childRules">
      <etm-title>基础信息</etm-title>
      <el-form-item label="头像">
        <etm-upload-avatar
          v-model="childForm.avatar"
          resource-name="daycare/child/new/image"
          media-type="image"
      ...
      <div class="layout-row">
        <div class="layout-col">
          <el-form-item label="学员姓名" prop="childName">
            <el-input ref="first" v-model="childForm.childName" />
          </el-form-item>
        </div>
        <div class="layout-col">
      ...
      <el-form-item label="地区">
        <etm-address v-model="childForm.areaCode" :area-id="childForm.areaCode" />
      </el-form-item>
      <el-form-item label="详细地址">
        <el-input v-model="childForm.address" placeholder="请输入详细地址" />
      </el-form-item>
      ...
      <el-form-item
        ...
        required
      >
        <div v-for="(contact, index) of childForm.relationshipList" :key="index" class="contact" :class="{edit : childId}">
          <el-form-item
          ...
          >
            <el-select v-model="contact.relationshipId" clearable>
              <el-option v-for="title in relationList" :key="title.value" :label="title.label" :value="title.value" />
            </el-select>
          </el-form-item>
          <el-form-item
          ...
          >
            <el-input v-model="contact.parentName" :disabled="contact.lock" placeholder="联系人姓名" />
          </el-form-item>
          ...
          >
            <el-input v-model="contact.telephone" type="text" placeholder="联系人电话" maxlength="11" @input="checkPhone(index)" />
          </el-form-item>

          <etm-font-icon v-if="index === 0" name="plus" @click.native="addContact" />
          <etm-font-icon v-else name="minus" @click.native="removeContact(index)" />
        </div>
      </el-form-item>
      ...
      <div class="layout-row">
        <div class="layout-col">
          <el-form-item label="QQ">
            <el-input v-model="childForm.qqNumber" placeholder="请输入QQ号" />
          </el-form-item>
        </div>
        <div class="layout-col">
          <el-form-item label="微信">
            <el-input v-model="childForm.wechatNumber" placeholder="请输入微信号" />
          </el-form-item>
        </div>
      </div>
      ...
      <div class="layout-row">
        <div class="layout-col">
          <el-form-item label="学员状态">
            <el-select v-model="childForm.statusId" clearable @change="forceUpdate">
              <el-option v-for="(status) of statusList" :key="status.value" :label="status.label" :value="status.value" />
            </el-select>
          </el-form-item>
        ...
        <div class="layout-col">
          <el-form-item label="重要程度">
            <el-select v-model="childForm.importanceAttrId" clearable @change="forceUpdate">
              <el-option
      ...
      <div class="layout-row">
        ...
        <div class="layout-col">
          <el-form-item label="标签">
            <el-select v-model="childForm.tagIdList" clearable multiple @change="forceUpdate">
              <el-option v-for="(tag) of tagIdList" :key="tag.value" :label="tag.label" :value="tag.value" />
            </el-select>
          </el-form-item>
        </div>
      </div>
      <el-form-item label="会员卡">
        <div v-for="(contact, index) of childForm.membershipCards" :key="index" class="contact" :class="{edit : childId}">
          <el-form-item>
            <el-select v-model="contact.cardTypeId" clearable placeholder="请选择会员卡类型">
              <el-option v-for="title in userCardType" :key="title.membershipCardTypeId" :label="title.membershipCardTypeName" :value="title.membershipCardTypeId" />
            </el-select>
          </el-form-item>
          ...
          >
            <el-input v-model="contact.cardNumber" placeholder="请输入会员卡号" />
          </el-form-item>

          <el-form-item>
            <el-select v-model="contact.id" clearable placeholder="请选择关联联系人">
              <template v-for="item in childForm.relationshipList">
                <el-option v-if="item.relationshipId && item.parentName && item.telephone" :key="item.cid" :label="`${item.parentName}(${relationMap[item.relationshipId].relationshipName}) ${item.telephone}`" :value="item.cid" />
              </template>
            </el-select>
          </el-form-item>

          <etm-font-icon v-if="index === 0" name="plus" @click.native="addContact2" />
          <etm-font-icon v-else name="minus" @click.native="removeContact2(index)" />
        </div>
      </el-form-item>
      ...
      <div class="layout-row">
        <div class="layout-col">
          <el-form-item label="跟进人员">
            <el-select v-model="childForm.followerId" clearable :disabled="edit" @change="forceUpdate">
              <el-option
        ...
        <div class="layout-col">
          <el-form-item label="市场人员">
            <el-select v-model="childForm.marketerId" clearable @change="forceUpdate">
              <el-option
                v-for="(employee) of employeeList"
                :key="employee.value"
                :label="employee.label"
      ...
      <el-form-item label="备注">
        <el-input v-model="childForm.remark" maxlength="200" show-word-limit type="textarea" :rows="4" placeholder="请输入备注内容" />
      </el-form-item>
    </etm-form>
  </div>
...
// import { getChildStateList } from '@/api/daycare/childManageSetting'
import { parseTime } from '@/utils'
// import { revokeBatch } from '@/api/guardcare/childManage/childManage'

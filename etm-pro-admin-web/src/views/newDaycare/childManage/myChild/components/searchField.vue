<template>
  <div class="searchField-wrap">
    ...
    <etm-form inline type="dialog">
      <el-form-item v-for="(form, index) of forms" :key="index" :label="form.label" class="el-form-content">
        <el-form-item>
          <el-select v-model="form.field" @change="fieldChange(form, index)">
            <el-option v-for="(field) of formsFields" :key="field.label" :label="field.label" :value="field.value" />
          </el-select>
        </el-form-item>

        <el-form-item>
          <el-select v-model="form.operationValue" @change="$forceUpdate()">
            <el-option v-for="(op) of form.operationList" :key="op.operation" :label="op.label" :value="op.operation" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="form.operationValue !== 'IS_NULL' && form.operationValue !== 'IS_NOT_NULL'">
          ...
          <template v-if="form.controlType === 'SELECT'">
            <el-select v-model="form.value" filterable>
              <el-option v-for="(option) of form.optionList" :key="option.value" :label="option.value" :value="option.key" />
            </el-select>
          </template>
          ...
          <template v-if="form.controlType === 'RADIO'">
            <el-select v-model="form.value" filterable>
              <el-option v-for="(option) of form.optionList" :key="option.key" :label="option.value" :value="option.key" />
            </el-select>
          ...
          <template v-if="form.controlType === 'CHECKBOX'">
            <el-select v-model="form.value" filterable>
              <el-option v-for="(option) of form.optionList" :key="option.value" :label="option.key" :value="option.value" multiple />
            </el-select>
          ...
          <template v-if="form.controlType === 'CASCADE_SELECT'">
            <el-cascader :options="form.optionList" :props="cascadeProps" :show-all-levels="false" @change="handleCascaderChange($event ,form)" />
          </template>

        </el-form-item>

        <etm-font-icon v-if="index === 0" name="plus" @click.native="addForms" />
        <etm-font-icon v-else name="minus" @click.native="removeForms(index)" />
      </el-form-item>
      <el-form-item style="margin-bottom: 16px;display: block">
        <div class="btn-box">

    ...
    >
      <etm-layout-split>
        <div>
          ...
          <etm-form ref="form" type="dialog" :model="form">
            <etm-title>基本信息</etm-title>
            <el-row>
              <el-col :span="12">
                <el-form-item label="幼儿姓名" prop="childName" :rules="formValidate.nullValueRule(true, '请输入幼儿姓名')">
                  <el-input v-model="form.childName" />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="性别" prop="gender" :rules="formValidate.nullValueRule(true, '请选择性别')">
            ...
            <el-form-item label="出生日期" prop="birthday" :rules="formValidate.nullValueRule(true, '请选择出生日期')">
              <el-date-picker v-model="form.birthday" value-format="yyyy-MM-dd" />
            </el-form-item>
            <el-form-item label="现住地" prop="currentAreaCode">
              <etm-address v-model="form.currentAreaCode" :area-id="form.currentAreaCode" />
            </el-form-item>
            <el-form-item label="详细地址" prop="currentAddress">
              <el-input v-model="form.currentAddress" placeholder="请输入详细地址" />
            </el-form-item>

            <etm-title>入园信息</etm-title>

            <el-form-item label="联系人" required>
              <div v-for="(item, index) in form.relationshipList" :key="index" class="contacts">
                <el-row>
                  <el-col :span="5">
                    <el-form-item :prop="`relationshipList.${index}.relationshipId`" :rules="formValidate.nullValueRule(true, '请选择称呼', ['change', 'blur'])">
                      <el-select v-model="item.relationshipId">
                        <el-option v-for="(i, k) in relationship" :key="k" :label="i.relationshipName" :value="i.relationshipId" />
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item :prop="`relationshipList.${index}.parentName`" :rules="formValidate.nullValueRule(true, '请输入联系人姓名')">
                      <el-input v-model="item.parentName" placeholder="输入联系人姓名" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item :prop="`relationshipList.${index}.telephone`" :rules="formValidate.nullValueRule(true, '请输入联系人号码')">
                      <el-input v-model="item.telephone" placeholder="输入手机号码" @input="checkoutPhone($event, index)" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item :prop="`relationshipList.${index}.attendanceCardNumber`">
                      <el-input v-model="item.attendanceCardNumber" placeholder="接送卡号" />
                    </el-form-item>
                  </el-col>
                  <el-col :span="1">
                    <etm-font-icon v-if="index === 0" style="margin-bottom: 16px;" name="plus" @click.native="addContact" />
                    <etm-font-icon v-else style="margin-bottom: 16px;" name="minus" @click.native="removeContact(index)" />
                  </el-col>
                </el-row>
              </div>
            </el-form-item>
            <el-form-item label="户口地">
              <etm-address v-model="form.householdRegisterAreaCode" :area-id="form.householdRegisterAreaCode" />
            </el-form-item>
            <el-form-item prop="enrollmentTime" :rules="formValidate.nullValueRule(true, '请选择入托日期')">
              <div slot="label">
                入托日期
              ...
              <el-date-picker v-model="form.enrollmentTime" value-format="yyyy-MM-dd" />
            </el-form-item>
            <el-form-item label="报读班级" required>
              <el-row>
                <el-col :span="12">
                  <el-form-item prop="gradeId" :rules="formValidate.nullValueRule(true, '请选择年级', ['blur', 'change'])">
                    <el-select v-model="form.gradeId" @change="gradeChange">
                      <el-option v-for="(item, index) in clazzLists" :key="index" :label="item.name" :value="item.id" />
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item prop="clazzId" :rules="formValidate.nullValueRule(true, '请选择班级', ['blur', 'change'])">
                    <el-select v-model="form.clazzId">
                      <template v-if="form.gradeId !== null">
                        <el-option v-for="(item, index) in clazzLists2" :key="index" :label="item.name" :value="item.id" />
                      </template>
                    </el-select>
                  </el-form-item>
                </el-col>
            ...
            <etm-title show-toggle open-text="展开更多" close-text="收起更多" @toggle="toggle">更多信息</etm-title>
            <el-form-item label="国籍/地区" prop="nationality">
              <el-select v-model="form.nationality">
                <el-option v-for="(item, index) in nationList" :key="index" :label="item.zh" :value="item.zh" />
              </el-select>
            </el-form-item>
            ...
            <div v-if="more">
              <el-form-item label="留守儿童" prop="isLeftBehindChildren">
                <el-radio-group v-model="form.isLeftBehindChildren">
                  <el-radio :label="true">是</el-radio>
                  <el-radio :label="false">否</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="血型" prop="bloodType" placeholder="请选择">
                <el-select v-model="form.bloodType">
                  <el-option label="A型" value="A" />
                  <el-option label="B型" value="B" />
                  <el-option label="AB型" value="AB" />
                  <el-option label="RH型阳性" value="RH_A" />
                  <el-option label="RH型阴性" value="RH_B" />
                  <el-option label="O型" value="O" />
                  <el-option label="其他" value="OTHER" />
                </el-select>
              </el-form-item>
              <el-form-item label="健康状态" prop="healthCondition">
                <el-select v-model="form.healthCondition" placeholder="请选择">
                  <el-option label="健康" value="HEALTH" />
                  <el-option label="一般" value="ORDINARY" />
                  <el-option label="患病" value="SICK" />
                </el-select>
              </el-form-item>
              <el-form-item label="曾患疾病" prop="onceSick">
                <el-select v-model="form.onceSick" placeholder="请选择">
                  <el-option v-for="(item, index) in sickList" :key="index" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="食物过敏" prop="foodAllergyInfo">
                <el-input v-model="form.foodAllergyInfo" placeholder="请输入过敏情况" />
              </el-form-item>
              <el-form-item label="药物过敏" prop="drugAllergyInfo">
                <el-input v-model="form.drugAllergyInfo" placeholder="请输入过敏情况" />
              </el-form-item>
              <el-form-item label="疫苗接种" prop="isCompleteVaccination">
                <el-radio-group v-model="form.isCompleteVaccination">
                  <el-radio :label="true">已完成</el-radio>
                  <el-radio :label="false">未完成</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="自己吃饭" prop="isEatByYourself">
              ...
              <el-form-item label="备注" prop="remarks">
                <el-input v-model="form.remarks" placeholder="输入备注内容" type="textarea" :rows="4" :maxlength="200" />
              </el-form-item>
            </div>
          </etm-form>
        </div>

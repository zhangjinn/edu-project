<template>
  <div class="sourceWrap">
    <div class="addSourceType">
      <el-button type="primary" @click="editSource(null, false, '')"><i class="el-icon-plus" /> 添加新类型</el-button>
    </div>

    <div class="sourceShadow">
      <div class="sourceScroll">
        <div v-for="(item, index) in dragData" :key="item.id" class="sourceBox">
          <etm-title border show-toggle open-text=" " close-text=" " @toggle="toggle(item, $event)">
            {{ item.text }}
            <template slot="tool">
              <div class="edit-class" @click="editSource(item.id, true,item.text)"><i class="iconfont iconedit-square1" /> 编辑类型</div>
              <div class="del-class" @click="deleteSource(item.id)"><i class="el-icon-minus" /> 删除</div>
            </template>
          </etm-title>
    ...
    <etm-pop
      ...
      @confirm="confirm"
    >
      <etm-form ref="form" type="dialog" :model="popConfig" :rules="formRules">
        <el-form-item label="来源类型" prop="name">
          <el-input v-model="popConfig.name" maxlength="30" />
        </el-form-item>
      </etm-form>
    </etm-pop>
  </div>

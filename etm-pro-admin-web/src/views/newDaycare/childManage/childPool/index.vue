<template>
  <div class="index-wrap">
    <etm-layout-split>
      <div slot="header">
        <etm-tabs v-model="currentTab" setting @setting="setting">
          <el-tab-pane label="公海" name="publicPool">
            <!--<etm-screen-tab v-if="searchItems.length" :category-list="searchItems" title="快捷筛选" @change="searchChange" />-->
            <search-field category="CHILD_POOL" @query="query" @change="fieldChange" />
          </el-tab-pane>
      ...
      <div slot="body">
        <template v-if="currentTab === 'publicPool'">
          ...
          >
            <el-table-column slot="left" label="学员姓名" fixed="left" width="100">
              <template v-slot="{row}">
                <etm-text type="primary" @click="handleRowClick(row)">{{ row.name }}</etm-text>
              </template>
            </el-table-column>
            <el-table-column slot="right" fixed="right" width="100" label="操作">
              <template v-slot="{row}">
                <etm-text type="primary" @click="handleFollow(row)">领取</etm-text>

...
import { queryTree, querySon_organization } from '@/api/newDaycare/areaSetHead'
...
export default {
  ...
  methods: {
    ...
    getSon() {
      querySon_organization({ organizationId: this.form.value }).then(res => {
        this.tableData = res.data
      })
    },
    handleChange() {
      querySon_organization({ organizationId: this.form.value[this.form.value.length - 1] }).then(res => {
        this.tableData = res.data
      })
    }
  }

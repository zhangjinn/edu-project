<template>
  <div class="courseDetail-wrap">
    <etm-field-label type="left" label="课程名称">{{ course.courseName }}</etm-field-label>
    <etm-field-label type="left" label="课程类型">{{ course.courseTypeName }}</etm-field-label>
    <etm-field-label type="left" label="上课班级">{{ course.clazzName }}</etm-field-label>
    ...
    <etm-field-label type="left" label="上课教室">{{ course.classsroomName }}</etm-field-label>
    <etm-field-label type="left" label="日期">{{ course.courseDate }}</etm-field-label>
    <!--    <etm-field-label type="left" label="上课时段">{{ course.courseTimePeriod }}</etm-field-label>-->
    <etm-field-label type="left" label="学员/容量">{{
      course.expChildList.length + course.formalChildList.length
    }}/{{ course.totalFormalChildNum + course.totalExpChildNum }}
    </etm-field-label>

    <etm-title show-toggle open-text="展开" close-text="收起" @toggle="showExp = !showExp">
      <span>学员/试听学位 {{ course.expChildList.length }}/{{ course.totalExpChildNum }}</span>
    </etm-title>
    <el-collapse-transition>
      <div v-show="showExp">
        <etm-table :columns="columns" :data="course.expChildList">
    ...
    <etm-title show-toggle open-text="展开" close-text="收起" @toggle="showFormal = !showFormal">
      <span>学员/正式学位 {{ course.formalChildList.length }}/{{ course.totalFormalChildNum }}</span>
    </etm-title>
    <el-collapse-transition>
      <div v-show="showFormal">
        <etm-table :columns="columns" :data="course.formalChildList">
          <!--<el-table-column label="操作">-->
          <!--  <template v-slot="{row}">-->
 ...
 */
import { queryCaCourseDetailById } from '@/api/newDaycare/nurseSchedule'
...
export default {
  ...
  methods: {
    init() {
      ...
      queryCaCourseDetailById({ caScourseManagementId: this.courseManagementId }).then(res => {
        this.course = res.data
        this.course.courseDate = parseTime(this.course.courseDate, '{y}-{m}-{d} 周{a}')

<template>
  <div class="formItem-wrap">
    <etm-form :model="formData" :rules="rule" type="dialog">
      ...
      <el-form-item v-if="formData.type === 'ATTENDANCE'" label="结算规则" prop="attendanceRuleList">
        <div class="attendanceBox">
          <div v-for="(list, index) in formData.attendanceRuleList" :key="index" class="ruleList">
            <template v-if="index === 0">
              ...
              <el-select v-model="list.comparisonOperator" class="bigSelect" @change="changeComparison">
                <el-option
            ...
            <el-input-number v-model="list.days" :min="0" placeholder="请输入天数" @change="changeDays($event, index)" />
            <span>天时，</span>
            <el-select v-model="list.settlementOperation" @change="changeSettlement($event, index)">
              <el-option
            ...
            <i v-if="index === 0" class="iconfont iconplus" @click="add" />
            <i v-else class="iconfont iconminus" @click="reduce(index)" />
          </div>
        </div>
      </el-form-item>
      <el-form-item v-else-if="formData.type === 'ABSENCE'" label="结算规则" prop="absenceRule">
        <div class="absenceBox">
          <el-input-number v-model="formData.absenceRule.price" :min="0" placeholder="请输入费用单价" /> 乘 缺勤天数
        </div>
      </el-form-item>
    </etm-form>
  </div>
...
export default {
  ...
  data() {
    return {
      materialPriceList: [],
      rule: {
        ruleName: [{ required: true, trigger: 'blur', validator: (rule, value, callback) => {
          if (!value) {
        ...
        attendanceRuleList: [{ required: true, trigger: 'blur', validator: (rule, value, callback) => {
          let flag = true
          value.forEach(item => {
        ...
        absenceRule: [{ required: true, trigger: 'blur', validator: (rule, value, callback) => {
          if (!value.price) {

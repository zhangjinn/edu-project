  ...
  <div class="table">
    ...
    >
      ...
      <el-table-column slot="right" label="操作" fixed="right" width="120">
        <template slot-scope="scoped">
          <etm-text type="primary" @click="enterAgain(scoped.row)">
            重新入园
          </etm-text>
        </template>
      </el-table-column>
...
import { updateChildAdmission, queryChildOutList } from '@/api/newDaycare/childManageGuard'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      queryChildOutList(param).then(res => {

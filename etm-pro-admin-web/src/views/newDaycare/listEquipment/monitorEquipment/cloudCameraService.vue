  ...
  <div class="cloudCameraService-wrap">
    ...
    >
      ...
      <el-table-column label="联系人姓名">
        <template v-slot="scoped">
          <etm-text type="primary" @click.stop="handleDetail(scoped.row)">{{ scoped.row.personName }}</etm-text>
        </template>
      </el-table-column>
      ...
      <el-table-column slot="right" label="操作">
        <template v-slot="scoped">
          ...
          <etm-text v-if="scoped.row.openState === 'ACTIVATED'" type="primary" @click.stop="handleContinue(scoped.row)">续费</etm-text>
          <etm-text v-if="scoped.row.openState === 'ACTIVATED'" type="danger" @click.stop="handleCloseService(scoped.row)">关闭服务</etm-text>
          <etm-text v-if="scoped.row.openState === 'NONACTIVATED'" type="primary" @click.stop="handleOpenService(scoped.row)">线下开通</etm-text>
        </template>
      </el-table-column>
    </etm-table-page>
    ...
    >
      <etm-layout-split line>
        <etm-title :title="drawerTitles[type]" border>
          <template slot="tool">
            <el-button size="mini" type="primary" @click="popConfirm">提交</el-button>
          </template>
        </etm-title>
        <div class="drawerBody">
          <edit ref="addForm" :type="type" :child-id="childId" :parent-id="parentId" :parent-phone="parentPhone" />
        </div>
      </etm-layout-split>
    </etm-drawer>

    <!--详情/编辑抽屉-->
    <etm-drawer v-if="showCloudDrawer" :visible.sync="showCloudDrawer">
      <etm-layout-split line>
        <etm-title :title="drawerTitles[type]" border>
          <template v-if="type==='edit'" slot="tool">
            <el-button plain size="mini" @click="type = '';showCloudDrawer = false">取消</el-button>
        ...
        <div class="drawerBody">
          <component :is="type" ref="editFrom" :child-id="childId" :parent-id="parentId" :parent-phone="parentPhone" :type="type" />
        </div>
      </etm-layout-split>
    </etm-drawer>
  </div>

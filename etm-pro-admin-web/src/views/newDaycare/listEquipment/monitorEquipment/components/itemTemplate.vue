        ...
        <div class="flex-wrap">
          ...
          <div class="flex-item">
            ...
            <el-time-select
              ...
              @change="handleTimeChange(slotProps.index, 0)"
            />
            <el-switch v-model="dragData[slotProps.index][0].enable" @change="handleTimeEnable(slotProps.index, 0)" />
          </div>
        </div>
        <div class="flex-wrap">
          ...
          <div class="flex-item">
            ...
            <el-time-select
              ...
              @change="handleTimeChange(slotProps.index, 1)"
            />
            <el-switch v-model="dragData[slotProps.index][1].enable" @change="handleTimeEnable(slotProps.index, 1)" />
          </div>
        </div>
      </template>
      <template #handle="slotProps">
        <el-select v-model="dragData[slotProps.index].typeId" class="handle-item" placeholder="请选择套餐时长" @change="handleChange(slotProps.index)">
          <el-option
        ...
        <el-input v-model="dragData[slotProps.index].sellMoney" class="handle-item" placeholder="请输入套餐金额" @blur="handleInputBlur(slotProps.index)" @focus="handleInputFocus(slotProps.index)" />
        <el-input v-model="dragData[slotProps.index].discountMoney" class="handle-item" placeholder="请输入优惠金额" @blur="handleInputBlur(slotProps.index)" @focus="handleInputFocus(slotProps.index)" />

  ...
  <div class="cameraManagement-wrap">
    ...
    >
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          ...
          <el-button v-if="hasAuthOfBool('newdaycare.monitorCare.empower')" plain @click="gotoClassroomAuthorize">托育班级授权</el-button>
          <el-button v-if="hasAuthOfBool('newdaycare.monitorGuard.empower')" plain @click="gotoChildClassroomAuthorize">幼儿园班级授权</el-button>
          <el-button plain @click="gotoWorderAuthorize">教职工授权</el-button>
        </div>
      </etm-table-tool-bar>
      <el-table-column label="名称">
        <template v-slot="scoped">
          <etm-text type="primary" @click.stop="handleDetail(scoped.row)">{{ scoped.row.cameraName }}</etm-text>
        </template>
      </el-table-column>
      ...
      <el-table-column slot="right" label="操作">
        <template v-slot="scoped">
          <etm-text type="primary" @click.stop="handleEdit(scoped.row)">编辑</etm-text>
    ...
    >
      <div class="popContent">
        <edit ref="addForm" :type="type" />
      </div>
    </etm-pop>

    <!--详情/编辑抽屉-->
    <etm-drawer v-if="showDrawer" :visible.sync="showDrawer">
      <etm-layout-split line>
        <etm-title :title="drawerTitles[type]" border>
          <template v-if="type==='edit'" slot="tool">
            <el-button plain size="mini" @click="type = '';showDrawer = false">取消</el-button>
            <el-button size="mini" type="primary" @click="popConfirm">保存</el-button>
          </template>
          <template v-if="type==='detail'" slot="tool">
            <el-button plain size="mini" @click="handleEdit">编辑</el-button>
          </template>
        </etm-title>
        <div class="drawerBody">
          <component :is="type" ref="editFrom" :camera-id="cameraId" :type="type" />
        </div>
      </etm-layout-split>
    </etm-drawer>
  </div>
...
export default {
  ...
  methods: {
    ...
    gotoClassroomAuthorize() {
      this.$router.push({ path: '/newdaycare_videoEquipment/classroomAuthorize' })
    },
    gotoChildClassroomAuthorize() {
      this.$router.push({ path: '/newdaycare_videoEquipment/childClassroomAuthorize' })
    },
    gotoWorderAuthorize() {
      this.$router.push({ path: '/newdaycare_videoEquipment/workerAuthorize' })
    },
    popConfirm() {
      let ref, fn, msg
      if (this.type === 'add') {

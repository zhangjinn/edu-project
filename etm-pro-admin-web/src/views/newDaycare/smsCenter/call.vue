    ...
    <etm-main-body>
      <etm-layout-split>
        <template slot="header">
          <etm-tabs v-model="activeName" @tab-click="tabChange">
            <el-button slot="tool" type="text" @click="popOpen">余额查询</el-button>
            <el-tab-pane label="人工通话记录" name="callRecords">
              <etm-tool-bar v-show="true" v-model="formData" @search="handleSearch" @reset="reset">
                <el-form-item label="通话时长:">
                  <el-input v-model="searchCondition1.startCallTime" placeholder="请输入时长" />
                  -
                  <el-input v-model="searchCondition1.endCallTime" placeholder="请输入时长" />
                  秒
                </el-form-item>
              </etm-tool-bar>
            </el-tab-pane>
        ...
        <template v-if="activeName==='callRecords'" slot="body">
          <etm-table-page
            :data="tableData"
            :columns="columns"
            border
          ...
          >
            ...
            <etm-table-tool-bar slot="tool">
              <div slot="right">
                <el-button type="primary" @click="smsCall">打电话
                </el-button>
                <el-button type="primary" @click="shenMsg = true">发短信
                </el-button>
              </div>
            </etm-table-tool-bar>
            <el-table-column slot="right" label="操作" width="120">
              <template v-slot="scoped">
                <template v-if="scoped.row.recordingUrl!=='0'">
                  <etm-text type="primary" @click="handlePlay(scoped.row)">播放
                  </etm-text>
                  <etm-text type="primary" @click="handleDownload(scoped.row)">下载
                  </etm-text>
                </template>
              </template>
            </el-table-column>
        ...
        <template v-else slot="body">
          ...
          >
            <el-table-column slot="right" label="账号状态" width="120">
              <template v-slot="scoped">
                <etm-status v-if="scoped.row.enable" type="primary">{{
                  scoped.row.enumEnable
                }}
                </etm-status>
                <etm-status v-else type="danger">{{ scoped.row.enumEnable }}</etm-status>
              </template>
            </el-table-column>
            <el-table-column slot="right" label="显示号码" width="220">
              <template v-slot="scoped">
                <span>{{ scoped.row.showPhone }}</span>
              </template>
            </el-table-column>
            <el-table-column slot="right" label="呼叫功能" width="120">
              <template v-slot="scoped">
                {{ scoped.row.openType }}
              </template>
            </el-table-column>
            <el-table-column slot="right" label="操作">
              <template v-slot="scoped">
                <etm-text v-if="!scoped.row.open" @click="handleOpen(scoped.row)">开通
                </etm-text>
                <etm-text v-if="scoped.row.open==='--'||scoped.row.open===true" @click="handleEdit(scoped.row)">编辑
                </etm-text>
                <etm-text v-if="scoped.row.open==='--'||scoped.row.open===true" @click="handleClose(scoped.row)">关闭
                </etm-text>
              </template>
            </el-table-column>
          </etm-table-page>
      ...
      >
        <shen-msg v-if="shenMsg" ref="childPopForm" />
      </etm-pop>
      ...
      >
        <div class="popContent2">
          <calculation v-if="popLogVisible" @confirm="callConfirm" />
        </div>
      </el-dialog>
      <!--编辑-->
      <etm-pop
      ...
      >
        <etm-form v-if="popVisibleEdit" ref="form" :model="popForm" type="dialog">
          <el-form-item
            label="显示号码"
            prop="popPhoneNum"

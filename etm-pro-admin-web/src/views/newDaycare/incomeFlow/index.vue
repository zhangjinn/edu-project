    ...
    <etm-main-body>
      <etm-layout-split>
        ...
        <div slot="body">
          <etm-table-page v-fixedHead :columns="columns" :data="tableData" selection @currentChange="currentChange">
            ...
            <el-table-column prop="type" label="收入类型" fixed="left" width="120" />
            <el-table-column prop="incomeExpenditureTypeName" label="收入科目" fixed="left" width="120" />
            <el-table-column prop="reconciliationStatus" label="对账状态" fixed="left" width="120" />
            <el-table-column prop="amountOfPayout" label="收入金额(元)" fixed="left" width="120" align="right">
              <template slot-scope="scoped">
                <span>{{ scoped.row.amountOfPayout.toFixed(2) }}</span>
              </template>
            </el-table-column>
            <el-table-column label="关联单号" fixed="left" width="150">
              <template slot-scope="scoped">
      ...
      >
        <etm-layout-split>
          ...
          <etm-form ref="sideFormRef" type="dialog" :model="sideForm">
            <el-form-item prop="incomeTypeSide" label="收入科目" :rules="[{required: true, message: '请选择支出科目'}]">
              <el-select v-model="sideForm.incomeTypeSide" placeholder="请选择收入科目">
                <el-option
            ...
            >
              <el-input v-model="sideForm.reconMoneySide" placeholder="请输入收入金额" />
            </el-form-item>
      ...
      >
        <etm-form ref="popFormRef" type="dialog" :model="popForm">
          <el-form-item label="收入科目" prop="incomeTypePop" :rules="[{required: true, message: '请选择收入类型'}]">
            <el-select v-model="popForm.incomeTypePop" placeholder="请选择收入科目">
              <el-option
          ...
          <el-form-item
            ...
            :rules="[formValidate.nullValueRule(true, '请输入收入金额'), formValidate.isNumberRule()]"
          >
            <el-input v-model="popForm.incomeMoneyPop" placeholder="请输入收入金额" />
          </el-form-item>
          ...
          <el-form-item label="到账日期" prop="intoDatePop" :rules="[{required: true, message: '请选择到账日期'}]">
            <el-date-picker
      ...
      >
        <etm-layout-split>
          <etm-title :show-line="true" border>详情</etm-title>
          <etm-field-label label="订单编号" type="left">{{ orderDetail.orderNo }}</etm-field-label>
          <etm-field-label label="联系人姓名" type="left">{{ orderDetail.contactName }}</etm-field-label>
          <etm-field-label label="联系人电话" type="left">{{ orderDetail.contactPhone }}</etm-field-label>
          <etm-field-label label="学员姓名" type="left">{{ orderDetail.childName }}</etm-field-label>
          <etm-field-label label="购买时长" type="left">{{ orderDetail.buyTime }}</etm-field-label>
          <etm-field-label label="支付金额" type="left">{{ orderDetail.payAmount }}</etm-field-label>
          <etm-field-label label="支付日期" type="left">{{ orderDetail.payDate }}</etm-field-label>
          <etm-field-label label="到期日期" type="left">{{ orderDetail.expireDate }}</etm-field-label>
        </etm-layout-split>
      </etm-drawer>
      <etm-drawer
      ...
      >
        <etm-layout-split>
          <order-drawer v-if="guardCareOrder==='order'&&showDrawer" :order-data="orderData" />
          <video-drawer v-else-if="guardCareOrder==='video'&&showDrawer" :video-data="videoData" />
        </etm-layout-split>
      </etm-drawer>
    </etm-main-body>
  </div>

  ...
  <div class="addClass-wrap">
    ...
    <etm-layout-split>
      <etm-title :show-line="false" border size="big">
        ...
        <template slot="tool">
          <template v-if="is_edit">
            ...
            <el-button type="primary" size="mini" @click="toggleSave">保存</el-button>
          </template>
          <template v-else>
            <el-button v-has="{authId : 'daycare.clazzTeach.update'}" plain size="mini" @click="handleEdit">编辑</el-button>
          </template>
        </template>
      </etm-title>
      ...
      <el-row>
        <el-col :span="14">
          <div v-if="!is_edit">
            <etm-field-label label="班级名称" type="left">{{ form.teClazzName }}</etm-field-label>
            <etm-field-label label="班主任" type="left">{{ form.headTeacherName }}</etm-field-label>

            <!--            <etm-field-label label="满班人数" type="left">{{ form.fullNumber }}</etm-field-label>-->
            <etm-field-label label="学员数量" type="left">{{ form.childNumber }}</etm-field-label>
            <!--            <etm-field-label label="剩余学位" type="left">{{ form.lastNumber }}</etm-field-label>-->
            <etm-field-label label="备注" type="left">{{ form.remark }}</etm-field-label>
          </div>
          <etm-form v-else ref="addClass" type="dialog" :model="form">
            <el-form-item
              label="班级名称"
              prop="teClazzName"
              :required="true"
            ...
            >
              <el-input v-model="form.teClazzName" placeholder="请输入班级姓名" value="" />
            </el-form-item>
            ...
            <el-form-item label="是否开启">
              <div><el-switch v-model="form.enable" @change="switchChange" /></div>
            </el-form-item>
          </etm-form>
        </el-col>
      </el-row>
      ...
      >
        <etm-table-tool-bar slot="tool">
          ...
          <div slot="right">
            ...
            <el-button
              ...
              size="small"
              type="primary"
              @click="exchangeTeachClasses(multipleStudent)"
            >批量调班
            </el-button>
          </div>
        ...
        <el-table-column slot="left" label="学员姓名" width="120" align="center" header-align="center">
          <template slot-scope="scoped">
            {{ scoped.row.childName }}
          </template>
        </el-table-column>
        <el-table-column slot="left" label="性别" width="120" align="center" header-align="center">
          <template slot-scope="scoped">
            {{ scoped.row.genderEnum }}
          </template>
        </el-table-column>
        <el-table-column slot="right" label="操作" width="120" align="center" header-align="center">
          <template slot-scope="scoped">
            <etm-text type="primary" @click="exchangeTeachClasses([scoped.row])">调班</etm-text>
            <etm-text type="danger" @click="getOutTeachClasses(scoped.row)">退班</etm-text>
          </template>
        </el-table-column>
      </etm-table>
    ...
    >
      <el-form label-width="80px">
        <el-form-item label="选择学员">
          <el-select v-model="joinStudent" style="width: 100%;" filterable multiple placeholder="请选择">
            <el-option
      ...
      <!--        <etm-table-tool-bar slot="tool">-->
      <!--          已选择-->
      <!--          <etm-text type="primary">2</etm-text>-->
      <!--        </etm-table-tool-bar>-->

      <!--        <el-table-column slot="left" label="学员姓名" width="120" />-->
      <!--        <el-table-column slot="left" label="年龄" width="120" />-->
      <!--        <el-table-column slot="right" label="联系人姓名" width="120" />-->
      <!--        <el-table-column slot="right" label="联系人电话" width="120" />-->
      <!--        <el-table-column slot="right" label="性别" width="120" />-->
      <!--        <el-table-column slot="right" label="跟进阶段" width="120" />-->
      <!--        <el-table-column slot="right" label="学员状态" width="120" />-->
      <!--      </etm-table-page>-->
    </etm-pop>
    ...
    >
      <etm-form type="dialog">
        ...
        <el-form-item label="选择班级">
          <el-select v-model="choiceClass" filterable placeholder="请选择班级">
            <el-option
...
import {
  queryTeachClassChildren,
  queryTeachClassDetail,
  updateTeachClassDetail,
  getOutTeachClass,
  queryTeachClasses,
  exchangeTeachClass,
  queryAllChildren,
  createJoinTeachChildren
...
export default {
  ...
  methods: {
    ...
    init() {
      queryTeachClassDetail({ teClazzId: this.classid }).then(res => {
        this.detailData = res.data
        this.form = {
          headTeacherId: res.data.headTeacherId,
          fullNumber: res.data.fullNumber,
          remark: res.data.remark,
          teClazzName: res.data.teClazzName,
    ...
    exchangeTeachClasses(choiceData) {
      ...
      queryTeachClasses({ clazzName: null }).then(res => {
    ...
    getChildren(search = null) {
      queryTeachClassChildren({ clazzId: this.classid, childName: search }).then(res => {
    ...
    toggleSave() {
      ...
      this.$refs.addClass.$children[0].validate(valid => {
        if (valid) {
          ...
          updateTeachClassDetail({ ...queryParams, teClazzId: this.classid }).then(res => {
            this.isEdit = false
            this.$emit('update', {})
            this.init()
            this.$message({
    ...
    newStudentConfirm() {
      ...
      createJoinTeachChildren({ clazzId: this.classid, childIdList: this.joinStudent }).then(res => {
        this.isShowNewStudent = false
    ...
    getOutTeachClasses(row) {
      this.$etmTip.init({
        ...
        cancelBtn: '取消',
        confirm: () => {
          getOutTeachClass({ childId: row.childId, clazzId: this.classid }).then(res => {
            this.$message({
              type: 'success',
              message: '退班成功'
            })
    ...
    /* 给学员调班*/
    // exchangeTeachClassMethod(ids, targetClassid) {
    //   exchangeTeachClass(ids, 1, targetClassid).then(res => {
    //
    //   })
    // },
    switchChange(event) {

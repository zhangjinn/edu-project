  ...
  <div class="education-wrap">
    ...
    <etm-table-page :columns="columns" :data="tableData" @currentChange="currentChange">
      ...
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button v-has="{authId : 'daycare.clazzCare.create'}" type="primary" @click="ClassOpen"><i class="el-icon-plus" />&nbsp;&nbsp;添加班级</el-button>
        </div>
      </etm-table-tool-bar>
      ...
      <el-table-column slot="right" label="操作" width="120">
        <template slot-scope="scoped">
          <etm-text v-has="{authId : 'daycare.clazzCare.update'}" type="primary" @click="classDetailOpen(scoped.row.clazzName, scoped.row.clazzId)">编辑</etm-text>
    ...
    >
      <etm-form ref="addClass" type="dialog" :model="form">
        <!--        <el-form-item label="所属园区">-->
        <!--          天河-->
        <!--        </el-form-item>-->
        <el-form-item label="班级名称" prop="name" :required="true" :rules="formValidate.nullValueRule(true, '请输入班级姓名')">
          <el-input v-model="form.name" placeholder="请输入班级姓名" value="" />
        </el-form-item>

    ...
    <etm-layout-split>
      <div slot="header">
        ...
        <etm-tabs ref="tabs" v-model="activeName">
          ...
          <!--          </el-tab-pane>-->
          <el-tab-pane v-if="hasAuth('daycare.cost.queryBatch', 'tabs', 'activeName')" label="早教费用" name="cost">
            <etm-tool-bar v-model="cost" @search="priceSearch" @reset="reset" />
          </el-tab-pane>
        </etm-tabs>
      </div>
      <div slot="body">
        <!--        <article-list v-if="activeName === 'article'" :item-type="itemSearch" />-->
        <cost-list v-if="activeName === 'cost'" :price-type="costSearch" />
      </div>
    </etm-layout-split>
  </div>

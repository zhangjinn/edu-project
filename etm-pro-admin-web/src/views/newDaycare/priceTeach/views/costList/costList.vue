  ...
  <div class="cost-wrap">
    <etm-table :columns="columns" :data="tableData" border>
      <etm-table-tool-bar slot="tool">
        <el-button slot="right" v-has="{authId: 'daycare.cost.create'}" type="primary" @click="addArticle"><i class="el-icon-plus" />&nbsp;&nbsp;添加费用</el-button>
      </etm-table-tool-bar>
      <el-table-column slot="left" fixed="left" label="费用名称" width="160">
        <template slot-scope="scoped">
          <etm-text type="primary" @click.stop="articleDetail(scoped.row)">{{ scoped.row.priceName }}</etm-text>
        </template>
      </el-table-column>
      <el-table-column slot="right" fixed="right" label="操作" width="120">
        <template v-slot="row">
          <etm-text v-has="{authId: 'daycare.cost.update'}" type="primary" @click="edit(row.row, true)">编辑</etm-text>
          <etm-text v-has="{authId: 'daycare.cost.remove'}" type="danger" @click="remove(row.row)">删除</etm-text>
        </template>
      </el-table-column>
    </etm-table>
    <!--添加弹框-->
      ...
      <etm-layout-split line>
        <etm-title :title="drawerTitle" border size="big">
          <template v-if="drawerVisible && drawerType==='edit'" slot="tool">
            <el-button plain size="mini" @click="cancelArticleDetail">取消</el-button>
          ...
          <template v-if="drawerVisible && drawerType==='detail'" slot="tool">
            <el-button v-has="{authId: 'daycare.cost.update'}" plain size="mini" @click="edit">编辑</el-button>
          </template>
        </etm-title>
...
export default {
  ...
  data() {
    return {
      ...
      popType: 'complex',
      columns: [
        { label: '单价', prop: 'price', align: 'right' },

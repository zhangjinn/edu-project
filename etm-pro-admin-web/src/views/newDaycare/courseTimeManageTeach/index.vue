    ...
    <etm-main-body>
      <etm-layout-split>
        <div slot="header">
          ...
          >
            <el-tab-pane v-if="hasAuth('daycare.scheduleCourseTeachWeek.queryBatch', 'tabs', 'activeName')" label="早教排课表(周)" name="weekView">
              <etm-tool-bar v-model="weekFormData" @reset="reset" @search="handleSearch($event,'weekView')" />
            </el-tab-pane>
            <el-tab-pane v-if="hasAuth('daycare.scheduleCourseTeachDay.queryBatch', 'tabs', 'activeName')" label="早教排课表(日)" name="dayView">
              <etm-tool-bar v-model="dayFormData" @reset="reset" @search="handleSearch($event,'dayView')" />
            </el-tab-pane>
          </etm-tabs>
        </div>
        <div slot="body">
            ...
            <div style="padding-bottom: 16px;">
              <div class="tool-bar">
                <div class="right">
                  ...
                  <template>
                    ...
                    />
                    <etm-day-change-tag v-model="currentDayDate" @change="handleDayChange" />
                    <div class="btn">
                      <el-button v-has="{authId: 'daycare.scheduleCourseTeach.createBatch'}" type="primary" @click="batchScheduling">批量排课</el-button>
                      <el-button v-has="{authId: 'daycare.scheduleCourseTeachDay.copy'}" type="primary" @click="handleCopyDayCourse">复制日课表</el-button>
                    </div>
                  </template>
                </div>
              </div>
              <el-table :data="tableData3" border>
                <el-table-column
                  ...
                  v-bind="{...col}"
                >
                  <template v-if="index === 0" v-slot="{row}">
                    <span>{{ row.classroom.roomName }}</span>
                  </template>
                  <template v-else v-slot="{row}">

                    <template v-for="(item,idx) of row.data">
                      <div v-if="item.timePeriodName === col.label " :key="idx" class="course-item">
                        <div
                        ...
                        >
                          ...
                          >
                            <div>
                              <div style="margin-bottom: 8px;" :style="{color: course.colorCode}">{{
                                course.courseName
                              }}
                              </div>
                              ...
                              <div class="course-item">
                                <etm-field-label type="left" label="上课老师"> {{ course.teacherName }}</etm-field-label>
                              </div>
                              <div class="course-item">
                          ...
                          <div class="popTip">
                            <span v-has="{authId: 'daycare.scheduleCourseTeach.query'}" type="primary" @click="doDetail(course)">详情</span>
                            <span v-has="{authId: 'daycare.scheduleCourseTeach.copy'}" type="primary" @click="doCopy(course)">复制</span>
                            <span v-has="{authId: 'daycare.scheduleCourseTeach.remove'}" type="primary" @click="doRemove(course)">删除</span>
                          </div>
                        </div>
                      </div>
                    </template>
                    <div class="btn" style="margin-top: 8px;">
                      <etm-text v-has="{authId:'daycare.scheduleCourseTeach.create'}" @click="handleAddCourse(row)"><i class="el-icon-plus" /></etm-text>
                    </div>
                  </template>
                </el-table-column>
              </el-table>
          ...
          <template v-if="activeName === 'weekView'">
            ...
            >
              ...
              <template slot="toolRight">
                <div class="btn">
                  <el-button v-has="{authId: 'daycare.scheduleCourseTeach.createBatch'}" type="primary" @click="batchScheduling">批量排课</el-button>
                  <el-button v-has="{authId: 'daycare.scheduleCourseTeachWeek.copy'}" type="primary" @click="copyDayCourse">复制周课表</el-button>
                </div>
              </template>

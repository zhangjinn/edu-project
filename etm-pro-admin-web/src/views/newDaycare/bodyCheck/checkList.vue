    ...
    <etm-table-page :columns="columns" :data="tableData" border @currentChange="currentChange">
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          ...
          <el-button v-has="{authId: 'daycare.bodyCheck.query'}" plain @click.stop="exportExcel">导出Excel
          </el-button>
        </div>
      </etm-table-tool-bar>
      <el-table-column slot="right" fixed="right" label="操作" width="120">
        <template v-slot="row">
          <etm-text v-has="{authId: 'daycare.bodyCheck.excelExport'}" type="primary" @click="handleDetail(row.row)">
            查看详情
          </etm-text>
        </template>
      </el-table-column>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: '待检人数', prop: 'pendingNumber' },
        { label: '实检人数', prop: 'actualInspectionNumber' },
        { label: '体温异常', prop: 'abnormalBodyTemperatureNumber' },
        { label: '体表异常', prop: 'bodySurfaceAbnormalitiesNumber' },
        { label: '检查时间', prop: 'ovTime', width: 180 }
      ],
      tableData: {},
      treasureBook: {},
      showBook: false,

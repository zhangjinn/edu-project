    ...
    <etm-table-page :columns="columns" :data="tableData" border @currentChange="currentChange">
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          ...
          <el-button v-has="{authId: 'daycare.unhealthy.excelExport'}" plain @click.stop="exportExcel">
            导出Excel
          </el-button>
        </div>
      </etm-table-tool-bar>
      <el-table-column slot="right" fixed="right" label="操作" width="120">
        <template v-slot="row">
          <etm-text v-if="row.row.abnormalConfirmationtTime === 0" type="primary" @click="handleEdit(row.row)">异常确认
          </etm-text>
        </template>
      </el-table-column>
    </etm-table-page>
    ...
    >
      <etm-form ref="form" :model="form" type="dialog">
        ...
        <el-form-item
          ...
          prop="illness"
        >
          <etm-screen-tab v-model="form.illness" :options="illnessList" multiple />
        </el-form-item>
      </etm-form>
    </etm-pop>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { label: '体温异常', prop: 'bodyTemperature' },
        { label: '常见病状', prop: 'checkSettingsNames' },
        { label: '检查日期', prop: 'checkTime' },
        { label: '确认时间', prop: 'abnormalTime', width: 180 }
      ],
      tableData: {},
      treasureBook: {},
      showBook: false

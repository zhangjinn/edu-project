  ...
  <div class="h2Manage-wrap">
    <etm-main-body>
      <etm-layout-split :body-styles="activeName === '活动管理'? {'background-color': 'transparent','padding': '0'} : {}">
        <div slot="header">
          <etm-tabs v-model="activeName" :show-toggle="false" @tab-click="handleTabChange">
            <el-tab-pane label="活动管理" name="活动管理" />
            <el-tab-pane label="中奖名单" name="中奖名单" />
            <el-tab-pane label="表单数据" name="表单数据" />
          </etm-tabs>
        </div>
        <div slot="body">
          <div v-if="activeName === '活动管理'" style="overflow: hidden;">
            <div class="detail">
              <img :src="item.cover" alt="活动封面">
              <div class="info">
                <p><strong>{{ item.name }}</strong></p>
                <p :class="'status-' + item.status">{{ getStatus(item.status ) }}</p>
            ...
            <el-row :gutter="16">
              <el-col :span="6">
                <div class="analysis-col">
                  <p>
                    累计访问次数
                    <el-tooltip class="comTooltip" effect="dark" placement="top">
                      <div slot="content">打开活动链接的次数，同一<br>访客多次访问次数累计。</div>
                      <!--                      <i class="iconfont iconquestion-circle" />-->
                      <i class="el-icon-question" />
                    </el-tooltip>
                  </p>
                  <p class="count"><strong>{{ manageData.totalPv }}</strong></p>
                </div>
              </el-col>
              <el-col :span="6">
                <div class="analysis-col">
                  <p>
                    累计访问人数
                    <el-tooltip class="comTooltip" effect="dark" placement="top">
                      <div slot="content">打开活动链接的访客数，同<br>一访客多次访问只计算一<br>次。</div>
                      <!--                      <i class="iconfont iconquestion-circle" />-->
                      <i class="el-icon-question" />
                    </el-tooltip>
                  </p>
                  <p class="count"><strong>{{ manageData.totalUv }}</strong></p>
                </div>
              </el-col>
              <el-col :span="6">
                <div class="analysis-col">
                  <p>
                    抽奖参与人次
                    <el-tooltip class="comTooltip" effect="dark" placement="top">
                      <div slot="content">抽奖的累计参与次数，同一<br>用户多次参与，抽奖人次不<br>去重。</div>
                      <!--                      <i class="iconfont iconquestion-circle" />-->
                      <i class="el-icon-question" />
                    </el-tooltip>
                  </p>
                  <p class="count"><strong>{{ manageData.participantsNum }}</strong></p>
                </div>
              </el-col>
              <el-col :span="6">
                <div class="analysis-col">
                  <p>
                    发放奖品总数
                    <el-tooltip class="comTooltip" effect="dark" placement="top">
                      <div slot="content">活动累计发出的奖品总数。</div>
                      <!--                      <i class="iconfont iconquestion-circle" />-->
                      <i class="el-icon-question" />
                    </el-tooltip>
                  </p>
                  <p class="count"><strong>{{ manageData.sendPrizes }}</strong></p>
                </div>
              </el-col>
            </el-row>
          </div>
          ...
          >
            <etm-table-tool-bar v-if="activeName === '中奖名单'" slot="tool">
              <el-button slot="right" plain @click.stop="handleExportExcel">导出Excel</el-button>
            </etm-table-tool-bar>
            <template v-if="activeName === '中奖名单'">
              <el-table-column label="中奖用户" prop="nickname" />
              <el-table-column label="中奖码" prop="winCode" />
              <el-table-column label="电子兑奖码" prop="eCode" />
              <el-table-column label="奖项等级" prop="prizeLevelName" />
              <el-table-column label="奖品" prop="prizeName" />
              <el-table-column label="红包额度" prop="redPackageAmount" />
              <el-table-column label="中奖时间" prop="winTime" />
              <el-table-column label="兑奖状态" prop="status" />
            </template>
            <template v-else>
              <el-table-column v-for="(it,i) in tableHeader" :key="i" :label="it">
                <template v-slot="scoped">
                  <img v-if="obj[i] === 'image'" :src="scoped.row[i]">
                  <p v-else>{{ scoped.row[i] }}</p>
                </template>
              </el-table-column>
            </template>
          </etm-table-page>

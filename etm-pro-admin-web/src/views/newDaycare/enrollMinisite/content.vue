<template>
  <div class="content-wrap">
    <etm-main-body>
      <etm-layout-split>
        <div slot="header">
          <etm-tabs v-model="activeName" @tab-click="handleReset">
        ...
        <div slot="body">
          ...
          >
            <template #after>
              <div v-for="(item, index) in cardData" :key="index" class="flex-item">
                <CardItem
                  :type="activeName"
                  :item="item"
                  class="card"
    ...
    >
      <div class="pop-content">
        <etm-field-label label="有效期" type="left">{{ expireTime }}</etm-field-label>
      </div>
    </etm-pop>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleGotoH5DataAnalysis(item) {
      ...
      this.$router.push({
        path: '/newdaycare_enrollMiniSiteManagement/h5Data',
        query: {
          id: item.id,
    ...
    handleGotoH2Manage(item) {
      ...
      this.$router.push({
        path: '/newdaycare_enrollMiniSiteManagement/h2Manage',
        query: {
          editType: this.activeName,
          id: item.id,
          activeName: '活动管理'

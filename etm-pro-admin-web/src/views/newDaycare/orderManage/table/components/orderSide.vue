    ...
    <etm-layout-split>
      <div class="auditBox">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <etm-field-label label="收费方式">{{ transferChargeType(orderBunchData.chargeType) }}</etm-field-label>
          </el-col>
          <el-col :span="8">
            <etm-field-label label="收费时段">{{ transferChargeTime(orderBunchData) }}</etm-field-label>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="8">
            <etm-field-label label="审核日期">{{ transferDate(orderBunchData.auditTime) }}</etm-field-label>
          </el-col>
          <el-col :span="8">
        ...
        <etm-table :data="orderBunchData.orderBunchItemList" :border="false" :stripe="false">
          ...
          <el-table-column slot="left" label="单价">
            <template slot-scope="scoped">
              <div class="actual">实收：¥ {{ formatMoney(scoped.row.actualUnitAmount) }}</div>
              <div class="origin">应收：¥ {{ formatMoney(scoped.row.unitAmount) }}</div>
            </template>
          </el-table-column>
          <el-table-column slot="left" label="数量" prop="number" />
          <el-table-column slot="left" label="小计">
            <template slot-scope="scoped">
              <div class="actual">实收：¥ {{ formatMoney(scoped.row.actualAmount) }}</div>
              <div class="origin">应收：¥ {{ formatMoney(scoped.row.totalAmount) }}</div>
            </template>
          </el-table-column>
        </etm-table>
            ...
            >
              <div class="totalBox">
                <el-row class="total">
                  ...
                  <el-col :span="4">
                    <etm-field-label label="应收总价">¥ {{ formatMoney(item.totalAmount) }}</etm-field-label>
                  </el-col>
                  <el-col :span="3">
                    <etm-field-label label="订金">¥ {{ formatMoney(item.depositAmount) }}</etm-field-label>
                  </el-col>
                  <el-col :span="4">
              ...
              <el-collapse-item :name="index">
                <div class="rowBox">
                  ...
                  <div v-if="item.orderItemList.length" class="rowBody">
                      ...
                      <div class="row column">
                        ...
                        <div class="col colItem" style="width: 28%;">
                          <div>
                            ...
                            <div v-else class="actual">实收：¥ {{ formatMoney(orderItem.actualAmount) }}</div>
                            <div class="origin">应收：¥ {{ formatMoney(orderItem.totalAmount) }}</div>
                          </div>
                        </div>
                        <div class="col colItem" style="width: 7%;">
                          <etm-text v-if="drawerType === 'edit'" type="danger" @click="removeItem(ind, index)">删除</etm-text>
                        </div>
                      </div>
                        ...
                        <div v-for="(orderItemItem, i) in orderItem.orderItemItemList" :key="i" class="row childColumn">
                          ...
                          <div class="col colItem" style="width: 28%;">
                            <div>
                              ...
                              <div v-else class="actual">实收：¥ {{ formatMoney(orderItemItem.actualAmount) }}</div>

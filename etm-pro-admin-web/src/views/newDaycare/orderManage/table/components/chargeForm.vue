  ...
  <div class="chargeForm-wrap">
    <etm-form :model="formData" :rules="rule" type="dialog">
      <el-form-item label="账单名称" prop="orderBunchName">
        <el-input ref="first" v-model="formData.orderBunchName" placeholder="请输入账单名称（不能超过10字）" />
      </el-form-item>
      <el-form-item label="收费方式" prop="chargeType">
        <el-radio-group v-model="formData.chargeType" @change="changeChargeType">
          <el-radio label="PER_MONTH">按月</el-radio>
          <el-radio label="PER_TERM">按学期</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="formData.chargeType === 'PER_MONTH'" label="收费时段" prop="chargeRangeTime">
        <el-date-picker
      ...
      <el-form-item v-else label="收费时段" prop="termId">
        <el-select v-model="formData.termId" @change="changeTerm">
    ...
    <div class="itemTable">
      ...
      <div class="buttonBox">
        ...
        <el-button type="dashed" @click="selectCourse"><i class="iconfont iconplus" />选择特色课课程</el-button>
      </div>
      ...
      <!--          <template slot-scope="scoped">-->
      <!--            <span v-if="scoped.row.itemType === '物品费用' && scoped.row.property === 'cycle'" class="disabledNumber">{{ scoped.row.number }}</span>-->
      <!--            <el-input-number-->
      <!--              v-else-->
      ...
      <!--            <span>-->
      <!--              {{-->
      <!--                scoped.row.itemType === '物品' ? scoped.row.company : scoped.row.itemType === '物品费用' && scoped.row.property === 'once' ? '笔' : ''-->
      <!--              }}-->
      <!--            </span>-->
      <!--          </template>-->
      <!--        </el-table-column>-->
      ...
      <div class="rowBox">
        ...
        <div class="rowBody">
          <div v-for="(orderItem, index) in formData.orderBunchItemList" :key="index" class="itemColumn">
            <div class="row column">
              <div class="col colItem" style="width: 26%;">
                <div class="itemImg">
                  <img src="../../../../../assets/images/etm-logo.png">
                </div>
                <div class="name">{{ orderItem.orderBunchItemName }}</div>
              </div>
              ...
              <div class="col colItem" style="width: 28%;">
                <div class="colSubtotal">
                  ...
                  <div v-else-if="orderItem.orderItemItemList && orderItem.orderItemItemList.length > 1">实收：¥
              ...
              <div class="col colItem" style="width: 7%;">
                <etm-text type="danger" @click="removeItem(index)">删除</etm-text>
              </div>
            </div>
            <template v-if="orderItem.orderItemItemList && orderItem.orderItemItemList.length > 1">
              <div v-for="(orderItemItem, ind) in orderItem.orderItemItemList" :key="ind" class="row childColumn">
                <div class="col colItem" style="width: 26%;" />
                <div class="col colItem" style="width: 18%;">
                  <div>
      ...
      <div class="selPop">
        ...
        >
          ...
          <template v-if="selPopType === 'materialPrice'">
            <el-table-column slot="right" label="单价">
              <template slot-scope="scoped">
                {{ scoped.row.sellPrice + (scoped.row.property === 'once' ? '元/笔' : '元/月') }}
              </template>
            </el-table-column>
          </template>
        </etm-table>
...
import { queryGuardPriceList } from '@/api/newDaycare/priceGuard'
          ...
          if (!flag) {
            ...
            if (item.property === 'cycle') {
              ...
              if (monthStart && monthEnd) {
                ...
                const timeArr = this.getYearAndMonth(monthStart, monthEnd)
                data.totalAmount = item.sellPrice * timeArr.length
                data.actualAmount = item.sellPrice * timeArr.length
                for (let i = 0; i < timeArr.length; i++) {
                  const itemData = {
                    itemType: '物品费用',
                    orderItemName: item.materialPriceName,
                    number: 1,
                    month: timeArr[i],
                    unitAmount: item.sellPrice,
                    actualUnitAmount: item.sellPrice,
                    totalAmount: item.sellPrice,
                    actualAmount: item.sellPrice
                  }

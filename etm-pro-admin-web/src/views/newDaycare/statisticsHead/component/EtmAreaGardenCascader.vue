<template>
  <div class="EtmAreaGardenCascader-wrap">
    ...
    <el-select v-model="organizationValue[0]" placeholder="请选择区域" @change="select($event, 'area')">
      <el-option v-for="(area, index) in areaList" :key="index" :label="area.label" :value="area.value" />
    </el-select>
    <el-select v-model="organizationValue[1]" placeholder="请选择园所" @change="select($event, 'organization')">
      <el-option v-for="(organization, index) in organizationList" :key="index" :label="organization.label" :value="organization.value" />
    </el-select>
  </div>

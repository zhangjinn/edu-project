      ...
      <etm-layout-split>
            ...
            <el-tab-pane :label="tabPlanLabel">
              <etm-tool-bar v-show="true" v-model="formData" @search="handleSearch" @reset="handleReset">
                <el-form-item label="园所：">
                  <etm-area-garden-cascader :clear="clear" :has-default="true" @change="getOrganizationId" />
                </el-form-item>
                <el-form-item label="幼儿姓名：">
                  <el-input v-model="searchForm.childName" placeholder="请输入幼儿姓名" />
                </el-form-item>
                <el-form-item label="状态：">
                  <el-select v-model="searchForm.status">
                    <el-option label="在园" value="IN" />
                    <el-option label="退园" value="OUT" />
                  </el-select>
                </el-form-item>
              </etm-tool-bar>
            </el-tab-pane>
        ...
        <div slot="body">
          ...
          >
            ...
            <el-table-column slot="left" label="幼儿姓名" prop="childName" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.childName }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="所在园所" prop="organizationName" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.organizationName }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="班级" prop="clazzName" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.clazzName }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="状态" prop="status" width="120">
              <template slot-scope="scoped">
            ...
            <el-table-column slot="left" label="应出勤（天）" prop="shouldArriveDays" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.shouldArriveDays }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="已出勤（天）" prop="arrivedDays" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.arrivedDays }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="缺勤（天）" prop="absenceDays" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.absenceDays }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="请假（天）" prop="requestForLeaveDays" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{
            ...
            <el-table-column slot="left" label="结算科目" prop="settlementItem" width="120">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''">{{ scoped.row.settlementItem }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="预收金额（元）" prop="balance" align="right" width="150">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main' : ''">{{ formatMoney(scoped.row.balance) }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="left" label="结算规则" prop="settlementRule" width="360">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main textCenter' : ''" v-html="scoped.row.settlementRule" />
              </template>
            </el-table-column>
            <el-table-column slot="left" label="应退金额（元）" prop="toBeSettledAmount" align="right" width="150">
              <template slot-scope="scoped">
            ...
            <el-table-column slot="left" label="结算可退金额（元）" prop="actualSettlementAmount" align="right" width="180">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main' : ''">{{
            ...
            <el-table-column slot="left" label="应消金额（元）" prop="amountPayable" align="right" width="150">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main' : ''">{{ formatMoney(scoped.row.amountPayable) }}</div>
              </template>
            </el-table-column>
            <el-table-column slot="right" label="实消金额（元）" prop="actualConsumptionAmount" align="right" width="150">
              <template slot-scope="scoped">
                <div :class="getClass(scoped.$index) ? 'main' : ''">{{

<template>
  <div class="editCustomer-wrap">
    ...
    <etm-form ref="form" type="dialog" :model="customerDetail">
      <el-form-item label="学员姓名" prop="customerName" :rules="formValidate.nullValueRule(true,'请输入学员姓名')">
        <el-input v-model="customerDetail.customerName" />
      </el-form-item>
      <el-form-item label="联系人姓名" prop="parentName" :rules="formValidate.nullValueRule(true,'请输入联系人姓名')">
        <el-input v-model="customerDetail.parentName" />
      </el-form-item>
      <el-form-item label="联系人电话" prop="parentPhone" :rules="formValidate.nullValueRule(true,'请输入联系人电话')">
        <el-input v-model="customerDetail.parentPhone" />
      </el-form-item>
      <el-form-item label="性别" prop="gender">
        <template>
          <el-radio v-model="customerDetail.gender" label="MALE">男</el-radio>
          <el-radio v-model="customerDetail.gender" label="FEMALE">女</el-radio>
        </template>
      </el-form-item>
      ...
      <el-form-item label="学员来源" prop="birthday">
        <el-select v-model="customerDetail.sourcesName" style="width: 100%">
          <el-option v-for="(item, index) in sourcesList" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
    </etm-form>
  </div>

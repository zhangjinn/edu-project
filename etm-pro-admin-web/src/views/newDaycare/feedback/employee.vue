  ...
  >
    ...
    >
      <template slot-scope="{row}">
        ...
        >查看图片</etm-text>
        <etm-text v-if="row.status === 'PENDING' || row.status === 'PROCESSED'" type="primary" @click="$emit('handle', row)">回复</etm-text>
        <etm-text v-if="row.status === 'PROCESSED'" type="primary" @click="$emit('result', row)">回复详情</etm-text>
      </template>
    </el-table-column>
  </etm-table-page>
</template>
  ...
  methods: {
    ...
    refreshTableData() {
      const options = {
        ...
        startDate: Array.isArray(this.filter[2].value) ? this.filter[2].value[0] : null,
        endDate: Array.isArray(this.filter[2].value) ? this.filter[2].value[1] : null,
        confirmStartDate: Array.isArray(this.filter[1].value) ? this.filter[1].value[0] : null,
        confirmEndDate: Array.isArray(this.filter[1].value) ? this.filter[1].value[1] : null,
        employeeName: this.filter[3].value || null,

  ...
  >
    <el-table-column slot="left" label="头像" prop="avatar" align="center" width="80" fixed="left">
      <template slot-scope="{row}">
        <div class="avatar">
          <img v-if="row.avatar !== '--' && row.avatar" :src="row.avatar" alt="avatar">
          <img v-else :src="defaultAvatar" alt="defaultAvatar">
        </div>
      </template>
    </el-table-column>
    <el-table-column
    ...
    >
      <template slot-scope="{row}">
        ...
        >查看图片</etm-text>
        <etm-text v-if="row.status === 'PENDING'" type="primary" @click="$emit('edit', row.id)">编辑</etm-text>
        <etm-text v-if="row.status === 'PENDING'" type="primary" @click="$emit('cancel', row.id)">撤销</etm-text>

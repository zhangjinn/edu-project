      ...
      <etm-layout-split>
        <div slot="header">
          <etm-tabs v-model="activateName">
            <el-tab-pane label="已确认" name="alreadySure">
              <etm-tool-bar v-model="alreadySureFormData" @search="search" />
            </el-tab-pane>
            <el-tab-pane label="已归还" name="alreadyGiveBack">
              <etm-tool-bar v-model="alreadyGiveBackFormData" @search="search" />
            </el-tab-pane>
          </etm-tabs>
        </div>
        <div slot="body">
          <template v-if="activateName === 'alreadySure'">
            ...
            >
              <etm-table-tool-bar slot="tool">
                <div slot="right">
                  <el-button type="primary" @click="giveBack(null)">归还</el-button>
                  <el-button type="primary" @click="lendRegisterPop = true">借阅登记</el-button>
                </div>
              </etm-table-tool-bar>
              <template slot="left">
                <el-table-column
                ...
                >
                  <template slot-scope="{ row }">
                    <template v-if="row.bookBarcode !== '--'">
                      <etm-text type="primary" @click="giveBack(row.bookBarcode, row.bookLendingDetailsId)">归还</etm-text>
                      <etm-text v-if="row.bookStatus !== 'REPORTED_LOSS'" type="primary" @click="renewal(row.bookLendingDetailsId)">续借</etm-text>
                      <etm-text v-if="row.bookStatus !== 'REPORTED_LOSS'" type="primary" @click="breakage(row.bookLendingDetailsId)">报损</etm-text>
                    </template>
                  </template>
                </el-table-column>
              </template>
        ...
        >
          <template v-if="!giveBackLibrarySearch">
            <etm-field-label type="left" label="书刊条码">{{ giveBackPopData.bookBarcode }}</etm-field-label>
            <etm-field-label type="left" label="图书名称">{{ giveBackPopData.bookManageName }}</etm-field-label>
            <etm-form ref="giveBackPopRef" :model="giveBackPopData" :rules="giveBackPopRules" type="dialog">
              <el-form-item label="图书位置" prop="storageLocation">
                <el-input v-model="giveBackPopData.storageLocation" />
              </el-form-item>
            </etm-form>
            <etm-field-label type="left" label="学员电话">{{ giveBackPopData.phone }}</etm-field-label>
            <etm-field-label type="left" label="学员名称">{{ giveBackPopData.childName }}</etm-field-label>
            <etm-field-label type="left" label="到期日期">{{ giveBackPopData.dateOfExpiry }}</etm-field-label>
          </template>
          <template v-else>
            <etm-form v-if="giveBackPop" ref="giveBackPopRef" :model="giveBackPopData" :rules="giveBackPopRules" type="dialog">
              <el-form-item label="书刊条码" prop="bookBarcode">
                <el-input v-model="giveBackPopData.bookBarcode" v-in-focus @blur="blurSearch" />
              </el-form-item>
              <etm-field-label type="left" label="图书名称">{{ giveBackPopData.bookManageName }}</etm-field-label>
              <el-form-item label="图书位置" prop="storageLocation">
                <el-input v-model="giveBackPopData.storageLocation" />
          ...
          <etm-layout-split>
            <div slot="default">
              ...
              <template v-if="Object.keys(renewalDetailData).length > 1">
                <etm-title size="big" :show-line="false" border title="详情">
                  <template slot="tool">
                    <el-button type="primary" @click="renewalPop = false">取消</el-button>
                    <el-button type="primary" @click="renewalPopSave">保存</el-button>
                  </template>
                </etm-title>
                <div class="content">
                  <div class="cover-img">
                    <div class="cover">
                      <img v-if="renewalDetailData.cover[0].url" :src="renewalDetailData.cover[0].url" alt="">
                      <img v-else :src="bookDefaultCover" alt="">
                    </div>
                  </div>
                  <!--                  <etm-field-label type="left" label="封面">-->
                  <!--                    <img :src="bookDetailPopData.imgSrc" alt="" style="width: 100px;height: 100px;">-->
                  <!--                  </etm-field-label>-->
                  <etm-field-label type="left" label="名称">{{ renewalDetailData.bookManageName }}</etm-field-label>
                  <etm-field-label type="left" label="书刊条码">{{ renewalDetailData.bookBarcode }}</etm-field-label>
                  <etm-field-label type="left" label="类型">{{ renewalDetailData.bookTypeName }}</etm-field-label>
                  <etm-field-label type="left" label="作者">{{ renewalDetailData.author }}</etm-field-label>
                  <etm-field-label type="left" label="出版社">{{ renewalDetailData.publishingHouse }}</etm-field-label>
                  <etm-field-label type="left" label="学员名称">{{ renewalDetailData.childName }}</etm-field-label>
                  <etm-field-label type="left" label="到期日期">{{ renewalDetailData.dateOfExpiry }}</etm-field-label>
                  <etm-field-label type="left" label="借阅日期">{{ renewalDetailData.borrowingDate }}</etm-field-label>
                  <etm-form ref="renewalFormRef" :model="renewalDetailData" type="dialog">
                    <el-form-item
                      label="续借时长"
                    ...
                    >
                      <el-input v-model="renewalDetailData.renewalDuration" @input="renewalDurationChange">
                        <div slot="append">个月</div>
                      </el-input>
                    </el-form-item>
                  </etm-form>
                  <etm-field-label type="left" label="续后到期">{{ renewalDetailData.renewalOfAfter }}</etm-field-label>
                </div>
              </template>
            </div>
          </etm-layout-split>
            ...
            <div slot="default">
              ...
              <template v-if="Object.keys(borrowDetailDrawerData).length !== 0">
                ...
                <div class="content">
                  ...
                  <etm-field-label type="left" label="借阅时长">{{ borrowDetailDrawerData.duration }}个月</etm-field-label>
        ...
        >
          <etm-layout-split>
            ...
            >
              <el-form-item label="书刊条码" prop="bookBarcode">
                <el-input v-model="lendRegisterPopData.bookBarcode" v-in-focus @change="lendRegisterPopBookBarcodeChange" />
              </el-form-item>
              <etm-field-label type="left" label="图书名称">{{ lendRegisterPopData.bookManageName }}</etm-field-label>
              <etm-field-label type="left" label="图书位置">{{ lendRegisterPopData.storageLocation }}</etm-field-label>
              <el-form-item label="学员姓名" prop="childName">
                <etm-text
                  v-if="lendRegisterPopData.childName"
                  type="primary"
                  @click="chooseChildPop=true"
                >{{ lendRegisterPopData.childName }}</etm-text>
                <el-button v-else type="primary" icon="el-icon-plus" @click="chooseChildPop=true">选择学员</el-button>
              </el-form-item>
              <etm-field-label type="left" label="借阅日期">{{ lendRegisterPopData.createDate }}</etm-field-label>
              <el-form-item label="借阅时长" prop="duration">
                <el-input v-model="lendRegisterPopData.duration" @input="durationChange">
                  <div slot="append">个月</div>
                </el-input>
              </el-form-item>
              <etm-field-label type="left" label="到期日期">{{ lendRegisterPopData.endDate }}</etm-field-label>
            </etm-form>
          </etm-layout-split>
        </etm-pop>

        <!--        选择学员弹窗-->
        <choose-child :show-pop.sync="chooseChildPop" @chooseChildInfo="chooseChildInfo" />
      </etm-layout-split>
    </etm-main-body>
  </div>

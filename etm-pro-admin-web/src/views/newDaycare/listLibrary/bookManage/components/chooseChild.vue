<template>
  <div class="chooseChild-wrap">
    <etm-pop
      ...
      @close="handleCancel"
    >
      <etm-tool-bar :value="formData" @search="queryChild" />
      <etm-table :data="tableData" :columns="columns">
        <template slot="right">
          <el-table-column show-overflow-tooltip prop="typeList" label="学员类型">
        ...
        <template slot="right">
          <el-table-column prop="op" label="操作">
            <template slot-scope="{row}">
              <etm-text type="primary" @click="handelChoose(row)">选择</etm-text>
            </template>
          </el-table-column>
        </template>
      </etm-table>

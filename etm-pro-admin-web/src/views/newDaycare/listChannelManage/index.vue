<template>
  <div class="index-wrap">
    <etm-main-body>
        ...
        <div slot="body">
          ...
          >
            <etm-table-tool-bar slot="tool">
              <template slot="right">
                <el-button type="primary" icon="el-icon-plus" @click="showAddPopup">添加渠道</el-button>
            ...
            <template slot="left">
              <el-table-column label="渠道名称" prop="channelName">
                <template slot-scope="{row}">
                  <etm-text @click="channelName(row)">{{ row.channelName }}</etm-text>
                </template>
              </el-table-column>
            </template>
            <template slot="right">
              <el-table-column label="操作" prop="operate">
                <template slot-scope="{row}">
                  ...
                  <etm-text type="danger" @click="handleDelete(row)">删除</etm-text>
                </template>
              </el-table-column>
            </template>
          </etm-table-page>
      ...
      >
        <etm-layout-split>
          <etm-title size="big" :show-line="false" border title="编辑渠道">
            <template slot="tool">
              <template v-if="isEdit">
                <div class="operate">
                  <el-button size="mini" @click="cancelSide">取消</el-button>
                  <el-button type="primary" size="mini" @click="submitSideChannel('update')">保存</el-button>
                </div>
              </template>
              <template v-else>
                <div class="operate">
          ...
          <div class="content">
            <template v-if="isEdit">
              <etm-form ref="sideFormRef" type="dialog" :model="sideFormData" :rules="sideFormRules">
                <el-form-item label="渠道名称" prop="channelName">
                  <el-input v-model="sideFormData.channelName" placeholder="请输入名称" />
                </el-form-item>
                <el-form-item label="渠道类型" prop="channelTypeId">
                  <el-select v-model="sideFormData.channelTypeId" placeholder="请选择渠道类型">
                    <el-option
                ...
                <el-form-item label="渠道联系人" prop="channelPeople">
                  <el-input v-model="sideFormData.channelPeople" placeholder="请输入渠道联系人" />
                </el-form-item>
                <el-form-item label="渠道负责人" prop="employeeId">
                  <el-select v-model="sideFormData.employeeId" placeholder="请选择渠道负责人">
                    <el-option
                ...
                <el-form-item label="渠道联系人电话" prop="telephone">
                  <el-input v-model="sideFormData.telephone" placeholder="请输入渠道联系人电话" maxlength="11" />
                </el-form-item>
            ...
            <template v-else>
              <etm-field-label type="left" label="渠道名称">{{ sideFormData.channelName }}</etm-field-label>
              <etm-field-label type="left" label="渠道类型">{{ sideFormData.channelTypeName }}</etm-field-label>
              <etm-field-label type="left" label="渠道联系人">{{ sideFormData.channelPeople }}</etm-field-label>
              <etm-field-label type="left" label="渠道负责人">{{ sideFormData.employeeName }}</etm-field-label>
              <etm-field-label type="left" label="渠道联系人电话">{{ sideFormData.telephone }}</etm-field-label>
              <etm-field-label type="left" label="所在城市">{{ sideFormData.siteName }}</etm-field-label>
              <etm-field-label type="left" label="备注">{{ sideFormData.remark }}</etm-field-label>
            </template>
          </div>
        </etm-layout-split>
      </etm-drawer>
      ...
      >
        <template v-if="showPop">
          <etm-form ref="sideFormRef" type="dialog" :model="sideFormData" :rules="sideFormRules">
            <el-form-item label="渠道名称" prop="channelName">
              <el-input ref="first" v-model="sideFormData.channelName" placeholder="请输入名称" />
            </el-form-item>
            <el-form-item label="渠道类型" prop="channelTypeId">
              <el-select v-model="sideFormData.channelTypeId" placeholder="请选择渠道类型">
                <el-option

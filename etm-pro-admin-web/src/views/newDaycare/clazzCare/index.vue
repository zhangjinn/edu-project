  ...
  <div class="index-wrap">
    <etm-layout-split>
      <template slot="header">
        <etm-tabs ref="tabs" v-model="activeName" :setting="hasAuthOfBool('daycare.course.setting')" @setting="toSetting">
          <el-tab-pane v-if="hasAuth('daycare.clazzCare.queryBatch', 'tabs', 'activeName')" label="托育班级" name="nurseries">
            <etm-tool-bar
      ...
      <template slot="body">
        <keep-alive>
          <component :is="tabComponent[activeName]" :search="searchInfo" />
        </keep-alive>
      </template>
    </etm-layout-split>
  </div>
...
export default {
  ...
  methods: {
    ...
    toSetting() {
      this.$router.push('/newdaycare_clazzCareSetting')
    },
    handleSearch(e, key) {
      const obj = {}

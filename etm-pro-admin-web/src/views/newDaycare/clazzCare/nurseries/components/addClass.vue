    ...
    <etm-layout-split>
      <etm-title :show-line="false" border size="big">
        ...
        <template slot="tool">
          ...
          <template v-else>
            <el-button v-has="{authId : 'daycare.clazzCare.update'}" plain size="mini" @click="handleEdit">编辑</el-button>
          </template>
        </template>
      </etm-title>
      ...
      <el-row>
        <el-col :span="14">
          <div v-if="!is_edit">
            <etm-field-label label="班级名称" type="left">{{ form.caeClazzName }}</etm-field-label>
            <etm-field-label label="教室" type="left">{{ form.clazzRoomName }}</etm-field-label>
            ...
            <etm-field-label label="老师" type="left">{{ form.teacherNameLists }}</etm-field-label>
            <etm-field-label label="班级容量" type="left">{{ form.fullNumber }}</etm-field-label>
            <etm-field-label label="适用年龄段" type="left">{{ form.generationName }}</etm-field-label>
          ...
          <etm-form v-else ref="addClass" type="dialog" :model="form">
            ...
            >
              <el-input v-model="form.caeClazzName" placeholder="请输入班级姓名" value="" />
            </el-form-item>
            ...
            <el-form-item label="适用年龄段" prop="generationId" :rules="formValidate.nullValueRule(true, '请选择年龄段')">
              <el-select v-model="form.generationId" placeholder="请选择适用年龄段">
                <el-option v-for="(item, index) in daycareGenerationList" :key="index" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
      ...
      >
        ...
        <etm-table-tool-bar slot="tool">
          ...
          <div slot="right">
            <el-button :disabled="!form.enable" size="small" type="primary" @click="newStudentOpen"><i class="el-icon-plus" />&nbsp;&nbsp;添加学员
            </el-button>
            <el-button
              :disabled="!form.enable"
              size="small"
        ...
        <el-table-column slot="right" label="操作" width="120" align="center" header-align="center">
          <template slot-scope="scoped">
            <etm-text type="primary" @click="exchangeClasses([scoped.row])">调班</etm-text>
            <etm-text type="danger" @click="getoutClasses(scoped.row)">退班</etm-text>
          </template>
        </el-table-column>
      </etm-table>

...
import {
  ...
  querySettlementRuleById,
  querySettlementRuleList,
  updateSettlementRule,
  updateSettlementRuleEnable
...
export default {
  ...
  methods: {
    getData() {
      querySettlementRuleList().then(res => {
    ...
    switchChange(row) {
      ...
      updateSettlementRuleEnable(param).then(() => {
        this.getData()
      })
    },
    add() {
    ...
    popConfirm() {
      this.$refs.popForm.$children[1].$children[0].validate(valid => {
        if (valid) {
          ...
          createSettlementRule(param).then(() => {
    ...
    getDetail() {
      querySettlementRuleById({ ruleId: this.ruleData.ruleId }).then(res => {
        const data = res.data
        this.formData = {
        ...
        if (data.type === 'ATTENDANCE') {
          ...
          this.formData.attendanceRuleList = [{
            comparisonOperator: '',
            days: undefined,
            settlementOperation: '',
            money: undefined,
    ...
    confirmDrawer() { // 编辑规则进行保存
      this.$refs.drawerForm.$children[1].$children[0].validate(valid => {
        if (valid) {
          ...
          updateSettlementRule(param).then(() => {
    ...
    remove(row) { // 删除规则
      this.$etmTip.init({
        ...
        cancelBtn: '取消',
        confirm: () => {
          removeSettlementRule({ ruleId: row.ruleId }).then(() => {

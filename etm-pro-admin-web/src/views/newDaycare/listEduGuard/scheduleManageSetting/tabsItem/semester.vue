<template>
  <div class="callWrap">
    <etm-title :show-line="false" border>当前学期</etm-title>
    <semester-template
      :data.sync="currentData"
      :no-slot="['color']"
      @timeChange="updateNew($event,currentData)"
    />
    <etm-title :show-line="false" border>其他学期</etm-title>
    <semester-template
      :data.sync="dragData"
      btn-text="添加新类型"
      :no-slot="['color']"
      ...
      @toTop="toTop"
      @delete="deleteItem"
      @timeChange="updateNew($event,dragData)"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    deleteItem(e) {
      ...
      delSemesterSetting({ id: this.dragData[e[0].index].id }).then(res => {
    ...
    toTop(e) {
      if (!this.dragData[e[0].index].id) {
        return this.$message.warning('请先完善内容')
      }

      this.$confirm('确定要设置为当前学期?', {
        type: 'warning',
        confirmButtonClass: 'el-button--warning'
      }).then(() => {
        updateCurrentSemesterSetting({
          id: this.dragData[e[0].index].id,
          currentTerm: true
        }).then((res) => {
          const newObj = JSON.parse(JSON.stringify(this.dragData[e[0].index]))
          newObj.currentTerm = true

          if (this.currentData.length) {
            const oldObj = JSON.parse(JSON.stringify(this.currentData[0]))
            oldObj.currentTerm = false
            this.dragData.splice(e[0].index, 1, oldObj)
            this.currentData.splice(0, 1, newObj)
          } else {
            this.dragData.splice(e[0].index, 1)
    ...
    updateNew(index, list) {
      ...
      if (target.id) {
        // 存在id，修改
        data.id = target.id
        putSemesterSetting(data).then(res => {
    ...
    heavy() {
      const startTimeArr = []
      const endTimeArr = []
      const allTimeArr = this.dragData.concat(this.currentData)
      allTimeArr.forEach((item) => {

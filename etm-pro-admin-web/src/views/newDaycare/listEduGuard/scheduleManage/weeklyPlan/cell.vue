  ...
  <div class="EtmCell">
    <div class="EtmCellInner" :style="{ background:bgColor(data.color) }">
      <ul>
        ...
        <li v-if="data.courseName" class="item">
          {{ data.courseName }}
        </li>
      ...
      <div v-show="data.courseName" class="popTip">
        <div class="popTipInner">
          <span v-has="{authId: 'guardcare.scheduleWeek.update'}" @click.stop="HandleEdit">编辑</span>
          <span v-has="{authId: 'guardcare.scheduleWeek.remove'}" @click.stop="HandleDelete">删除</span>
        </div>
      </div>
    </div>
  </div>

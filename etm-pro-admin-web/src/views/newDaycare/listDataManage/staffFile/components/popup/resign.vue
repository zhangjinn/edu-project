<template>
  <etm-form ref="etmForm" type="dialog" :model="resignData">
    <el-form-item label="已选员工">
      <span>{{ resignData.employeeRecordName }}</span>
    </el-form-item>
    <el-form-item label="离职原因" required>
      <el-select v-model="resignData.headquartersSettingId" @change="handleTemplateChange">
        <el-option v-for="(temp, i) of resignTemplates" :key="i" :label="temp.label" :value="temp.value" />
      </el-select>
    </el-form-item>
    <el-form-item label="备注" prop="content">
      <el-input v-model="resignData.remarks" type="textarea" :rows="4" />
    </el-form-item>
  </etm-form>
</template>

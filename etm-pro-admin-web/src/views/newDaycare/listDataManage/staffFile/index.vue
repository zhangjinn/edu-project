    ...
    <etm-layout-split>
      <div slot="header">
        <etm-tabs v-model="activeName" :setting="true" @setting="setting">
          <el-tab-pane name="staffOnActive" label="在职员工">
            <etm-tool-bar v-model="formData1" @search="handleSearch('staffOnActive',formData1)" @reset="reset" />
          </el-tab-pane>
          <el-tab-pane name="staffOffActive" label="离职员工">
            <etm-tool-bar v-model="formData2" @search="handleSearch('staffOffActive',formData2)" @reset="reset" />
          </el-tab-pane>
        </etm-tabs>
      </div>
      <div slot="body">
        <staff-on-table v-if="activeName === 'staffOnActive'" ref="staffOnActive" :store-detail-tip-data="storeDetailTipData" @update="updateDigitalData" />
        <staff-off-table v-if="activeName === 'staffOffActive'" ref="staffOffActive" :store-detail-tip-data="storeDetailTipData" />
      </div>
    </etm-layout-split>
  </div>
...
export default {
  ...
  methods: {
    ...
    setting() {
      this.$router.push({ name: 'staffSetting' })
    },

<template>
  <div>
    <template v-if="recommendedRecipeData.length > 0">
      <div v-for="(recipe, index) in recommendedRecipeData" :key="index" class="pop-recommend-item">
        <div class="nav">
          <div class="nav-left">
            <!--            <span v-for="(category, ii) in recipe.suitableAge" :key="ii">-->
            <!--              {{ category }}-->
            <!--              <span v-if="ii < recipe.suitableAge.length-1">, </span>-->
            <!--            </span>-->
            <span>{{ (recipe.suitableAge || []).join(', ') }}</span>
          </div>
          <div v-show="recipe.weekRecipeList.length >= 3" class="nav-right">
            <i class="iconfont iconleft-circle" :class="{'noClick':!recipe.showLeftIcon}" @click="handleMoveLeft(index)" />
            <i class="iconfont iconright-circle" :class="{'noClick':!recipe.showRightIcon}" @click="handleMoveRight(index)" />
          </div>
        </div>
        <div class="recipe-classify-scroll">
          <ul class="recipe-classify-wrapper">
            <li v-for="(item, idx) in recipe.weekRecipeList" :key="idx" :class="`recipe-classifys-item-${index}`" @click="handleRecipeDetail(item.id)">
              <div class="recipe-classify-wrapper-item">
                <div class="recipe-icon"><span class="iconfont iconshipu" /></div>
                <div class="recipe-detail">
                  <div class="name">{{ item.name }}</div>
                  <div class="num">已使用：{{ item.useCount }}</div>
                  <div class="tag">
                    <el-tag v-for="(tag, i) in item.tag" :key="i">{{ tag }}</el-tag>
                  </div>
                </div>
              </div>
            </li>
    ...
    <etm-pop
      ...
      @cancel="popVisibleRecommendRecipeDetail = false"
      @confirm="popVisibleRecommendRecipeDetail = false"
    >
      <div class="popRecommendRecipeDetailContent">
        <div class="popRecipeDetailNav">
          <div class="popRecipeDetailNavLeft">
            <div class="type">
              <!--              <span v-for="(category, ii) in currentRecipeData.suitableAge" :key="ii">-->
              <!--                {{ category }}-->
              <!--                <span v-if="ii < currentRecipeData.suitableAge.length-1">, </span>-->
              <!--              </span>-->
              <span>{{ (currentRecipeData.suitableAge || []).join(', ') }}</span>
            </div>
            <div class="name">{{ currentRecipeData.name }}</div>
            <div class="num">已使用：{{ currentRecipeData.useCount }}</div>
            <div class="tag">
              <el-tag v-for="(tag, i) in currentRecipeData.tag" :key="i">{{ tag }}</el-tag>
            </div>
          </div>
          <div class="popRecipeDetailNavRight">
            <el-button type="primary" @click="handleSelectedRecipe">选择该食谱</el-button>
          </div>
        </div>
        <div class="popRecipeDetailWrapper">
          <div class="left-icon">
            <i class="iconfont iconleft-circle" :class="{'noClick':!showDetailLeftIcon}" @click="handleDetailMoveLeft" />
          </div>
          ...
          <div class="right-icon">
            <i class="iconfont iconright-circle" :class="{'noClick':!showDetailRightIcon}" @click="handleDetailMoveRight" />
          </div>
        </div>
      </div>
    </etm-pop>
...
import { checkRecommendedRecipes, putRecommendedRecipes } from '@/api/newDaycare/recipesManage'
export default {
  ...
  methods: {
    init() {
      queryWeekRecipe().then((res) => {
        ...
        // console.log('this.carouselRecipeIds:')
        // console.log(this.carouselRecipeIds)
      })
    },
    ...
    queryWeekRecipeDetailData(id) {
      queryWeekRecipeDetail(id).then((res) => {
        const result = res.data
        this.currentRecipeData.tableData.content = result.mealSectionList.map(v => {
          return {
            label: v.label.name,
            data: v.data
          }
    ...
    handleSelectedRecipe() {
      if (!this.currentRecipeData.suitableAge.includes(this.generationName)) {
        this.$message.error('请选择该适用年龄下的食谱')
        return
      }
      const param = {
        ...
        if (result) {
          ...
          if (result.result === 'OK') {
            putRecommendedRecipes(param).then((res) => {
              ...
              this.popVisibleRecommendRecipeDetail = false

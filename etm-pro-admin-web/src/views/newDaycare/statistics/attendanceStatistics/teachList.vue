<template>
  <div class="teachList-wrap">
    <etm-table id="xlsxTable" v-fixedHead :columns="colOption" :data="tableData">
      <etm-table-tool-bar slot="tool">
        <div slot="right">
          <el-button plain @click="exportTableToExcel({id: 'xlsxTable', name:'早教考勤统计'})">导出Excel
          </el-button>
        </div>
      </etm-table-tool-bar>
    </etm-table>
...
export default {
  ...
  data() {
    return {
      colOption: [
        {
          label: '学员姓名', prop: 'childName', formatter: (row, column, cellValue) => {
            if (!cellValue) {
              return '空'
            } else {
              return cellValue
          ...
          label: '应到次数', prop: 'shouldNum'
        }, {
          label: '实到次数', prop: 'actuallyNum'
        }, {
          label: '消课课程小计', prop: 'totalTimeItemList', formatter: (row, column, cellValue) => {
            if (!cellValue) {
              return '空'
            } else {
              let v = ''
          ...
          label: '消课总课时', prop: 'totalTime'
        }, {
          label: '请假次数', prop: 'askNum'
        }, {
          label: '缺勤次数', prop: 'absentNum'
        }, {
          label: '出勤率', prop: 'probability', formatter: (row, column, val, index) => {
